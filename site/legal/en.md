...
- **Conversations with the helper** — what an owner typed or said to it, what
  ...
  nobody else can, so a conversation holds one person's words. **They are not
  read by anybody else unless that owner allows it**, on their own page, where
  the switch can be turned off again at any time. Turning it off does not
  delete anything — it means nobody but them reads what is there.
