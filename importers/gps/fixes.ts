...
const importer: GpsImporter = {
  ...
  detect(head, filename) {
    ...
    // The object shape's own leading `[?` — B2240. `parse` already reads a
    // plain top-level array of `{t, lat, lon}` objects (the doc comment
    // above has always claimed it), but this line only ever looked at the
    // first line, and an array literal's first line is `[{"t": …` — one
    // character short of the bare JSON-Lines-object pattern below.
    return /^\[\s*-?[\d.]+\s*,/.test(first) || /^\[?\s*\{\s*"(t|time|timestamp)"/.test(first);
  },
