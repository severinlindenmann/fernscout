 ...
 *
 * That is the iOS shape. The phone export (Android, since 2024 — B1819) skips
 * the `geo:` URI and writes the same pair as `"47.38000°, 8.21000°"` — bare,
 * degree-signed, comma-space-separated. Both are read here so every caller
 * gets the pair without caring which platform wrote the file. */
export function parseGeoUri(value: unknown): { lat: number; lon: number } | undefined {
  if (typeof value !== "string") return undefined;
  const body = value.startsWith("geo:")
    ? value.slice(4)
    : /^-?[\d.]+°,\s*-?[\d.]+°$/.test(value)
      ? value.replace(/°/g, "")
      : undefined;
  if (body === undefined) return undefined;
  const [lat, lon] = body.split(",").map(Number);
