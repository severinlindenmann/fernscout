...
completed: "2026-09-07T13:08:59Z"
---
...
Confirmed the diagnosis in the file before building on it, per the
instruction to reproduce rather than trust a guessed cause: the earlier
"draft filter" hypothesis (B318's territory) was already ruled out on disk —
9 gallery items, 9 derivatives, correct `includeDrafts`/media-route gating —
and `STORY_WINDOW` fully explains the "three of nine" count, as the file
already recorded. What was left was the fragment-link question, and it is
real: `grep -n "hashForDay(" app components lib` found exactly one caller,
`app/TripStory.tsx:281`'s own `history.replaceState`, and no `<a href="#day-…">`
anywhere in the codebase. So nothing *deliberately* hands out a `#day-<slug>`
link — every generated permalink (`lib/feed.ts`, `lib/search.ts`,
`components/StructuredData.tsx`, `app/sitemap.ts`,
`lib/digest/dayWhatsapp.ts`) already uses the real `/day/<slug>` route. The
owner's actual link, `/viki#day-hanoi-nachtleben`, was not generated by any
of those — it was copied straight out of the browser's own address bar,
which `history.replaceState(null, "", hashForDay(day))` was rewriting to a
`#day-…` fragment on every scroll step. That is the mechanism this ticket's
"what is left" section predicted without quite naming it: the fragment *is*
shared, just not through a link anyone wrote.

**Fix, in `app/TripStory.tsx:281`**: the day-step branch of the
`history.replaceState` effect now writes `trip.href(\`/day/${day.slug}\`)`
instead of the bare hash (falling back to the hash only in the — never true
in production — case where `TripStory` renders outside `TripProvider`).
`/day/<slug>` is not new infrastructure: it is the permalink route that
already exists for exactly this
(`app/[user]/(trip)/day/[slug]/page.tsx`, and its `/trips/<id>/…`
counterpart), and it already calls `buildStoryProps(tripId, { openAt:
entry.date, … })` — centred correctly, server-side, on the named day. So a
link copied from the address bar mid-scroll, from now on, is a link that
reopens on the right day when pasted and reloaded, which a `#fragment` (never
sent to the server) could not do. The hash-reading code on mount is left in
place, unchanged, so links already shared with the old `#day-…` shape keep
working exactly as they did (see "what is left" above — a brief loading gap
outside the default window, not landing on nothing).

**Test**: `test/story-day-permalink.test.tsx` — asserts (a) the `replaceState`
call site in `app/TripStory.tsx` writes through `trip.href` rather than the
bare hash, (b) the old hash-reading code on mount is untouched, and (c)
`TripProvider`'s `href()` builds exactly the path the real day-permalink
routes answer to, for both the current trip (`/​<user>/day/<slug>`) and a
past one (`/<user>/trips/<id>/day/<slug>`).

**What this test does not cover, and why**: the actual address-bar rewrite
only happens inside a `useEffect` after a real mount, and `TripStory` always
renders `TripHero` (containing `MiniMap`) somewhere in its tree, which pulls
in `motion/react`, `next/image`, and `ResizeObserver` — none of which
anything in `test/` currently stubs for a real `createRoot` mount (every
existing `TripStory` test uses `renderToStaticMarkup`, which never runs
effects at all). Building that harness for one ticket felt like the wrong
trade against reusing the already-correct `/day/<slug>` route and testing the
wiring at the seam. **A person should**: open a multi-day trip's story page,
scroll a few days in, copy the address bar, open that URL in a new tab, and
confirm it lands on the day just left rather than the trip's default —
before and after this change, to see the difference directly.
