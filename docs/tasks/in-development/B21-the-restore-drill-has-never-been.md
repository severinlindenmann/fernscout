...
4. **Check the failure paths.** — **DONE**, and they are in the same file. See
   ...
   person looks" turned out to be *no* — captured as **B62**.
...
`TimeoutStartSec=30min` is the only bound. Captured as **B62**.
...
and will fire again, **not whether the run succeeded**. Captured as **B62**.
...
- **B62** — a failed nightly backup tells nobody (no `OnFailure=`, and the
