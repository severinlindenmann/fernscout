...
- **Files a date with no day onto the trip itself**, in `filedToTrip` and the
  trip's own `costs.items` — never dropped, and never moved to a neighbouring
  day. A cost you paid is a fact whether or not a day exists for it (B1844).
