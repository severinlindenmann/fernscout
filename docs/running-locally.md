...
### `/legal` shows placeholders until you give it a real imprint

`site/legal/en.md` and `de.md` are the repository's **template** — every
clone ships the same `[Operator name]` / `[Street and number]` /
`[contact email]` placeholders, on purpose (B2249): the operator's own name
and home address must never be the thing a `git clone` republishes.

To see (or serve) a real imprint, drop `en.md`/`de.md` with the real text
under `CONTENT_DIR/legal/` — `lib/legal.ts`'s `legalDir()` reads that folder
first and only falls back to the checkout's template when it is absent. For a
quick local check:

```bash
mkdir -p /tmp/fs-content/legal
cp real-imprint-en.md /tmp/fs-content/legal/en.md
CONTENT_DIR=/tmp/fs-content npm run dev
```

`docs/runbook.md` has the equivalent step for the actual server.
