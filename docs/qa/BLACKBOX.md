...
Mail is written to `<DATA_DIR>/mail/<user>/` — and to `<DATA_DIR>/mail/.mail/`
when it belongs to no journal yet, which is a signup code; every emailed code
is `123456`, and the testers are told that a person reads it to them — which
