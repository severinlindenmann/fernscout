...
export default async function AgentPage({ searchParams }: PageProps<"/agent">) {
  ...
  const owned = identity
    ? // Even with nothing in it — B1019, and since B1708 that is what
      // `journalsFor` answers for every caller: a journal made a minute ago is
      // its owner's, here and on the landing page alike.
      (await journalsFor(identity.email)).filter(
