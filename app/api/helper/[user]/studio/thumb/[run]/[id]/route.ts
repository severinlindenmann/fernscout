 ...
 * - **A derivative, never the original.** `extractThumbnail` makes a WebP
 *   image, or a looping animated WebP for a clip, and caches it inside the
 *   private staging run; the
 ...
 *
 * HEIC/HEIF uses ingest's decoder fallback; a video gets two seconds of
 * animated WebP from ffmpeg, which an `<img>` loops without a player.
 * Missing decoders or corrupt media still return 404 for a quiet placeholder.
 */
export async function GET(
  request: Request,
  { params }: RouteContext<"/api/helper/[user]/studio/thumb/[run]/[id]">,
) {
  ...
  const thumbnail = await extractThumbnail(file, width);
  if (!thumbnail) {
    return new Response("Not found", { status: 404 });
  }
