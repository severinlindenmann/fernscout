...
export async function POST(request: Request, { params }: RouteContext<"/api/helper/[user]/assemble-day">) {
  ...
  const input: DraftInput = {
    ...
    // were real words. Absent, every reader falls back to the date it already
    // knows how to format. The photo import names one (B2081): the place the
    // person answered, or the date in words.
    ...(title ? { title } : {}),
    date,
