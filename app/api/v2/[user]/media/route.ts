...
export async function POST(request: Request, { params }: RouteContext<"/api/v2/[user]/media">) {
  ...
  if (contentType.includes("application/json")) {
    ...
    // Who, and through which door — the two facts about an upload that cannot
    // be recovered from the file afterwards (B1864). The identity is whatever
    // this bearer token resolved to, never anything invented.
    const result = await storeMediaV2(user, intent, upload, { uploadedBy: bearer.session.email, source: "api" });
  ...
  const result = await storeMediaV2(user, intent, { filename: file.name, bytes }, {
    uploadedBy: bearer.session.email,
    source: "api",
  });
  if (!result.ok) return refuseWriteResult(result);
  return ok(result.item, { status: 201 });
