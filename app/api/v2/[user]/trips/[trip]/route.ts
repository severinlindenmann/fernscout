...
/**
 * A 308 to the same call at the trip's current address, when `trip` is an id
 * this journal renamed away and nothing lives at it any more — B2015. `null`
 * when there is nothing to redirect to: `trip` was never renamed, the chain
 * loops back to itself, or the trip it now names does not exist either (a
 * genuine `unknown_trip`, not a stale link).
 */
function renamedTripRedirect(request: Request, user: string, trip: string): Response | null {
  const resolved = resolveRenamedTripId(user, trip);
  if (resolved === trip || !getTrip(tripRef(user, resolved))) return null;
  const url = new URL(request.url);
  url.pathname = url.pathname.replace(`/trips/${encodeURIComponent(trip)}`, `/trips/${encodeURIComponent(resolved)}`);
...
export async function GET(request: Request, { params }: RouteCtx) {
  ...
  // B2218 (found in B2204's own security review): this gate previously
  ...
  // scoped to a single trip); only `mayActAsOwner` is the owner themselves.
  const doc = buildTripDoc(user, trip, stored, daysModeOf(request), {
    includePrivatePlan: mayActAsOwner(bearer.session, user),
  });
...
export async function PUT(request: Request, { params }: RouteCtx) {
  ...
  // B2009 — ids assigned, nights derived (or dates recounted), and
  // `plan.private.stops` checked against the now-final route ids. Has to
  // wait for a successful parse: an unassigned id is not a route id yet.
  if (parsed.data.plan) {
    const { plan, problem } = derivePlan(parsed.data.plan, parsed.data.dates.from);
    if (problem) return fail("invalid_plan", problem, undefined, 400);
    parsed.data.plan = plan as typeof parsed.data.plan;
  }
 ...
 */
/**
 * How a patch is checked against the rest of the document.
 *
 * `whole` is the API's contract: a stored trip must be complete — every
 * required-or-declined section answered — before any write lands, so an
 * agent is told about an open section the first time it touches the trip.
 *
 * `sections` is the studio's: the owner is editing one part of their own
 * trip from a browser, and a trip made in the studio starts with costs,
 * translations, figures, buddies and teaser all still open. Refusing the
 * plan because the party has not been drawn yet is not a contract, it is a
 * wall (B2011, found on the live journal). The sections the patch sends are
 * still validated by their own schemas; only the demand that everything
 * else be answered first is dropped.
 */
...
export async function applyTripPatch(
   ...
   * been declined.
   */
  const hasMedia = mode === "whole" && tripDays(user, trip).some((d) => d.media && d.media.length > 0);
  ...
  // `tripPatch` is `tripBase.partial()`: every section present is checked by
  // its own schema, and nothing asks for the sections that are absent.
  // The stored variants (B2243 review F1): the merged document is partly what
  // is already on disk, so the cost bounds — which judge only what a caller
  // sends — are not re-applied to it.
  const finalParsed = mode === "sections" ? tripPatchStored.safeParse(merged) : tripCreateStored.safeParse(merged);
  ...
  const localeProblem = checkTranslations(finalParsed.data.translations ?? undefined, journal.locales, journal.defaultLocale);
  if (localeProblem?.kind === "invalid") {
    return fail("invalid_translations", localeProblem.message, localeProblem.problems, 400);
  }
  if (localeProblem?.kind === "incomplete" && mode === "whole") {
    return fail("incomplete", ERROR_CODES.incomplete, { missing: localeProblem.missing }, 422);
  }
  ...
  const coverProblem = checkCover(finalParsed.data.cover ?? undefined, coverMediaSrcs);
  if (coverProblem) return fail("invalid_cover", coverProblem, undefined, 400);

  // B2009 — same derivation the PUT route runs, against the merged document:
  // ids assigned, nights derived (or dates recounted), and
  // `plan.private.stops` checked against the now-final route ids.
  if (finalParsed.data.plan) {
    const { plan, problem } = derivePlan(finalParsed.data.plan, finalParsed.data.dates?.from ?? stored.dates.from);
    if (problem) return fail("invalid_plan", problem, undefined, 400);
    finalParsed.data.plan = plan as typeof finalParsed.data.plan;
  }
  ...
  // In `sections` mode the parse was partial, so the type is looser than the
  // file's; the values are the stored document's own plus validated sections,
  // and `applyNullClears` already turned every null into an absence.
  const toWrite = tripFields as TripFile;
