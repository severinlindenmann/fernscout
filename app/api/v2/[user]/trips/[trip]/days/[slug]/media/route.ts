...
function readCurrent(user: string, tripId: string, slug: string, trip: TripFile) {
  ...
  const doc = dayDoc.parse(withResolvedTest(dayEchoInput(stored), trip, stored));
...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  const echo = dayDoc.parse(withResolvedTest(dayEchoInput(result.day), gate.trip, result.day));
