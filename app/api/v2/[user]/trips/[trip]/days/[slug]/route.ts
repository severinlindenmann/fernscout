...
import { deriveCountryCode, type DayFile, type TripFile } from "@/lib/api/v2/documents";
...
export async function PUT(request: Request, { params }: RouteCtx) {
  ...
  if (stored) {
    const currentDoc = dayDoc.parse(withResolvedTest(dayEchoInput(stored, tripRef(user, tripId)), gate.trip, stored));
  ...
  raw.slug = slug;
  // B1907 — before the required-or-declined check below: a caller who names a
  // country the server can place is not asked to decline the code as well.
  ...
  if (dryRun) {
    const preview = dayDoc.parse(withResolvedTest(dayEchoInput(toWrite, tripRef(user, tripId)), gate.trip, toWrite));
  ...
  const echo = dayDoc.parse(withResolvedTest(dayEchoInput(day, tripRef(user, tripId)), gate.trip, day));
 ...
 * merged document is re-validated (`dayPatchedPublished`) so a published day
 * stays provably complete after every change. A draft is checked for shape
...
export async function applyDayPatch(
  ...
  // B1907, and only because this patch named the country: a patch about a
  // caption leaves everything it did not mention exactly as it found it. It
  // runs before the merge, so an old "no code was recorded" decline is
  // retracted by the same rule any other answered decline is.
  deriveCountryCode(patch);
  ...
  const finalParsed = (stored.status === "draft" ? dayPatchedDraft : dayPatchedPublished).safeParse(merged);
  ...
  if (dryRun) {
    const preview = dayDoc.parse(withResolvedTest(dayEchoInput(toWrite, tripRef(user, tripId)), trip, toWrite));
    return ok(preview, { etag: etagFor(preview) });
  }

  // B2245 — the body was awaited since `stored` was read, and a publish (or
  // any other write) landing in that window would be overwritten here —
  // `status: stored.status` putting a just-shared day back to draft. Re-read
  // synchronously right before the write (nothing can interleave between this
  // and `writeDayFile`) and answer the same 409 a stale If-Match gets.
  const now = readDayFile(user, tripId, slug);
  if (JSON.stringify(now) !== JSON.stringify(stored)) {
    if (!now) return fail("unknown_day", ERROR_CODES.unknown_day, undefined, 404);
    return fail("stale_document", ERROR_CODES.stale_document, dayDoc.parse(withResolvedTest(dayEchoInput(now, tripRef(user, tripId)), trip, now)), 409);
  }

  writeDayFile(user, tripId, slug, toWrite);
  const day = await serviceWeather(user, tripId, slug, patch.weather, toWrite);
  const echo = dayDoc.parse(withResolvedTest(dayEchoInput(day, tripRef(user, tripId)), trip, day));
