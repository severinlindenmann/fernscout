...
export async function POST(request: Request, { params }: RouteContext<"/api/v2/[user]/contacts/import">) {
  ...
  const body = (jsonBody.value) as { rows?: unknown } | null;
  ...
  return ok({
    ...
    next:
      filed > 0
        ? `${filed} row${filed === 1 ? " is" : "s are"} pending, each sent one mail asking whether they want anything ` +
          "from this journal — none of them is a recipient, a reader, or anything else until that address " +
