...
/**
 * `GET /api/v2/<user>/figures/presets` — moved from
 * `/api/v1/<user>/travellers/presets` (B1609): every word a figure may be
 * described in, and twelve places to start from. Unchanged in behaviour;
 * only the address and the response envelope (`fail()`) are v2's.
 *
 ...
 * and **the name is never written to a file**. What lands in a figure's own
 * document is `skin: deep, hair: black`; what never lands there is
 * `preset: west-african`, which would be a sentence about somebody's
 * ethnicity in a file the owner did not think they were writing — and false
...
export async function GET(
  ...
  return ok({
    ...
    preview: `${base}/api/v2/${user}/figures/preview?figure={…} draws one, ?party=[{…},{…}] draws the group as the hero will arrange it. Show somebody their figure before writing it down — a person cannot confirm a description they cannot see.`,
    ...
    note:
      ...
      "the name: a preset name in a figure's own document is a claim about somebody's " +
      "background that stops being true the moment they change the hair. Fields nobody " +
