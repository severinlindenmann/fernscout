 ...
 * owner-cookie web door (`app/api/web/[user]/figures/[id]/route.ts`) can
 * reach the exact same validation and ETag rules — the same shape
 * `applyTripPatch` already gives the trip's own web doors, so a figure saved
 * from a browser and one saved by an agent can never quietly diverge on
 * what "a usable figure" means.
 */
export async function applyFigurePut(user: string, id: string, request: Request): Promise<Response> {
  if (!getUser(user)) return fail("no_such_journal", `No journal called "${user}".`, undefined, 404);
 ...
 * owner-cookie web door (`app/api/web/[user]/figures/[id]/route.ts`, B2022)
 * can reach the exact same refusal — a figure deleted from the studio and
 * one deleted by an agent read back the same `referencedBy` shape, same
 * reasoning `applyFigurePut` already gives above.
 */
export async function applyFigureDelete(user: string, id: string, request: Request): Promise<Response> {
  if (!getUser(user)) return fail("no_such_journal", `No journal called "${user}".`, undefined, 404);
