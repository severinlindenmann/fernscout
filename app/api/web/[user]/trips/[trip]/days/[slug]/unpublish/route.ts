...
export async function POST(
  ...
  // `slug` is `Entry.slug` here too — bare — same resolution the sibling
  // PATCH/GET route needs; see `resolveDayStem`'s own doc comment.
  const stem = resolveDayStem(user, tripId, slug);
  if (!stem || !readDayFile(user, tripId, stem)) {
    return Response.json({ error: "unknown_day" }, { status: 404 });
  }

  return applyUnpublish(user, tripId, stem);
}
