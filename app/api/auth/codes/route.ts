 ...
 * owner's own, or somebody on the named trip.
 *
 * Not the instance admin (B1166). `agentScope` in
 * `app/api/auth/codes/redeem/route.ts` never accepted `isAdminEmail` — it
 * asks only whether the redeeming address is the journal's own `owner.email`
 * — so a code granted here on the admin's behalf mailed a code that
 * redemption always refused with `invalid_code`, and `issueCode` had already
 * revoked any code live for that address on the way to writing one nobody
 * could spend. The admin does not need this door: `test/admin-address.test.ts`
 * ("writes to another journal's private trip with its own journal's token")
 * already has them requesting an agent code for *their own* journal and using
 * that token cross-journal, because `ownsUser` (`lib/api/auth.ts`) grants the
 * admin's session every journal regardless of which one minted it. This
 * function only ever gated the request step for a journal the admin does not
 * own by `config.json`, which is redundant with that path and, until now, the
 * only way to reach an unredeemable code.
 */
async function mayRequestAgentToken(
  ...
  const isOwnerOrAdmin = user.owner.email === address;
