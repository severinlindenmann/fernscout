 ...
 *
 * **`write:gps` is refused here too — found beside the `isOwner` widening
 * this route's own `guard` otherwise falls back to (B2204's security
 * review, merge 2026-09-24).** The token is
 * minted with the owner's own address (`issueGpsToken`'s only caller,
 * `POST /api/auth/{user}/gps-token`), so before this check a phone holding
 * nothing but a positions-only credential proved "the owner's own address"
 * here exactly as a trip-scoped token would — and every row on the journal
 * carries that same address, since the owner mints all of their own keys.
 * `visible` below would have been every live key, and `POST` would have
 * let it revoke any of them. A trip-scoped token is still admitted: seeing
 * and revoking its own row is what B323 built this branch for.
 */
async function callerEmail(user: string, request: Request): Promise<string | null> {
  ...
  if (!agent || agent.owner !== user || agent.scope === GPS_IMPORT_SCOPE) return null;
  return agent.email;
}
 ...
 *
 * `write:gps` (B2204) is checked here rather than inside `describeScope`,
 * because this is the one caller allowed to see it — `describeScope`'s own
 * comment says why the other caller, `/status`, must never be handed it. */
function scopeOf(row: { scope: string | null; expiresAt: string }): { scope: "owner" | "trip" | "gps"; trip?: string } {
  if (row.scope === GPS_IMPORT_SCOPE) return { scope: "gps" };
