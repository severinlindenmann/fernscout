    ...
    <main id="main" className="min-h-screen bg-surface-base">
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-16">
        ...
        */}
        ...
        {/*
          The table above checks green-700 and coral-600 against page grounds
          only. Both also sit on a tinted badge fill one layer in — a fill
          that does not appear as a GROUND because nothing else is ever set
          on it — and that pairing failed here even though the page-ground
          rows above pass (B1841). Checked separately, the same way B1798's
          dark-mode badge fills are below.
        */}
        <h3 className="mt-10 font-display text-lg font-semibold text-ink-strong">
          What may carry words, on their own badge fill
        </h3>
        <p className="mt-2 max-w-2xl text-ink-body">
          <code className="rounded bg-surface-subtle px-1">green-700</code> and{" "}
          <code className="rounded bg-surface-subtle px-1">coral-600</code> also carry text
          directly on a tinted fill, one layer in from the page — a badge, a chip, a role=
          &quot;alert&quot;. The fill, not the text, moved to clear 4.5:1 here (B1841).
        </p>
        <div className="mt-6 overflow-x-auto rounded-xl border border-line-quiet bg-surface-raised">
          <table className="w-full min-w-[36rem] text-sm">
            <thead>
              <tr className="border-b border-line-quiet text-left">
                <th className="px-3 py-2 font-display text-ink-strong">Token (on fill)</th>
                <th className="px-3 py-2 font-display text-ink-strong">Ratio</th>
              </tr>
            </thead>
            <tbody>
              {(
                ...
                const fillHex = hex[fillToken];
                const ratio = contrast(hex[textToken], fillHex);
                return (
                  <tr key={`${textToken}-on-${fillToken}-light`} className="border-b border-line-quiet last:border-0">
                    <th scope="row" className="px-3 py-2 text-left font-mono text-xs text-ink-strong">
                    ...
                    <td className="px-3 py-2">
                      <span
                        className="rounded px-2 py-1 font-mono text-xs"
                        style={{ background: fillHex, color: hex[textToken] }}
                      >
                        {ratio.toFixed(2)}
                      </span>
                      <span className="ml-2 text-xs text-ink-secondary">{verdict(ratio)}</span>
