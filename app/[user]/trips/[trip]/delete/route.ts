...
export async function GET(
  ...
  // B2052: a browser opening this address is sent to the trip's page, not handed raw JSON (relative Location, B460).
  if (request.headers.get("sec-fetch-mode") === "navigate") return new Response(null, { status: 303, headers: { location: `/${encodeURIComponent(user)}/studio/trip?trip=${encodeURIComponent(trip)}` } });
