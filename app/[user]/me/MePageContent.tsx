...
/**
 * One trip in "what you can read" — B621, its pencil removed by B2018.
 *
 * The owner's own way to change a trip's title, dates, visibility, id or
 * deletion is the studio now (`/studio/trip?trip=<id>`, reached from the
 * header's Studio tab and from the trip page's own `OwnerTools`), so this row is the plain link every other reader
 * already had — B1412/B2018's Why says why the owner's own read of a trip
 * should not double as the page it is edited from.
 */
function TripRow({
  trip,
  reasonKey,
  ...
  return (
    <li>
      ...
      >
        ...
        {/* `resolveViewer`'s own answer to *why this reader may open it*
            — B1585/B41: the wording is the trip gate's, never this
            component's own. */}
  ...
  return (
    <>
      <ul className="mt-3 divide-y divide-line-quiet overflow-hidden rounded-2xl border border-line-quiet bg-surface-raised">
        {shown.map((trip) => (
          <TripRow key={trip.id} trip={trip} reasonKey={reason[trip.through]} />
        ))}
      </ul>
  ...
  return (
    <div className="min-h-screen">
      ...
      >
          ...
          <section className="mt-6">
            ...
            */}
            {viewer.trips.length === 0 ? (
              ...
              <ReadableTrips trips={viewer.trips} />
            )}
          </section>
        )}
        ...
        */}
        {sessionsShared !== null && writableTrips.length > 0 && (
          <HelperConsentList
            username={username}
