 ...
 *  Not exported past this file since B2018 removed `/me`'s pencil editor, its
 *  last outside caller — every remaining use (`PhotoBadge`, `EntryVisibility`,
 *  `VisibilityControl`) is inside this module. */
...
export function TripVisibility() {
  ...
  // D4 (spec §6, B1938) — the trip page's own control deep-links into "Who
  // may read this trip" with the trip already chosen, the same shape
  // `OwnerTools`' day edit tile already uses for `/studio/day/edit?slug=`.
  // It used to open `TripVisibilityFor` in place, a second implementation of
  // the same choice with no preview; the studio flow is the one
