...
export default function AgentHandover({
  ...
  onIssued?: () => void;
  /** The heading and sentence above the button — off where the page's own
   *  title and lede already say it (the studio's agent page, B2091). */
  intro?: boolean;
}) {
  const { t } = useI18n();
  ...
  async function mint() {
    ...
    setPrompt(
      handoverPrompt({
        // The address the owner is on right now — B2091. `site.url` handed a
        // local or self-hosted instance's owner the main instance's address,
...
export function HandoverPrompt({
  ...
  return (
    <div ref={block} tabIndex={-1} className="mt-3 outline-none">
      ...
      */}
      <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-surface-subtle p-3 text-xs leading-6 text-ink-strong">
        {prompt}
      </pre>
      <div className="mt-3">
