 ...
 * board. Every number here is read back from the same `GET .../studio/run`
...
export default function FoundStep({
  ...
  runId: string;
  onContinue: () => void;
  /** Whether the talk-or-type screen was part of this import (`importStep`). */
  speech: boolean;
}) {
  const step = importStep("found", speech);
  const { t, tn } = useI18n();
  ...
  if (error) {
    return <p className="mt-4 text-sm text-coral-600">{t("studio.photos.found.error")}</p>;
  }
  if (!data) {
    return <p className="mt-4 text-sm text-ink-secondary">{t("studio.photos.found.loading")}</p>;
  }
  ...
  return (
    <div className="mt-4">
      <StepIndicator
        ...
        label={t("studio.photos.step.found", { current: String(step.current), total: String(step.total) })}
      />
      <h2 className="font-display text-xl font-semibold leading-tight text-ink-strong">
        {tn("studio.photos.board.photoCount", total, { count: String(total) })},{" "}
        {tn("studio.photos.found.dayCount", days, { count: String(days) })}
      </h2>
      <p className="mt-1 text-sm text-ink-secondary">{t("studio.photos.found.subtitle")}</p>

      <ul className="mt-4 divide-y divide-line-faint rounded-xl border border-line-strong">
        <Row label={t("studio.photos.found.dates")} value={`${withDate} / ${total}`} />
        <Row label={t("studio.photos.found.places")} value={`${withPlace} / ${total}`} />
        <Row label={t("studio.photos.found.timeOfDay")} value={`${withTime} / ${total}`} />
        <Row label={t("studio.photos.found.weather")} value={t("studio.photos.found.weatherValue")} />
        <Row label={t("studio.photos.found.who")} value={t("studio.photos.found.whoValue")} />
      </ul>

      {withoutPlace > 0 && (
        <div className="mt-4 rounded-xl bg-surface-subtle p-4">
          <p className="text-sm font-semibold text-ink-strong">
            {tn("studio.photos.found.withoutPlaceTitle", withoutPlace, { count: String(withoutPlace) })}
          </p>
          <p className="mt-1 text-sm text-ink-body">{t("studio.photos.found.noPlaceBody")}</p>
        </div>
      )}
      ...
      >
        {tn("studio.photos.found.seeDays", days, { count: String(days) })}
      </button>
    </div>
  );
}
