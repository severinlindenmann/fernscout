...
import { thumbSrc, type PhotoBadge } from "@/components/extract/PhotoTile";
import { useI18n } from "@/components/LocaleProvider";
import { isNativeShell, pickNativePhotos } from "@/components/nativeShell";
 ...
 *  `app/api/helper/[user]/studio/upload/route.ts`. Not imported: that file
...
/** `id` is the staged file's own id, once the upload has answered with it.
 *  It is the only way to draw a HEIC in a browser that cannot decode one —
 *  B1874 — so it is kept rather than dropped with the rest of the reply. */
type Tile = { file: File; state: TileState; id?: string };
...
const REJECT_REASON_KEY: Record<string, TranslationKey> = {
  too_large: "studio.photos.upload.rejected.tooLarge",
  // B2205: a picture that declares more pixels than the server decodes is
  // over a size limit too — the same sentence is the truthful one.
  too_many_pixels: "studio.photos.upload.rejected.tooLarge",
  run_full: "studio.photos.upload.rejected.runFull",
  journal_over_capacity: "studio.photos.upload.rejected.overCapacity",
};
...
const STATE_KEY: Record<TileState, TranslationKey> = {
  ...
  sending: "studio.photos.upload.state.sending",
...
export default function UploadStep({
  ...
  /** Whether the talk-or-type screen was part of this import — decides
   *  this screen's place in the strip (`importStep`). */
  speech: boolean;
}) {
  const step = importStep("upload", speech);
  ...
  async function acquireWakeLock() {
    ...
    try {
      ...
      // themselves, which is what studio.photos.upload.awakeWhy tells them.
    }
  }
  ...
  async function send(indices: number[]) {
    ...
    try {
      for (const slice of chunk(indices, BATCH)) {
        ...
        try {
          const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/upload`, {
            method: "POST",
            body,
          });
          if (!res.ok) throw new Error(String(res.status));
          const data = (await res.json()) as {
            accepted?: { filename: string; id: string }[];
            rejected?: { filename: string; reason: string }[];
            stagedBytes?: number;
          };
          // Matched by filename, the same way `rejected` below is. A file the
          // run already holds is deduped out of `accepted` entirely, so it
          // keeps whatever preview it had; nothing here invents an id.
          const acceptedIds = new Map((data.accepted ?? []).map((a) => [a.filename, a.id]));
          ...
          setTiles((t) =>
            t.map((x, n) =>
              slice.includes(n)
                ? {
                    ...x,
                    state: rejectedNames.has(x.file.name) ? "failed" : "done",
                    id: acceptedIds.get(x.file.name) ?? x.id,
                  }
                : x,
            ),
          );
  ...
  return (
    <div>
      <StepIndicator
        ...
        label={t("studio.photos.step.upload", { current: String(step.current), total: String(step.total) })}
      />
      ...
      <ul className="mb-3 divide-y divide-line-faint rounded-xl border border-line-strong text-sm">
        <li className="flex items-center justify-between px-4 py-2.5">
          <span className="text-ink-strong">{t("studio.photos.upload.limits.formats")}</span>
          <span className="text-ink-secondary">{t("studio.photos.upload.limits.formatsValue")}</span>
        </li>
        <li className="flex items-center justify-between px-4 py-2.5">
          <span className="text-ink-strong">{t("studio.photos.upload.limits.upTo")}</span>
          <span className="text-ink-secondary">{t("studio.photos.upload.limits.upToValue", { count: String(MAX_FILES_PER_RUN) })}</span>
        </li>
        <li className="flex items-center justify-between px-4 py-2.5">
          <span className="text-ink-strong">{t("studio.photos.upload.limits.each")}</span>
          <span className="text-ink-secondary">
            {t("studio.photos.upload.limits.eachValue", {
        ...
        <li className="flex items-center justify-between px-4 py-2.5">
          <span className="text-ink-strong">{t("studio.photos.upload.limits.storage")}</span>
          <span className="text-ink-secondary">
            {t("studio.photos.upload.limits.storageValue", {
      ...
      {stagedBytes !== undefined && stagedBytes >= JOURNAL_STAGING_MAX_BYTES * JOURNAL_STAGING_WARN_FRACTION && (
        <div className="mb-3 rounded-xl border border-line-strong px-4 py-2.5">
          <p className="text-sm text-ink-secondary">
            {t("studio.photos.upload.storageBar", {
      ...
      />
      {/* Inside the iPhone shell the same label opens the phone's own picker
       *  instead of the `<input>`, because that is the one door through
       *  which the HEIC original arrives rather than a JPEG iOS made at pick
       *  time (B1750, B2113). The tiles, the batches and the route are the
       *  same from here on. On the web this handler does nothing and the
       *  label opens the `<input>` as a label does. */}
      <label
        htmlFor="extract-upload-input"
        onClick={(e) => {
          if (busy || !isNativeShell()) return;
          e.preventDefault();
          void pickNativePhotos().then((files) => {
            if (files.length) setTiles(files.map((file) => ({ file, state: "queued" as const })));
          });
        }}
      ...
      >
        ...
        {t("studio.photos.upload.choose")}
      </label>

      <p className="mt-2 text-sm text-ink-secondary">{t("studio.photos.upload.perDay")}</p>
      <p className="mt-1 text-sm text-ink-secondary">{t("studio.photos.upload.icloud")}</p>
      <p className="mt-1 text-sm text-ink-secondary">
        <strong>{t("studio.photos.upload.awake")}</strong> — {t("studio.photos.upload.awakeWhy")}
      </p>
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700">
              {tn("studio.photos.upload.chips.done", tiles.filter((x) => x.state === "done").length, {
            ...
            {eta !== undefined && (
              <span className="rounded-full border border-line-strong px-2.5 py-1 text-xs font-medium text-ink-secondary">
                {tn("studio.photos.upload.chips.etaMinutes", eta, { count: String(eta) })}
              </span>
            )}
          </div>
        </div>
          ...
          <PhotoStrip
            ...
            photos={tiles.map(
              (tile, n): PhotoStripItem => ({
                ...
                file: tile.file,
                src: thumbSrc(username, runId, tile.id),
                alt: tile.file.name,
      ...
      {failed.length > 0 && (
        <div role="status" className="mt-3 rounded-xl border border-coral-400 bg-coral-50 p-4">
          <p className="text-sm font-semibold text-coral-600">
            {tn("studio.photos.upload.failurePanel.title", failed.length, { count: String(failed.length) })}
          </p>
          <p className="mt-1 text-sm text-coral-600">{t("studio.photos.upload.failed")}</p>
          ...
          {rejectionReasons.length > 0 && (
            <ul className="mt-1 text-sm text-coral-600">
              {rejectionReasons.map((reason) => (
                <li key={reason}>{t(REJECT_REASON_KEY[reason] ?? "studio.photos.upload.rejected.other")}</li>
              ))}
            </ul>
          )}
          {!busy && (
            <button
              type="button"
              onClick={() => send(failed)}
              className="mt-3 inline-flex min-h-11 items-center rounded-full bg-coral-400 px-5 text-base font-semibold text-navy-900 transition-colors hover:bg-coral-300"
            >
              {t("studio.photos.upload.retry", { count: String(failed.length) })}
            </button>
          )}
        </div>
      )}
      ...
      {tiles.length > 0 && !busy && failed.length === 0 && (
        ...
        >
          {t("studio.photos.upload.send", { count: String(tiles.length) })}
        </button>
      )}

      {/* The design's own reassurance line for this screen — distinct from
       *  `studio.photos.upload.awakeWhy` above, which explains *why* to keep the
      ...
      {started && (
        <p className="mt-2 text-sm text-ink-secondary">{t("studio.photos.upload.reassure")}</p>
      )}
    </div>
  );
}
