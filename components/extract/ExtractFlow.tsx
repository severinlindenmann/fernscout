 ...
 *
 * Opens a run the moment the page loads (`POST .../studio/start`, Task 1.2)
 ...
 * starting a fresh run, the shell asks `GET .../studio/runs` (read-only)
 ...
 * actual extension is `GET .../studio/run`'s existing `extendOnTouch` call,
...
export default function ExtractFlow({
  ...
  // `undefined` until `GET .../studio/runs` answers; `null` once the person
   ...
   *
   * A failed check is answered as an error, the same `studio.photos.flow.startError`
  ...
  async function checkResume() {
    setError(false);
    try {
      const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/runs`);
  ...
  /** `POST .../studio/start` with the person's own answers to Step 02 —
  ...
  async function start(answers: {
    ...
    setRun(null);
    try {
      const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/start`, {
        method: "POST",
   ...
   *  condition under which `GET .../studio/run`'s own `extendOnTouch` will
   ...
   *  `GET .../studio/run` call that has always carried `extendOnTouch`,
  ...
  return (
    // The frame — `w-full` included (B1799) — is StudioPage's `<main>` now.
    <>

      {error && (
        <div className="mt-4">
          <p className="text-sm text-coral-600">{t("studio.photos.flow.startError")}</p>
          <button
            type="button"
            onClick={checkResume}
      ...
      {(resumable === undefined || (resumable === null && !run && !awaitingStart)) && !error && (
        <p className="mt-4 text-sm text-ink-secondary">{t("studio.photos.flow.starting")}</p>
      )}
      ...
      {resumeNotice && (
        <div className="mt-4">
           ...
           *  needs, in the same breath, which days are already safe in the
           *  journal (only those added at the last step, B2057); a screen
           *  earlier is not the same as here. */}
          <p data-testid="resume-notice-reassurance" className="text-sm text-ink-body">
            {t("studio.photos.resume.onlyAdded")}
          </p>
          <p data-testid="resume-notice-clock" className="mt-1 text-sm text-ink-body">
            {resumeNotice.kind === "notWarned" && t("studio.photos.resume.notWarned")}
            {resumeNotice.kind === "justExtended" &&
              t("studio.photos.resume.justExtended", {
            ...
            {resumeNotice.kind === "extended" &&
              t("studio.photos.resume.extended", { until: new Date(resumeNotice.until).toLocaleString() })}
          </p>
        </div>
      )}
      ...
      {done && !foundSeen && !atBoardEnd && run && (
        <>
          <p className="mt-4 text-sm text-ink-body">{t("studio.photos.flow.done", { count: String(uploaded) })}</p>
          <FoundStep username={username} runId={run.runId} onContinue={() => setFoundSeen(true)} speech={consentedSpeech} />
        </>
      )}
