 ...
 * counts as clear.
 *
 * **What is saved is not the trip's `people:`.** `PATCH .../studio/party`
...
export default function WhoCameScreen({
  ...
  async function save() {
    ...
    setSaveError(false);
    try {
      const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/party`, {
        method: "PATCH",
  ...
  if (error) {
    return (
      <div className="mt-4">
        <p className="text-sm text-coral-600">{t("studio.photos.whoCame.error")}</p>
        <button
          type="button"
          onClick={() => void load()}
  ...
  return (
    <div className="mt-4 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        {onBack ? (
          <button
            ...
            aria-label={t("studio.photos.whoCame.header")}
          >
            ← {t("studio.photos.whoCame.header")}
          </button>
        ) : (
          ...
          <span className="text-sm font-semibold text-ink-strong">← {t("studio.photos.whoCame.header")}</span>
        )}
      </div>
      <h2 className="font-display text-xl font-semibold text-ink-strong">{t("studio.photos.whoCame.title")}</h2>

      <div className="flex items-center justify-center gap-4 rounded-2xl border border-line-faint bg-surface-subtle p-4">
        <button
          type="button"
          aria-label={t("studio.photos.whoCame.fewer")}
        ...
        <button
          type="button"
          aria-label={t("studio.photos.whoCame.more")}
      ...
      <div className="rounded-2xl border border-line-faint bg-surface-raised p-4">
        <p className="text-sm font-semibold text-ink-strong">{t("studio.photos.whoCame.namesTitle")}</p>
        <div className="mt-2 flex flex-col gap-2">
          <label className="flex min-w-0 items-center justify-between gap-3">
            <span className="shrink-0 text-sm text-ink-secondary">{t("studio.photos.whoCame.you")}</span>
            <input
              value={names[0]}
              onChange={(e) => setNames([e.target.value, names[1]])}
              placeholder={t("studio.photos.whoCame.tapToName")}
              className="min-w-0 flex-1 rounded-full border border-line-strong px-3 py-1.5 text-right text-sm text-ink-strong"
            />
          </label>
          <label className="flex min-w-0 items-center justify-between gap-3">
            <span className="shrink-0 text-sm text-ink-secondary">{t("studio.photos.whoCame.second")}</span>
            <input
              value={names[1]}
        ...
        {showSuggestion && (
          ...
          >
            {t("studio.photos.whoCame.suggestion", {
      ...
      {saveError && <p className="text-sm text-coral-600">{t("studio.photos.whoCame.saveError")}</p>}
      ...
      >
        {busy ? t("studio.photos.whoCame.saving") : t("studio.photos.whoCame.save")}
      </button>
      <p className="text-sm text-ink-secondary">{t("studio.photos.whoCame.reassure")}</p>
    </div>
  );
}
