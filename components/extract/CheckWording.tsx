  ...
  return (
    <div>
      ...
      <p className="mt-1 text-center text-sm text-ink-secondary">
        {t("studio.photos.checkWording.paused", { clock: clockFor(recordedSeconds) })}
      </p>
      ...
      <div className="mt-3 rounded-xl border border-line-strong bg-surface-raised p-3">
        ...
        {editing ? (
          <textarea
            ...
            aria-label={t("studio.photos.checkWording.correctLabel")}
      ...
      {split && (
        <div className="mt-3 rounded-lg border border-line-faint bg-surface-subtle p-3">
          <p className="text-sm font-semibold text-ink-strong">{t("studio.photos.checkWording.tipTitle")}</p>
          <p className="mt-1 text-sm text-ink-secondary">{t("studio.photos.checkWording.tipBody")}</p>
        </div>
      )}
      ...
      >
        {t("studio.photos.checkWording.keep")}
      </button>
      <button
        type="button"
        onClick={onRedo}
      ...
      >
        {t("studio.photos.checkWording.sayAgain")}
      </button>
    </div>
  );
}
