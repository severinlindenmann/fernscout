...
export default function StepIndicator({
  ...
  return (
    ...
    <div data-step-indicator className="mb-4 flex items-center gap-2 rounded-full bg-surface-subtle px-3 py-1.5">
...
/**
 * The photo import's wizard screens, in order — B2081. Each screen's position
 * and the total come from this one list, so a screen that is skipped (the
 * talk-or-type choice, asked only once speech is consented to) is not
 * counted, and the strip never jumps from 1 to 3.
 */
export function importStep(
  screen: "trip" | "mode" | "upload" | "found",
  speech: boolean,
): { current: number; total: number } {
  const screens = ["trip", ...(speech ? ["mode"] : []), "upload", "found", "board"];
  return { current: screens.indexOf(screen) + 1, total: screens.length };
}
