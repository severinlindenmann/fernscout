 ...
 * `POST .../studio/commit` recorded on `DayRow.entrySlug`. This screen
 ...
 * that route is a picker over every day, which is exactly "fix anything in
 * what this run just wrote" for a screen naming several days at once with
 * none of them singled out. Wording itself is still not edited *here* —
 * this screen still builds no text box of its own — only where it opens.
 * The inline "Open your agent" beside the publish hint stays on `/agent`:
 * publishing is the one act that still has to be said in words, not
 * clicked, and the studio has no publish flow to send it to.
 *
 * **"Draft" is not a guess either.** Every day this screen can possibly
 * name came out of `commitReadyDays`/`studio/commit`, which never
...
export default function PreviewScreen({
  ...
  if (error) {
    return (
      <div className="mt-4">
        <p className="text-sm text-coral-600">{t("studio.photos.preview.error")}</p>
        <button
          type="button"
          onClick={() => void load()}
  ...
  if (!data) {
    return <p className="mt-4 text-sm text-ink-secondary">{t("studio.photos.preview.loading")}</p>;
  }
  ...
  if (sortedDays.length > 0) {
    summaryParts.push(
      t("studio.photos.preview.dateRange", {
    ...
    summaryParts.push(tn("studio.photos.preview.dayCount", sortedDays.length, { count: String(sortedDays.length) }));
  }
  if (totalPhotos > 0) {
    summaryParts.push(tn("studio.photos.preview.photoCount", totalPhotos, { count: String(totalPhotos) }));
  }
  if (partyNames.length > 0) {
    summaryParts.push(t("studio.photos.preview.peopleLine", {
    ...
    <div className="mt-4 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        {onBack ? (
          <button
            ...
            aria-label={t("studio.photos.preview.header")}
          >
            ← {t("studio.photos.preview.header")}
          </button>
        ) : (
          <span />
        )}
      ...
      <div>
        {tripId && days.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="shrink-0 rounded-full border border-line-strong px-2 py-0.5 text-xs font-semibold text-ink-secondary">
              {t("studio.photos.preview.draft")}
            </span>
          </div>
        )}
        {trip === null && <p className="mt-1 text-xs text-ink-secondary">{t("studio.photos.preview.tripLoadError")}</p>}
        ...
        {tripId && days.length > 0 ? (
          ...
          <p className="mt-2 text-sm text-ink-body">{t("studio.photos.preview.nothingAdded")}</p>
        )}
        ...
        {skipped.length > 0 && (
          <p className="mt-3 text-sm text-ink-secondary">
            {tn("studio.photos.preview.skipped", skipped.length, { count: String(skipped.length) })}
          </p>
        )}
        {undatedCount > 0 && (
          <p className="mt-1 text-sm text-ink-secondary">
            {tn("studio.photos.preview.undatedSkipped", undatedCount, { count: String(undatedCount) })}
          </p>
        )}

        {tripId && days.length > 0 && (
          <p className="mt-3 text-sm text-ink-body">
            {/* B2169 — the studio's publish page (B2140) asks the owner's
      ...
      {tripId && (
        <div>
          <h3 className="text-base font-semibold text-ink-strong">{t("studio.photos.preview.peopleTitle")}</h3>
          <p className="mt-1 text-sm text-ink-secondary">{t("studio.photos.preview.peopleHint")}</p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <input
              ...
              placeholder={t("studio.photos.preview.namePlaceholder")}
            ...
            <input
              ...
              type="email"
              placeholder={t("studio.photos.preview.emailPlaceholder")}
              className="min-h-11 rounded-full border border-line-strong px-4 text-sm"
            />
            <button
              type="submit"
            ...
            >
              {peopleBusy ? t("studio.photos.preview.adding") : t("studio.photos.preview.addPerson")}
            </button>
          </form>
          {peopleError && <p className="mt-2 text-sm text-coral-600">{t("studio.photos.preview.peopleError")}</p>}
          {people && (
            <p className="mt-2 text-sm text-ink-secondary">
              {tn("studio.photos.preview.peopleAdded", people.length, { count: String(people.length) })}
            </p>
          )}
        </div>
      )}
 ...
 *  photographs and `POST .../studio/commit` recorded the real slug it
