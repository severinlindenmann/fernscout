 ...
 * function `studio/enrich`'s route imports to actually charge — never a
 ...
 * `studio/enrich` to write a caption onto every photograph first. A day
...
export default function CreditsScreen({
  ...
  async function takeSample(photoId: string) {
    ...
    setSampling(true);
    try {
      const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/sample`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function spendAndBuild() {
    ...
    setError(false);
    try {
      const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/enrich`, {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    return (
      <div className="mt-4">
        <p className="text-sm text-ink-secondary">
          {error ? t("studio.photos.credits.error") : t("studio.photos.board.loading")}
        </p>
        {error && (
          <button
            type="button"
  ...
  return (
    <div className="mt-4 flex flex-col gap-3">
      {canPick && eligiblePhotos.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-ink-strong">{t("studio.photos.credits.pickTitle")}</p>
          <p className="mt-1 text-sm text-ink-secondary">{t("studio.photos.credits.pickSub")}</p>
          <div className="mt-2">
            <PhotoStrip
              size="grid"
              columns={4}
      ...
      <PhotoViewer
        ...
        extra={
          openIndex !== null && (
            ...
            >
              {sampling ? t("studio.photos.credits.sampling") : t("studio.photos.credits.useThisOne")}
            </button>
          )
        }
      />
      ...
      {sample && (
        <div className="rounded-xl border border-line-faint p-3">
          <p className="text-xs font-semibold text-ink-secondary">{t("studio.photos.credits.sampleLabel")}</p>
      ...
      {total > 0 && (
        <div className="rounded-xl border border-line-strong p-4">
          <p className="text-sm text-ink-body">
            {tn("studio.photos.credits.price", total, { total: formatCredits(total) })}{" "}
            {tn("studio.photos.credits.forPhotos", photoCount, { count: String(photoCount) })}
          </p>
          <p className="mt-1 text-sm text-ink-secondary">
            {t("studio.photos.credits.balance", { balance: formatCredits(credits) })}
          </p>
          ...
          <p data-testid="tied-to-run" className="mt-3 text-sm font-semibold text-ink-strong">
            {t("studio.photos.credits.tiedToRun")}
          </p>
          <button
            type="button"
          ...
          >
            {busy === "spend" ? t("studio.photos.credits.spending") : tn("studio.photos.credits.spend", total, { total: formatCredits(total) })}
          </button>
        </div>
      )}
      ...
      >
        {busy === "build" ? t("studio.photos.flow.building") : t("studio.photos.credits.buildFree")}
      </button>

      {error && (
        <p role="status" className="text-sm text-coral-600">
          {t("studio.photos.credits.error")}
        </p>
      )}
    </div>
  );
