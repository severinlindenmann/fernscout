...
import { thumbSrc } from "@/components/extract/PhotoTile";
...
type DayGroupWithPlace = DayGroup & {
   ...
   *  `app/api/helper/[user]/studio/run/route.ts`, the same lookup that
...
const STATUS_KEY: Record<DayStatus, TranslationKey> = {
  told: "studio.photos.board.status.told",
  noPlace: "studio.photos.board.status.noPlace",
  notYet: "studio.photos.board.status.notYet",
};
...
export default function DayBoard({
   ...
   *  Task 4.3. `GET .../studio/run` carries the same `extendOnTouch` call
  ...
  async function patchPhoto(photoId: string, patch: Record<string, string>) {
    const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/run`, {
      method: "PATCH",
  ...
  async function answerQuestion(group: DayGroup, question: Question, answer: string) {
    const res = await fetch(`/api/helper/${encodeURIComponent(username)}/studio/day`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        run: runId,
  ...
  if (error) {
    return (
      <div className="mt-4">
        <p className="text-sm text-coral-600">{t("studio.photos.board.error")}</p>
        <button
          type="button"
          onClick={() => void load()}
  ...
  // Every *real* day can be told (`nextDate` undefined) while the undated
  // card still has open questions — B1803 kept it out of `nextDayToTell`
  // because it has no weekday to name, not because it is done. Without this,
  // the fallback below claimed "Every day is told" over a card with
  // unanswered questions still open.
  const undatedOpen = groups.some(
    (group) => group.undated && statusFor(questions[keyFor(group)] ?? []) !== "told",
  );
  ...
  return (
    <div className="mt-4">
      <StepIndicator
        total={daysTotal}
        current={daysTold}
        label={tn("studio.photos.step.daysTold", daysTotal, { current: String(daysTold), total: String(daysTotal) })}
      />
      <ul className="divide-y divide-line-faint rounded-xl border border-line-strong">
        {groups.map((group, groupIndex) => {
          ...
          // not actually seen.
          const summary = place
            ? tn("studio.photos.board.summary.withPlace", photoCount, { place, count: String(photoCount) })
            : tn("studio.photos.board.summary.noPlace", photoCount, { count: String(photoCount) });
          return (
            <li key={key} className={isSelected ? "rounded-lg ring-2 ring-yellow-300" : undefined}>
              ...
              >
                <span className="text-sm font-semibold text-ink-strong">
                  {group.undated ? t("studio.photos.board.undated") : group.date}
                </span>
              ...
              {selected === key && (
                <div className="border-t border-line-faint px-4 py-3">
                  ...
                  <div className="mt-3 flex flex-col gap-2">
                    {open.length === 0 ? (
                      <p className="text-sm text-ink-secondary">{t("studio.photos.board.doneDay")}</p>
                    ) : (
                      // B2081 — one question at a time: the current one
                      // open, the rest named below it, never three cards
                      // with three progress bars stacked.
                      open.slice(0, 1).map((question, openIndex) => (
                        <AskCard
                          key={question.id}
                          question={question}
                    ...
                    {open.length > 1 && (
                      <div data-testid="later-questions">
                        <p className="text-xs font-semibold text-ink-secondary">{t("studio.photos.board.stillToAsk")}</p>
      ...
      {/* B2110 — the suggestion hides while that day's own card is open:
       *  the card below it is already the thing it points at. */}
      {nextDate ? (
        selected !== nextDate && (
        <>
          <button
          ...
          >
            {t("studio.photos.board.tellMeAbout", { weekday: weekdayLabel(nextDate, locale) })}
          </button>
          <p className="mt-2 text-xs text-ink-secondary">{t("studio.photos.board.anyOrder")}</p>
        </>
        )
      ) : undatedOpen ? (
        selected !== "undated" && (
        <>
          <button
            ...
            className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-full bg-yellow-400 px-5 text-base font-semibold text-yellow-950 transition-colors hover:bg-yellow-300"
          >
            {t("studio.photos.board.tellMeUndated")}
          </button>
          <p className="mt-2 text-xs text-ink-secondary">{t("studio.photos.board.anyOrder")}</p>
        </>
        )
        ...
        <>
          <p className="mt-4 text-sm text-ink-secondary">{t("studio.photos.board.allTold")}</p>
          {/* B2081 — every day told: the way on is a real primary in the
           *  bar, not "Done for now". */}
          <StepPrimary label={t("studio.photos.board.seeWhatWeMade")} onClick={onLeave} />
        </>
      )}
      ...
      <button
        type="button"
        onClick={onLeave}
        className="mt-4 min-h-11 text-sm font-semibold text-ink-strong underline underline-offset-2"
      >
        {t("studio.photos.board.leave")}
      </button>
      ...
      <p className="mt-2 text-xs text-ink-secondary">{t("studio.photos.flow.left")}</p>
