 ...
 * ledger read (`extract/run`'s `spentCredits`), never a number kept beside
 ...
 *
 * **"Publish this trip" is a link to `/agent`, never a call this component
  ...
  return (
    <div className="mt-4 flex flex-col gap-4">
      ...
      <div className="text-center">
        <h2 className="font-display text-xl font-semibold text-ink-strong">{t("extract.ready.savedAsDraft")}</h2>
        <p className="mt-1 text-sm text-ink-secondary">{t("extract.ready.nobodyCanSee")}</p>
      </div>

      <dl className="flex flex-col gap-2 rounded-2xl border border-line-faint bg-surface-raised p-4">
        <div className="flex items-center justify-between">
          <dt className="text-sm text-ink-secondary">{t("extract.ready.days")}</dt>
          <dd className="text-sm font-semibold text-ink-strong">{days.length}</dd>
        </div>
        <div className="flex items-center justify-between">
          <dt className="text-sm text-ink-secondary">{t("extract.ready.photographs")}</dt>
          <dd className="text-sm font-semibold text-ink-strong">{photoCount}</dd>
        </div>
        {heldBack > 0 && (
          <div className="flex items-center justify-between">
            <dt className="text-sm text-ink-secondary">{t("extract.ready.heldBack")}</dt>
            <dd className="text-sm font-semibold text-ink-strong">
              {tn("extract.ready.heldBackValue", heldBack, { count: String(heldBack) })}
            </dd>
          </div>
        )}
        {typeof spentCredits === "number" && spentCredits > 0 && (
          <div className="flex items-center justify-between">
            <dt className="text-sm text-ink-secondary">{t("extract.ready.credits")}</dt>
      ...
      <div className="rounded-2xl border border-line-faint bg-surface-subtle p-4">
        <p className="text-sm font-semibold text-ink-strong">{t("extract.ready.publishSeparate.title")}</p>
        <p className="mt-1 text-sm text-ink-secondary">{t("extract.ready.publishSeparate.body")}</p>
      </div>
