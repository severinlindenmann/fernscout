...
import {
  ...
  localizedTripTitle,
  localizedEntryTitle,
  languageName as nameOfLanguage,
  type TranslationKey,
} from "@/lib/i18n";
...
type Ctx = {
  ...
  /** `{ year: true }` adds the year when it is not the current one (B2139). */
  formatLongDate: (date: string, options?: { year?: boolean }) => string;
  formatShortDate: (date: string) => string;
  formatStay: (nights: number) => string;
  /** A language named in the reader's own language ("Englisch" on a German
   *  page), not its endonym — B2077, B2139. */
...
export default function LocaleProvider({
  ...
  const value = useMemo<Ctx>(() => {
    ...
    const formatLongDate = (date: string, options?: { year?: boolean }) => {
    ...
    const languageName = (code: string) => nameOfLanguage(code, locale);
