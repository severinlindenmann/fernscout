...
export default function FullPhoto({ item }: { item: GalleryItem }) {
  ...
  // caller, so a caption here would only say the same words twice. The alt
  // text is a different sentence — what is in the frame, for somebody who
  ...
  if (!resizable(item.src)) {
    return (
      <span className="fs-photo fs-photo-full relative block" {...frame}>
        ...
        <img {...img} src={item.src} alt={alt} className={className} draggable={false} />
      </span>
    );
  }
