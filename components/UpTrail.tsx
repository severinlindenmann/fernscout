 ...
 *
 * `crumbs` defaults to `useUpCrumbs()`, but `PageHeader` may pass its own —
 * B1992's `backTo` override — so the two draw the same single crumb rather
 * than this component silently recomputing the URL-derived trail underneath
 * an override the phone row above it already honoured.
 */
export default function UpTrail({
  className,
  crumbs: crumbsProp,
}: {
  className?: string;
  ...
  const { t } = useI18n();
  const hookCrumbs = useUpCrumbs();
  const crumbs = crumbsProp ?? hookCrumbs;
