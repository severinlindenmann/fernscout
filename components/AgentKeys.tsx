...
/** "21:03" today, a short date and time otherwise — B2091: two keys made the
 *  same day are told apart by the minute they were made. */
...
type Key = {
  ...
  /** "gps" (B2204) is a positions-only token — a phone uploading where
   * somebody went, nothing else. Absent on a row minted before this field
   * existed, read the same as `GET /api/auth/{user}/keys` reads it. */
  scope?: "owner" | "trip" | "gps";
};
...
export default function AgentKeys({
  ...
  // Which key's Revoke is being asked about — B2091: revoking is a question
  // first, never a single tap.
    ...
    <div className="mt-5">
      ...
      <ul className="mt-3 space-y-2">
        {keys.map((key) => {
          ...
          const made = when(key.createdAt);
          return (
            <li
              key={key.id}
              className="rounded-xl border border-line-quiet bg-surface-raised px-4 py-3"
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="text-base">
                  ...
                  <span className="block text-sm text-ink-secondary">
                    {[
                      t("me.keysMade", { time: made }),
                      t("me.keysUntil", { date: when(key.expiresAt) }),
                      key.lastSeenAt
                        ? t("me.keysUsed", { date: when(key.lastSeenAt) })
                        : t("me.keysUnused"),
                    ].join(" · ")}
                  </span>
                </span>
              ...
              {asking === key.id && (
                <div className="mt-3">
                  <ConfirmPanel
                    label={t("me.keysRevoke")}
                    question={t("me.keysRevokeQuestion", { time: made })}
                    confirmLabel={t("me.keysRevokeConfirm")}
                    busyLabel={t("me.keysRevokeBusy")}
                    busy={busy === key.id}
                    tone="destructive"
