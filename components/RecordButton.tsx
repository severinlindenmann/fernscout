...
export default function RecordButton({
  ...
  useEffect(() => {
    // B2228 — React StrictMode (dev only) mounts, cleans up and mounts again
    // to prove an effect survives it. The cleanup below sets `unmounted` so a
    // `start()` in flight when the real unmount happens never opens the
    // microphone; without resetting it here, the replay's first cleanup
    // leaves it `true` for the component's whole remaining life and every
    // `start()` in dev returns at its unmounted guard before doing anything,
    // silently.
  ...
  const hold = {
    onPointerDown: () => {
      // B994/B995 — marks the click this press will produce as already
      // handled (by `press()` here and `release` on the matching
      // pointerup/leave/cancel), so `toggle` does not read it as a second,
      // independent press. See `toggle`.
    ...
    // target it leaves the moment somebody starts speaking — B1004. Either
    // way no click follows a leave (a click needs pointerup back over this
    // element), so the flag is cleared here too — left set, it would swallow
    // the next, unrelated click instead of going stale on its own.
    onPointerLeave: () => {
      fromPointer.current = false;
      if (holdToTalk) release();
    },
    onPointerCancel: () => {
      fromPointer.current = false;
      release();
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <p role="status" className="text-center text-sm text-cream-50">
          {busy
            ? t("agent.speechWorking")
            : recording
              ? t("studio.photos.ask.listening")
              : (label ?? t("studio.photos.ask.tapToSpeak"))}
        </p>
