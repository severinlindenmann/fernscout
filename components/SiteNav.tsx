...
import { TRIP_DESTINATIONS } from "@/lib/navDestinations";
...
const ICONS: Record<string, typeof BookOpen> = {
  "/": BookOpen,
  // was `/extract` (B1825 moved the route and left the drawing behind). An
  // upload arrow says "give the machine a file"; the studio is where a
  // person writes a day, plans a trip and decides who may read it, and only
  // three of its twelve flows involve a file at all.
  "/gallery": Images,
  "/map": Map,
...
export function useNavEntries(): NavEntry[] {
  ...
  // **No account row — B2016.** The credits-and-storage page (B821) had its
  // own owner-only tab here; the owner asked for one place to administer the
  // journal, so it moved under the studio hub's own Journal group
  // (`components/studio/StudioHub.tsx`) instead, the same move B1963 already
  // made for the studio hub itself. `ACCOUNT_DESTINATION` stays in
  // `lib/navDestinations.ts` for `lib/search.ts` to read, same as
  // `STUDIO_DESTINATION` below.

  // **No studio row — B1963.** The owner asked for the studio to keep the
  // shape the agent's control had: its own dark button in the header, and
  // its own section above this list in the phone panel (`PageHeader`). A row
  // here as well was two links to one page in one header.
  //
  // What moved with it is the reasoning that used to live on
  // `STUDIO_DESTINATION`: the hub is **not** gated on a capability, because
  // each flow it lists carries its own and an instance with `extract` off
  // still has a working studio. That argument now has to hold on the header
  // control instead — and it did not, until this ticket: the button was
  // gated on `helperEnabled`, so dropping this row would have left an owner
  // with the helper off no way in at all. `PageHeader` gates on `isOwner`
  // alone now.
