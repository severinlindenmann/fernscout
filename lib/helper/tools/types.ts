...
type Proposed = {
  ...
  refuse?: string;
  /**
   * The refusal sentence's own interpolation, when it needs one — B1746,
   * `create_trip`'s own "a trip called that already exists, use it" needs
   * to name the trip. Every `refuse` before this one was a plain sentence
   * with nothing to fill in; `proposalFor` passes this straight to `say`
   * as its second argument, `undefined` doing exactly what leaving it out
   * always did.
   */
  refuseVars?: Record<string, string>;
  /**
