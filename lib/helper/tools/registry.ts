...
import { PRINTED_TOOLS } from "@paid/printOrder/lib/helper/tools/areas/printed";
...
/**
 * The areas a WhatsApp turn may ever be offered — B1891.
 *
 * `trips` because it is the hub every other tool resolves a trip id off,
 * `days` and `files` because together they carry the whole funnel a phone
 * number is trusted for: resolving which day, staging its photographs,
 ...
 * per-tool allowance below, which holds even for a tool inside one of these
 * three areas that the WhatsApp funnel does not use.
 */
export const WHATSAPP_AREAS: readonly AreaKey[] = ["trips", "days", "files"];
