...
import { dictionaryFor, translateIn } from "../locales";
...
export function sayIn(locale: string): Say {
  // B2093 — a `{count}` of "1" reads its `.one` sibling when the dictionary
  // has one, the same rule `plural()` applies in the browser: "1 thing", not
  // "1 thing(s)".
  return (key, vars) =>
    translateIn(
      locale,
      (vars?.count === "1" && dictionaryFor(locale)[`${key}.one`] ? `${key}.one` : key) as Parameters<typeof translateIn>[1],
      vars &&
        Object.fromEntries(
 ...
 * `paid/printOrder/lib/helper/tools/areas/printed.ts` writes a real, pending order — so a
