...
import { AREAS, TOOLS, WHATSAPP_AREAS, runTool, toolList, toolSchemas, type AreaKey } from "./tools";
...
async function book(
  ...
  await recordUsage({
    ...
    // (lib/instanceCosts.ts) instead of face value.
...
/**
 * Describing one photograph, once — B1866.
 *
 * What came before was a batch of captions in one language, kept nowhere and
 * re-bought every time somebody asked; this is one photograph, three texts in
 * every language the journal offers, and a form the caller stores beside the
 * picture so the next person who asks pays nothing. The model call itself is
 * the same machinery: one request, no streaming, no tools, structured output
 * against a JSON schema.
 *
 * The schema is derived from the Zod schema in `lib/photos/described.ts`, so
 * what the API is asked for and what a stored block is checked against are
 * one statement. `$schema` is stripped because a structured output takes a
 ...
 */
/** The vocabulary as the model is offered it, with a gloss on the two words a
 *  sweep of 1,269 backfilled blocks found being stretched — B1960. The gloss
 *  sits inside the list rather than in a sentence after it, because that is
 *  where the choice is actually made: prose two sentences later did not hold.
 *  `document` is the tag B1956 reads to mean "paper with words", so it has to
 *  mean exactly that and not "a thing with printing on it". */
const TAG_GLOSS: Partial<Record<(typeof DESCRIBED_TAG_VOCABULARY)[number], string>> = {
  document: "paper somebody is meant to read: a ticket, a menu, a form, a certificate, a sign. Never packaging, never a screen",
  packaging: "a box, a blister pack, a wrapper, a label on a product — printed, but nobody reads it for its own sake",
  screenshot: "an image of a screen. Never a photograph of something printed",
};

function taggingVocabulary(): string {
  return DESCRIBED_TAG_VOCABULARY.map((tag) => (TAG_GLOSS[tag] ? `${tag} (${TAG_GLOSS[tag]})` : tag)).join(", ");
}

function describeImageSystemPrompt(locales: readonly string[]): string {
  return `You are looking at one photograph from somebody's own travel journal, and writing three kinds of text about it for the owner to keep, edit or discard — nothing you say is written anywhere by itself.

The one rule, and it outranks everything else you might think makes a description better:

DESCRIBE ONLY WHAT IS VISIBLE IN THE FRAME. Do not name a place, a country, a landmark or a business — even one you recognise, even if signage in the photograph names it — because none of this is the place to turn a guess into a fact. A region, a nationality and a climate are places too: a lavender field is not evidence of Provence, and a mountain is not evidence of the Alps. Name what is in the frame — the plants, the stone, the architecture, the weather — never the part of the world it suggests. This holds when the region is only an adjective: "Mediterranean vegetation" and "an Alpine village" are the same guess as naming the sea or the range. A business, a brand and a marque are not the photograph either: name what the thing is — a red sign, an airline's cabin label, a soft-drink cooler — never whose it is. This holds even when the name is on a museum's own building, a shopfront that is plainly the subject of the picture, or a logo you can read clearly: the frame does not tell you whether the name was the point, so refuse it there rather than guess. Never identify a person: no name, no relationship, no guess at who somebody is. You may describe what is visibly happening (someone is walking, someone is cooking) but never who. Never guess a mood, an occasion, a reason, or anything about what the day meant.

NEVER READ A DOCUMENT OUT LOUD. Name the kind of thing it is — a boarding pass, a menu, a Wi-Fi card, a form, a ticket, a bank card, a name badge — and stop there. Never copy out a code, a password, a reference, a booking number, a document number, a card number, a postal address, an email address, a telephone number, a person's name or a date of birth, even when it is the largest and clearest thing in the frame, even when the whole point of the photograph was to remember it, and even when transcribing it would obviously be useful. This holds in the long description exactly as it holds in the caption: a long description of a document says what kind of document it is and what it is for, never what it says. And a document is described, never skipped: name its kind and what it is for in the caption and the alt text — "a certificate from a cooking class", "a train timetable on a phone" — and return null for its long description, because everything further there is to say about a document is what it says. This rule takes the words off the page, not the photograph out of the book; saying nothing at all is the one answer that is neither safe nor useful.

The three texts, which answer three different questions about the same picture:

caption — what a person would print under the picture. A handful of words naming the subject, the way somebody labels their own photograph, not an inventory of every shape and its position in the frame. An empty string is the correct answer when there is nothing safe to say.

altText — what the picture shows, in one sentence, for somebody who cannot see it. At most 250 characters. Plainer and more complete than the caption: the subject, what it is doing, and the setting only as far as the frame actually shows it. An empty string when there is nothing safe to say.

longDescription — only when there is genuinely something more to say: a group of people to walk through, a landscape with several things happening in it, a scene a single sentence cannot carry. Never for a document; see above. Two to four sentences then, describing the thing — never transcribing it. For an ordinary photograph, return null — null is the normal answer here, not a failure.

Write every one of these texts in each of these locales, this journal's own languages: ${locales.join(", ")}. There are no notes to take a language from here, so this is told to you rather than inferred.

tags — choose zero or more from this exact list, and nothing else: ${taggingVocabulary()}. Several tags may apply at once: a plate of food indoors at night is food, interior and night. Never invent a tag, and never stretch one to fit; an empty list is fine.

confidence — high, medium or low, for how sure you are of what you have written. Prefer an empty string or null over a guess, and say low when you did.

subject — where the thing the picture is about sits in the frame, as a box: x and y are its top-left corner and width and height its size, all as fractions of the frame between 0 and 1. For people, the box holds every face that matters, head to shoulders, not the whole body. For a landscape, a texture or a scene with no one thing to point at, return null. This is read by a layout program that must not cut the subject off, so a box that is slightly too large is better than one that is too small.

people — how many people are visible, as a whole number. 0 when there is nobody. Count, never identify.

printworthiness — a whole number from 1 to 5 for how much this looks like a photograph somebody would print in a book of their trip: 5 for a clear, well-lit picture with a subject, 1 for a blurred, dark or accidental frame or a picture of a document or a screen. It is a judgement of the frame, not of the day.`;
}
...
export async function describeImage(
...
): Promise<DescribedForm> {
  const form = describedFormSchema(locales);
  const schema = z.toJSONSchema(form) as Record<string, unknown>;
  // A structured output takes a schema, not a document about one.
    ...
    messages: [
      {
        ...
        content: [
          ...
          { type: "text" as const, text: "One photograph. Describe it." },
        ],
      },
    ],
  ...
  const text = response.content
    ...
    .join("")
    .trim();
  const parsed = form.parse(JSON.parse(text));

  // Checked against what the model was given rather than taken on its own
  // say-so — a tag outside the vocabulary is dropped, and alt text past the
  // limit is cut, because neither is a reason to throw away the whole answer.
  const allowed = new Set<string>(DESCRIBED_TAG_VOCABULARY);
  parsed.tags = settleDocumentTag(parsed.tags.filter((tag) => allowed.has(tag)));
  for (const code of Object.keys(parsed.altText)) {
    parsed.altText[code] = parsed.altText[code].slice(0, ALT_TEXT_LIMIT);
  }
  // Structured outputs cannot enforce a minimum or a bounded rectangle, so
  // the three signals are clamped into range here the same way the tag
  // vocabulary and the alt-text limit are checked above rather than trusted.
  const clamped = clampSignals(parsed);
  // An answer with no caption is refused exactly like one that does not fit
  // the schema — B1923. Throwing here is what keeps it out of a sidecar, and
  // the caller already refunds on a throw.
  if (captionIsEmpty(clamped)) throw new Error("the model described this photograph as nothing");
  return clamped;
}
 ...
 * the same one `describeImageSystemPrompt` carries for captions, restated
...
/**
 * `draft`: rough notes in, a title and prose out — the original mode, still
 * what `set_day_words` chaining and WhatsApp (`paid/whatsapp/lib/whatsapp/dispatch.ts`)
...
CHANGE THE SURFACE, NEVER THE MEANING. Fix a typo, fix casing, add the punctuation a sentence needs, join two fragments into one sentence, reorder for flow. Never add a single thing their text did not already say: no person, no pronoun that introduces one ("we", "I", "she") unless that exact person is already named or implied by an existing first-person word in their own text, no new action or verb ("arrived", "tasted", "visited") standing in for one they did not use, no place, no feeling, no time of day, no quantity. A fragment with no subject and no verb — "port at grahams" — stays a fragment: tidy its spelling and punctuation and stop there, never complete it into a sentence with a subject or an action of your own. When you are not sure whether a word you want to use is already implied by their text or is your own addition, leave their original wording exactly as it was rather than guess.
 ...
 * Measured with `paid/test/helper-tool-areas.test.ts`'s own numbers, not assumed.
 *
...
/** A subset of `AREAS`, in whatever order — what a channel's own candidate
 *  list (`channelAreas`) is, once it is no longer the full, fixed-length
 *  tuple `AREAS` itself is typed as. */
type AreaList = readonly (typeof AREAS)[number][];
...
/**
 * The areas a turn on `channel` may ever be offered — B1891. `web` sees the
 * whole registry, as before; `whatsapp` sees only `WHATSAPP_AREAS`
 * (`trips`, `days`, `files`), so a WhatsApp turn cannot pick, nor
 * `switch_area` into, `printed`, `journal`, `readers` or `money` in the
 * first place. This is a prompt-size narrowing — the actual boundary a call
 * cannot talk its way past is `runTool`'s own per-tool allowance
 * (`lib/helper/tools/run.ts`), which holds even for a tool inside one of
 * these areas that the WhatsApp funnel does not use.
 */
function channelAreas(channel: "web" | "whatsapp"): AreaList {
  if (channel === "web") return AREAS;
  const allowed = new Set<AreaKey>(WHATSAPP_AREAS);
  return AREAS.filter((area) => allowed.has(area.key));
}
 ...
 *  Built per turn from `channelAreas(channel)` so its enum and description
 *  never name a door this channel cannot open — B1891. */
function switchAreaTool(areas: AreaList) {
  return {
    name: "switch_area",
    description:
    ...
    input_schema: {
      ...
      properties: {
        area: { type: "string" as const, enum: areas.map((area) => area.key) },
      },
      required: ["area"],
      additionalProperties: false,
    },
...
function pickAreaSchema(areas: AreaList) {
  return {
    type: "object",
    properties: { area: { type: "string", enum: areas.map((area) => area.key) } },
    required: ["area"],
    additionalProperties: false,
  } as const;
}

function pickAreaSystemPrompt(areas: AreaList): string {
 ...
 *
 * `areas` is this turn's channel-scoped candidate list (`channelAreas`) —
 * B1891: a WhatsApp turn is never asked to choose, and can never be handed
 * back, an area outside its own allowance.
 */
async function pickArea(
  ...
  const text = response.content
    ...
    .join("")
    .trim();
  const allowed = new Set(areas.map((area) => area.key));
  try {
    const parsed = JSON.parse(text) as { area?: unknown };
    const area = AREA_KEYS.find((key) => key === parsed.area && allowed.has(key));
...
/**
 * A day, exactly as every date on this journal is written \u2014 `YYYY-MM-DD`,
 * the one shape `lib/helper/tools/areas/days.ts` ever hands a tool's caller
 * (a `read_day`/`start_day` result, a day slug, a trip's own `start`/`end`)
 * and the one shape the locale strings interpolate raw (`agent.tool.startDay`
 * says "for {date}" with nothing further translated) \u2014 B1307.
 */
const DATE_TOKEN = /\b(\d{4})-(\d{2})-(\d{2})\b/g;

/**
 * Every `YYYY-MM-DD` date literally present in a value \u2014 B1307, the date
 * twin of `numbersIn` above. Recurses into a tool's whole JSON result rather
 * than naming a field, for the same reason `numbersIn` does: a day's own
 * date rides in different fields depending on the tool (`date`, an
 * `arguments.date` a proposal resolved, a day slug's own prefix), and a list
 * of which ones count is a list that goes stale the next time a tool grows a
 * new one.
 */
function datesIn(value: unknown): string[] {
  if (typeof value === "string") {
    const found: string[] = [];
    for (const hit of value.matchAll(DATE_TOKEN)) {
      const [whole, , month, day] = hit;
      // A loose filter, not a calendar \u2014 a slug or a filename can contain
      // four digits, a dash and two more that are not a date at all
      // (unlikely, but `01`-`12`/`01`-`31` costs nothing and rules out the
      // obvious non-dates).
  ...
  if (value && typeof value === "object") return Object.values(value).flatMap(datesIn);
  return [];
}
...
/**
 * B1307's own retry — a date named in the answer that no tool call, today's
 * own date or the person's own message actually carried this turn.
 */
const MISDATED_RETRY = `Stop. Your last answer named a date that nothing this turn actually gave you — not the tool you called, not today's date, not what they said. Whatever card or reading is in front of them carries the real date; say that one, exactly as it came back to you, never one you worked out or remembered.`;
...
export async function answerInThread(
  ...
  /**
   * Every date a tool actually returned this turn — B1307.
   *
   * *"A day for 2026-09-11 in Ungarn 2026"* went on screen from `start_day`'s
   * own, server-composed sentence; the model's own trailing prose named a
   * different date for the same day, and the next press landed on whichever
   * one the card actually carried — not the one the person had just read.
   * The button was always right; the words next to it were not, and there
   * was nothing to check that against.
   *
   * Seeded with `today` (a fact the model is told outright, not something a
   * tool drew) and with any date already in the person's own message — an
   * honest echo of what they typed is not an invented date.
   */
  const datesSeen = new Set<string>(datesIn(today).concat(datesIn(said)));
  /**
  ...
  /**
   * This turn's channel-scoped candidate areas — B1891. `pickArea`,
   * `switch_area`'s enum and its handler below all read this rather than
   * the full `AREAS`, so a WhatsApp turn is never offered, nor able to
   * widen into, `printed`, `journal`, `readers` or `money`.
   */
  const reachableAreas = channelAreas(channel);

  /**
   * Who is actually calling, for `runTool`'s own boundary — B1891. Derived
   * from `channel`, which both doors already pass honestly: `/ask` (web) is
   * reached only past a cookie, and WhatsApp's dispatch only after
   * `journalForNumber` has matched the sender's number. This is *not* the
   * area-level narrowing above — it is what still refuses a disallowed tool
   * even if a bug in that narrowing, or a persuasive `switch_area` call,
   * ever put its schema in front of the model.
   */
  const caller: Caller = channel === "web" ? { username, how: "cookie" } : { username, how: "whatsapp" };
   ...
   */
  const activeAreas = new Set<AreaKey>([
    HUB_AREA,
    await pickArea(client, messages, reachableAreas, username),
  ]);
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
      ...
      for (const call of calls) {
        ...
        if (call.name === "switch_area") {
          ...
          // Checked against this turn's own channel-scoped list, not every
          // area — B1891. The enum offered already matches, but a model can
          // say anything, and the widening below must never honour an area
          // this channel was never allowed to reach even if it does.
          const reachableKeys = new Set(reachableAreas.map((area) => area.key));
          const key = AREA_KEYS.find((one) => one === wanted && reachableKeys.has(one));
        ...
        const { ok, result, blocks: drawn, proposal, refused } = await runTool(
          ...
          selected,
          said,
          // The boundary itself — B1891. Whatever area this call's schema
          // came from, `runTool` refuses it unless `caller` actually allows
          // it.
          caller,
        );
        if (proposal) roundHasProposal = true;
        // Every date this call's own result actually carried — B1307. The
        // model reads this same JSON as its `tool_result`, so a date in the
        // final answer that is not here was not read off it.
        for (const one of datesIn(result)) datesSeen.add(one);
        /**
  ...
  function amiss():
    ...
    /**
     * A date said in the answer that no tool this turn, the system prompt's
     * own `today`, or the person's own message actually carried — B1307,
     * the date twin of `invented` just above. Gated the same way: only once
     * some date-bearing tool has actually run, so a turn about nothing dated
     * is never checked against an empty set.
     */
    if (datesSeen.size > 0) {
      const misdated = datesIn(answer).filter((one) => !datesSeen.has(one));
  ...
  if (wrong !== "") {
    ...
    /**
     * B1308 — the first, wrong attempt's own blocks must not survive
     * untouched into the retry's result.
     *
     * `blocks` (~line 2451) is declared once for the whole turn and every
     * round, including the retry's own call to `rounds()` below, pushes
     * onto the same array — so whatever the *first, wrong* attempt drew (a
     * `read_day` preview for a date that turned out to be the wrong one, in
     * scenario-costs.md's own defect F) sat untouched in front of whatever
     * the retry drew, or said, next to it — even when the retry's own
     * second attempt failed too and its text was replaced by `PLAINLY`
     * below, the stale block still shipped beside that plain sentence.
     *
     * Not a blanket clear — B1304's own investigation into this exact bug
     * found that unsafe for one shape only: a proposal genuinely still
     * pending, whose card (`form`/`confirm`, carrying `.proposal`) the
     * model may not redraw on a retry that only corrects wording, and
     * which is the person's one way to press it. So the rule is narrower
     * than "keep everything" or "keep nothing": snapshotted and cleared
     * before the retry runs; afterwards, only a still-pending *card* from
     * the first attempt is restored, and only when the retry did not draw
     * a card of its own to replace it. An informational block — a preview,
     * a list, anything with no button on it — from the first, wrong
     * attempt is never restored, which is the actual defect this
     * reproduces: nothing else in the shape list needed a exception, since
     * nothing else is something a person acts on.
     */
    const firstAttemptBlocks = blocks.slice();
    blocks.length = 0;
    answer = withoutMarkers(await rounds());
    const drewACardOfItsOwn = blocks.some((one) => one.shape === "form" || one.shape === "confirm");
    if (!drewACardOfItsOwn) {
      blocks.push(...firstAttemptBlocks.filter((one) => one.shape === "form" || one.shape === "confirm"));
    }
    const again = amiss();
    if (again !== "") {
