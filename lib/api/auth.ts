...
import { GPS_IMPORT_SCOPE, isJournalWideScope, resolveSession, type Session } from "../auth";
...
/**
 * `allowGpsWrite` — B2204. A `write:gps` token is refused here, at the one
 * place every bearer route authenticates, unless the route says it is the one
 * door that token exists for (`POST /api/v2/{user}/import`). Refusal by
 * default means a new route gated only on `ownsUser` cannot leak to a phone's
 * positions-only token by forgetting a check.
 */
export async function authenticate(
  ...
  if (session.scope === GPS_IMPORT_SCOPE && !opts.allowGpsWrite) {
    return { ok: false, status: 403, error: "gps_write_only" };
  }
 ...
 *
 * The out-of-scope refusal itself (B540's `{"error":"out_of_scope", ...}`)
 * used to live here as `outOfScope()`, building a v1-shaped `Response`
 * directly. B1734 deleted the last caller (the v1 `track` route) along with
 * `/api/v1` itself; `lib/api/v2/auth.ts`'s `outOfScopeRefusal` is the v2
 * equivalent, shaped through `fail()` instead.
 */
