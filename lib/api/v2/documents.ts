...
/**
 * A day that names a country gets the ISO code for it — B1907, the half of
 * B1540 that was left open.
 *
 * `lib/flags.ts` already holds the whole GeoNames name → alpha-2 table for
 * the flag beside a place name, and has since B1540; it was only ever read
 * on the way *out*, so the code the site showed a reader was never the code
 * the file carried and a photobook, an export or a map had nothing to
 * translate. Deriving it here, where a day is written, means nothing
 * downstream has to care.
 *
 * Conservative on purpose: a name the table does not know leaves the field
 * absent rather than guessing, an explicit code is never overwritten, and a
 * day that has *declined* the code keeps its decline — a file saying both
 * "HU" and "no country code was recorded" answers its own question twice.
 *
 * Mutates, and runs at the door BEFORE the required-or-declined check, so a
 * caller who names a country is never asked to decline a code the server can
 * work out for itself — and the write echoes what is on disk.
 */
export function deriveCountryCode(doc: Record<string, unknown> | DayFile): void {
  const day = doc as Record<string, unknown>;
  if (typeof day.country !== "string" || !day.country) return;
  if (day.countryCode !== undefined) return;
  if ((day.declined as Record<string, unknown> | undefined)?.countryCode !== undefined) return;
  const code = countryCodeFor(day.country);
  if (code) day.countryCode = code;
}
