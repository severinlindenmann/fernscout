...
export const costsApplyRequest = z.strictObject({
  /**
   * No ceiling, deliberately (B1848) — this is the absence AGENTS.md asks a
   * silent one to be turned into a stated one. A `contacts` import is capped
   * (`MAX_IMPORT_ROWS`) because every row sends somebody a confirmation
   * mail; a row here is inert — it writes a number onto a day nobody is
   * mailed about — and a bank statement can genuinely be long (a year, every
   * card on the household). The real bound is `REQUEST_MAX_BYTES`
   * (`lib/validate/media.ts`), already published, on the request the rows
   * arrive in, so there is nothing further to publish here.
   */
