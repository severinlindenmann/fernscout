...
import { GPS_IMPORT_SCOPE, type Session } from "../../auth";
...
export async function resolveBearer(
  request: Request,
  opts: { allowGpsWrite?: boolean } = {},
): Promise<BearerAuth> {
  const auth: ApiAuth = await authenticate(request, opts);
  if (auth.ok) return { ok: true, session: auth.session };
  if (auth.error === "gps_write_only") return { ok: false, response: gpsWriteOnlyRefusal() };
...
/**
 * Whether this session is the narrow `write:gps` token — B2204. Exported so
 * the one route that means to let it through one door
 * (`POST /api/v2/{user}/import`) and the one route that must not
 * accidentally describe it as journal-wide (`GET /api/v2/{user}/status`,
 * whose `token.scope` wire enum is frozen to `"owner" | "trip"` and has no
 * room for a third value) both ask the same question rather than each
 * comparing `session.scope` to the raw string.
 */
export function isGpsWriteScope(session: Session): boolean {
  return session.scope === GPS_IMPORT_SCOPE;
}

/** The refusal for a `write:gps` token reaching anything but the one door
 * that accepts it. */
export function gpsWriteOnlyRefusal() {
  return fail(
    "forbidden",
    "This token writes positions only (`POST /api/v2/{user}/import` with " +
      '`kind: "gps"` and dryRun false) and cannot read or write anything else.',
    undefined,
    403,
  );
}
