 ...
 * revised by B2239.
 *
 * The day's own date, then `date-2`, `date-3`, … on a collision. B1442 first
 * chose "day", "day-2" here specifically to avoid the date — at the time the
 * *title* was also being set to the bare date, so "day" kept the slug from
 * reinforcing that same mistake. The title half of that fix stands (a day
 * left untitled still gets no `title:` at all, and every surface renders the
 * formatted date instead), but "day", "day-2" reads as a mistake in a shared
 * link (B2239): `/trips/usa-2026/day/day` names nothing. A date reads as a
 * real address — every surface already falls back to the formatted date for
 * display, so the raw ISO string only ever appears in the URL, which is
 * exactly where a date belongs. Existing days are untouched: this only
 * decides the address for a day created untitled from here on.
 *
 * **Orphaned media counts as taken too — B1827.** A trip whose `originals/`
 * already holds a folder at this date left by an earlier, unrelated import —
 * the common case is a camera-roll import into a trip that already keeps
 * photographs — used to get handed that slug here anyway, since this only
 * checked entry files, and then be refused a few lines down by
 * `createDraft`'s own `slugHasOrphanedMedia` check as `slug_taken`. Checking
 * the same thing here that `createDraft` checks after is what makes the
 * placeholder actually free rather than merely unwritten.
 */
function nextUntitledSlug(ref: string, dir: string, date: string): string {
  let slug = date;
  for (let n = 2; entryFileWithSlug(dir, slug) || slugHasOrphanedMedia(ref, slug); n++) {
    slug = `${date}-${n}`;
  }
  return slug;
}
...
function declineText(track: Track, why: "no" | "unknown"): string {
  const about: Record<Track, string> = {
    ...
    tags: "which tags this day carries",
    visibility: "whether this day is held back from anyone the trip lets in",
  };
  return why === "no"
    ? `Nothing to record: ${about[track]}.`
    : // B2081 — two of the phrases are plural; "this day's tags is unknown" was not English.
      `Not recorded: ${about[track]} ${track === "photos" || track === "tags" ? "are" : "is"} unknown.`;
}
...
function buildDayFile(
  ...
  deriveCountryCode(day); // B1907 — after `declined`, which it reads
  return day;
}
...
export function createDraft(ref: string, input: DraftInput): WriteResult {
  ...
  const slug = input.title ? slugify(input.title) : nextUntitledSlug(ref, dir, input.date);
 ...
 * (`paid/postcard/lib/postcard/send.ts`'s `orderPhotoFile`, `paid/photobook/lib/photobook/source.ts`'s
...
function applyEditToDay(day: DayFile, input: EditInput): DayFile {
  ...
  // about where the day was leaves the file exactly as it found it (see
  // test/photo-captions.test.ts, "left alone rather than gaining a block").
  if (input.country !== undefined) deriveCountryCode(next);
  return next;
}
