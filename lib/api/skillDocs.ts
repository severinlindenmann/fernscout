...
import { COST_LABEL_MAX_CHARS, COST_LINES_MAX } from "./v2/schemas/day";
...
function addATrip(): string {
  return doc("add-a-trip", [
    "## Creating a trip",
    "A trip's id is client-chosen — you `PUT` it into existence rather than " +
      "`POST` to a collection. It is not permanent: `POST .../rename` below changes it, when it " +
      "was chosen wrong.",
    "```http\nPUT /api/v2/{user}/trips/{trip}\nContent-Type: application/json\nAuthorization: Bearer fs_agent_…\n\n" +
      fieldTable("/api/v2/{user}/trips/{trip}", "put", {
        id: "must match the URL's `{trip}` — lowercase words joined by hyphens",
    ...
    "## Planning a trip",
    "`plan` is a route of stops for a trip that has not happened yet — write it, or decline it, " +
      "like every other section above:",
    "```json\n{\"plan\": {\"mode\": \"nights\", \"readers\": \"map\", \"route\": [" +
      "{\"location\": \"Bangkok\", \"lat\": 13.7563, \"lng\": 100.5018, \"nights\": 4}, " +
      "{\"location\": \"Chiang Mai\", \"lat\": 18.7883, \"lng\": 98.9853}" +
      "]}}\n```",
    "**Each stop gets a stable `id` for free.** Leave it out and the server assigns a slug of " +
      "`location`, unique within the route — write your own only when you need to name it before " +
      "you know what the server would have chosen. `costs.items[].stop` (see /skill/costs.md) " +
      "and `plan.private.stops` (below) both key off this same id.",
    "**`mode` decides who owns `arrive`/`leave`.** `nights` (the default) is the server's: every " +
      "time the route is written, it derives each stop's `arrive`/`leave` from the trip's own " +
      "start date, the route's order and each stop's `nights` — sending `arrive`/`leave` " +
      "yourself in this mode is pointless, since the next write recomputes them anyway. A stop " +
      "with no `nights` of its own keeps whatever `arrive` the chain already reached, loses its " +
      "own `leave`, and every later stop loses both — there is no way to know when an unstated " +
      "stay ends. `dates` mode is the opposite: `arrive`/`leave` are taken exactly as sent, " +
      "gaps and overlaps included, and nothing is derived. Switch back to `nights` without " +
      "resending `nights` and it is recounted from whatever dates `dates` mode left in place.",
    "**`see`** is an optional list of `{name, lat, lng, source?}` — places worth visiting at a " +
      "stop, never a bed and never a bill. `source` (on `see` and on a stop itself) says how the " +
      "coordinates were arrived at: `typed`, `maps-link`, `pin` or `coordinates`.",
    "**`plan.private` is the owner's alone.** `{links?: [{label,url}], stops?: {[id]: " +
      "{stay?: {name,lat,lng}, links?: [{label,url}]}}}` — a hotel booked, a confirmation link, " +
      "anything that must never reach the public map. `GET` drops the whole key for anyone who " +
      "is not the owner; a `stops` key that names an id not in `route` is refused, naming it. " +
      "Put a hotel here, never as an ordinary stop — a stop is drawn on the map for everyone " +
      "who may read the trip.",
    "**`readers`** says whether the trip's readers get the map alone (`map`, the default) or the " +
      "fuller stop-by-stop detail (`details`) — never `plan.private`, which `readers` has no " +
      "power to widen.",
    ...
    "```http\nPOST /api/v2/{user}/trips/{trip}/rename\nContent-Type: application/json\nAuthorization: Bearer fs_agent_…\n\n" +
      "{\"id\": \"the-new-id\"}\n```",
    "Owner only, and refused when the new id is not a lowercase-and-hyphens slug (60 " +
      "characters or fewer), is the trip's own current id, or is already another trip's id. " +
      "Nothing is lost: every database row a `trip_id` column names moves with it, and the old " +
      "id keeps answering — `GET`/`PATCH`/`DELETE` at it come back `308` to the new address, " +
      "and so does the reader-facing trip page.",
    "## Drawing the party",
...
function addADay(): string {
  ...
  return doc("add-a-day", [
    ...
    "**A country gets its code for free.** A day that names a `country` the server can place " +
      "comes back carrying the matching ISO-3166 `countryCode` too, even when you did not send " +
      "one — that code is what a photobook prints its chapter titles from, in the book's own " +
      "language, and it happens before the asked-or-declined check, so naming a country does " +
      "not oblige you to decline its code as well. A name the server cannot place, or a day " +
      "that has declined `countryCode`, is left exactly as you sent it: nothing is guessed, " +
      "and an unplaceable name is still asked for. Send the code yourself whenever you know it.",
    ...
    "A merge-patch: send only what changed. Nothing is required — attaching one photograph " +
      ...
      "you correct stays published and visible to whoever already read it. A draft may stay " +
      "incomplete (a day the person started in the studio often is, and may have no title yet): " +
      "publishing asks for what is missing. A published day must stay complete — a correction " +
      "that would leave it incomplete answers `422 incomplete`. If the day changed while your " +
      "patch was being checked — it was published, or somebody else saved — the answer is " +
      "`409 stale_document` and nothing was written: read it again and resend.",
...
function ingestPhotos(): string {
  ...
  return doc("ingest-photos", [
    ...
    "To attach a staged file to a trip and day later, send the same POST again with " +
      ...
      "copying it.",
    "To attach a photograph already stored **day-less** on a trip (`intent.day` declined, or " +
      "not yet named) to a day, `POST /api/v2/{user}/trips/{trip}/days/{slug}/media` with " +
      '`{"items": [{"src": "…"}]}` — the same `src` `GET ' + path + "` already answers with. " +
      "This re-points the day document at the existing bytes; it never moves or copies the " +
      "file. `GET " + path + "` still answers this trip's own `day` for it correctly afterwards " +
      "— that answer comes from whichever day document names the `src`, not from which folder " +
      "the bytes sit in.",
    ...
    "**`intent.caption` is what you say at this door, and only lasts until a day names the " +
      ...
      "caption is the upload's own answer, not a second copy of the current one. Edit a " +
      "captioned photograph on its day, not by uploading it again.",
    "## What is accepted",
    "| | |\n| --- | --- |\n" +
      `| images | ${IMAGE_FORMATS.join(", ")} — at most ${(IMAGE_MAX_BYTES / 1024 / 1024).toFixed(0)} MB, ${IMAGE_MAX_EDGE}px on the longest edge, ${(IMAGE_MAX_PIXELS / 1_000_000).toFixed(0)} MP total |\n` +
      `| video | ${videoRow()} |\n` +
      `| per day | at most ${MAX_ITEMS_PER_DAY} items |\n` +
      `| per request | at most ${(REQUEST_MAX_BYTES / 1024 / 1024).toFixed(0)} MB of body |\n` +
      `| per JSON call | at most ${(JSON_BODY_MAX_BYTES / 1024 / 1024).toFixed(0)} MB of body on every other \`/api/v2\` JSON call, \`/import\` aside (\`limits.jsonBodyMaxBytes\`) |`,
...
function costs(): string {
  return doc("costs", [
    ...
    "`budget.total` and, optionally, `budget.days` and `budget.currency` (absent means the " +
      ...
      "Either kind of cost line may carry `stop`, a `plan.route[].id` — see /skill/add-a-trip.md " +
      "— when the spend belongs to one particular stop rather than the trip as a whole.",
    `A write takes at most ${COST_LINES_MAX} cost lines on one day (or in \`items\`), each ` +
      `\`label\` at most ${COST_LABEL_MAX_CHARS} characters — \`limits.costLinesMax\` and ` +
      "`limits.costLabelMaxChars` at `GET /api/v2/status`. Past either, the write is refused " +
      "with `400 invalid_request` naming the field.",
    ...
    `The same bounds hold here: a \`label\` longer than ${COST_LABEL_MAX_CHARS} characters is cut ` +
      ...
      "and the answer says which date it was.",
  ]);
}
