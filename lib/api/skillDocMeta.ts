...
/** Open core: a guide to a feature this build does not carry is not offered. */
const PAID_GUIDES: Partial<Record<SkillDocSlug, string>> = { "send-postcards": "postcards", "make-a-photobook": "photobook" };
...
export const SKILL_DOC_SLUGS: SkillDocSlug[] = ([
    ...
    "send-postcards",
    "make-a-photobook",
  ] as SkillDocSlug[]).filter((slug) => {
  const area = PAID_GUIDES[slug];
  return !area || PAID_AREAS.includes(area);
});
