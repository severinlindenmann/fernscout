...
export async function attachStagedFiles(
  ...
  for (const [at, id] of ids.entries()) {
    ...
    staged.push({
      ...
      // The sidecar this file already has MOVES onto the trip with it, so
      // everything the inbox collected — a description, coordinates measured
      // off EXIF, which door it came through — survives the filing and there
      // is still exactly one metadata file afterwards (B1864).
  ...
  // The bytes. The sidecar has usually already gone with the photograph;
  // `removeInboxFile` tolerates that and still clears the one left behind by
  // a file the batch recognised as a duplicate and did not store again.
