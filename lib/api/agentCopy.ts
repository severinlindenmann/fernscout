...
export const PERFECT_TRIP_EXAMPLE = [
  ...
  '  "rates": {"JPY": 0.0058},',
  '  "costsBudget": {"total": 6200, "currency": "CHF"},',
  '  "translations": {',
...
export const TRIP_FIELDS: {
  ...
  },
  {
    ...
    required: false,
    what:
      "`{\"total\": 6200, \"currency\": \"CHF\"}` — a budget for the whole trip. `currency` is " +
      "the journal's own base currency when left out. Not the same question as " +
      "`costsVisibility`: this is a number, that is who gets to see it — a trip may answer " +
      "either, both, or neither. Correctable later at `PATCH .../trips/<id>` (`costs.budget`).",
  },
  {
    key: "intro",
    required: false,
  ...
  },
  {
    ...
    required: false,
    what:
      "How the party is drawn, the v2 way: `{\"mode\": \"off\"}`, `{\"mode\": \"journal\"}` " +
      "(the journal's own default set), or `{\"mode\": \"custom\", \"figures\": [\"id\", …]}` " +
      "naming figures that already exist in this journal's figure library. Takes precedence " +
      "over `travellers` when both are somehow given. Correctable later at " +
      "`PATCH .../trips/<id>` (`figures`).",
  },
  {
    key: "rates",
    required: false,
