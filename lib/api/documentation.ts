...
import { whatsappNumberForDisplay } from "../contactNumber";
...
export function instanceDocumentation(): string {
  ...
  // document must not claim a door open that one missing credential shuts.
  const helperOnWhatsapp =
    isEnabled("helper") && isEnabled("whatsapp") && isEnabled("whatsappInbound");
  ...
  const lines: string[] = [
    ...
    ...wrap(
      "**The owner, in their own studio.** For the journal's own owner, who " +
        ...
        "then-publish API this document describes underneath. It cannot " +
        "skip the owner's own confirmation for anything this document says " +
        "an agent cannot finish either — deleting, a real postcard — " +
        "because the studio is a different door onto this API, not a way " +
        "around it.",
      78,
    ),
    "",
