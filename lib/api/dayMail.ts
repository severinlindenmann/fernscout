 ...
 *
 * `send_mail` itself is read by `paid/whatsapp/lib/api/publishFlags.ts`, together with
...
export function mailSummary(outcome: DayLetterOutcome): Record<string, unknown> {
  if (!outcome.ok) {
    return {
      ...
      // There used to be a `needed`/`balance` pair here, carried only for
      // `no_credits` — B366. B840 stopped charging for a letter, so no reason
      // a send can come back with is about money.
    };
  }
  return {
    attempted: true,
