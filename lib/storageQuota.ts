...
/**
 * Whether an amount of storage is worth drawing or offering to free — B1270,
 * B2090. Under a megabyte a breakdown is a bar with nothing visible in it and
 * "Free up 10 KB" is a button for nothing. Counted in bytes, not as a share
 * of the ceiling: a share moves when the ceiling does, so buying 5 GB used
 * to round a small journal to 0% and take its breakdown away.
 */
export function worthShowing(bytes: number): boolean {
  return bytes >= 1024 ** 2;
}
