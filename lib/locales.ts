...
/** Each locale merged over English, kept while neither file changes, so a
 * page asking twice — a layout and the page under it — gets the same object
 * back, and so does the scope cut from it below. */
const merged = new Map<string, { english: Dictionary; own: Dictionary; dictionary: Dictionary }>();

/** Scoped subsets, keyed on the merged dictionary they were cut from: a
 * changed file makes a new merge, and the stale subsets go with the old one. */
const scoped = new WeakMap<Dictionary, Map<LocaleScope, Dictionary>>();

/**
 * Where a dictionary is handed to the browser, and which strings go with it —
 * `lib/localeScopes.json`, generated by `npm run i18n:keys` from what the
 * files under each provider actually mention (scripts/locale-scopes-lib.mjs
 * says how, and where each scope is).
 */
export type LocaleScope = keyof typeof LOCALE_SCOPES;
 ...
 *
 * With a `scope`, only the strings that scope's files can ask for. Every
 * dictionary handed to a client component is scoped — without one, every page
 * carried all ~4,000 strings in its RSC payload, and a German page twice — and
 * test/locale-scopes.test.ts holds every such call to naming one. Unscoped is
 * for the server, which pays nothing to hold the lot.
 *
 * A scope whose files reach a paid area this build carries ships whole: the
 * keys `paid/` uses are not visible from this repository, and a missing one
 * would render as its key.
 */
export function dictionaryFor(code: string, scope?: LocaleScope): Dictionary {
  const full = mergedDictionary(code);
  if (!scope) return full;
  const spec = LOCALE_SCOPES[scope];
  if (spec.paid.some((area) => PAID_AREAS.includes(area))) return full;
  let byScope = scoped.get(full);
  if (!byScope) {
    byScope = new Map();
    scoped.set(full, byScope);
  }
  let subset = byScope.get(scope);
  if (!subset) {
    subset = {};
    ...
    byScope.set(scope, subset);
  }
  return subset;
}

function mergedDictionary(code: string): Dictionary {
  const english = readDictionary(FALLBACK_LOCALE);
  if (code === FALLBACK_LOCALE) return english;
  const own = readDictionary(code);
  const key = `${contentRoot()}::${code}`;
  const hit = merged.get(key);
  if (hit && hit.english === english && hit.own === own) return hit.dictionary;
  const dictionary = { ...english, ...own };
  merged.set(key, { english, own, dictionary });
  return dictionary;
}
...
export function dictionariesFor(username: string, scope?: LocaleScope): Record<string, Dictionary> {
  const out: Record<string, Dictionary> = {};
  for (const code of localesFor(username)) out[code] = dictionaryFor(code, scope);
  return out;
}
...
/**
 * The language a journal's pages render in for this request — `readerLocale`
 * over the journal's own `locales`, with the request's cookie and
 * `Accept-Language`.
 *
 * The journal layout asks this, and so does every provider nested inside it
 * (the studio's, the reader's own page): each ships only its own scope's
 * strings, so each must render in exactly the language the layout above it
 * chose, and one function is how the two answers stay the same.
 *
 * Server-only: it reads the request's cookies and headers.
 */
export async function journalLocale(journal: { locales: string[]; defaultLocale: string }): Promise<string> {
  const { cookies, headers } = await import("next/headers");
  const { LOCALE_COOKIE } = await import("./requestKeys");
  const chosen = (await cookies()).get(LOCALE_COOKIE)?.value;
