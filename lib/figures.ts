...
import { getTripIds, getTrips, tripDir, tripRef } from "./trips";
...
/** One trip's own figures answer, read raw off `trip.json` — B2022, the
 *  figure library's own "per trip" list. `"declined"` covers both an
 *  explicit `declined.figures` and the absence of both a `figures` field and
 *  a decline (a trip somehow missing the required-or-declined answer
 *  altogether, which `journalWrite`/`tripCreate` should never have accepted,
 *  but is read the safe way rather than assumed complete). */
export type FigureTripRow = {
  id: string;
  title: string;
  start: string;
  end: string;
  status: "past" | "current" | "upcoming";
  answer: "journal" | "off" | "custom" | "declined";
  /** Only meaningful when `answer` is `"custom"`. */
  figures: string[];
  /** The stored `declined.figures` reason, when `answer` is `"declined"` —
   *  the per-trip "not answered yet" row quotes it (the mock's own wording:
   *  "The trip document declined figures: “…”"). */
  declinedReason?: string;
  /** Who is on this trip (name + email) — the trip picker's own "people on
   *  this trip who have no figure yet" line needs this to find them. */
  people: { name: string; email: string }[];
};

type TripFiguresField = { mode: "off" } | { mode: "journal" } | { mode: "custom"; figures: string[] };

/** Every trip in the journal, with its own figures answer — the same read
 *  `figureReferences` above does per trip, restated as something a chip can
 *  name rather than a boolean. */
/** When a figure's file was last written — what "the latest figures" means
 * for the library, since the document itself carries no stamp. 0 when the
 * file cannot be read, which sorts it last. */
export function figureModifiedAt(username: string, id: string): number {
  try {
    return fs.statSync(figurePath(username, id)).mtimeMs;
  } catch {
    return 0;
  }
}

export function figureTripRows(username: string): FigureTripRow[] {
  return getTrips(username).map((t) => {
    const dir = tripDir(tripRef(username, t.id));
    let declined: string | undefined;
    let field: TripFiguresField | undefined;
    try {
      const raw = fs.readFileSync(path.join(dir, "trip.json"), "utf8");
      const data = JSON.parse(raw) as { figures?: TripFiguresField; declined?: Record<string, string> };
      declined = data.declined?.figures;
      field = data.figures;
    } catch {
      // No trip.json, or it will not parse — read the safe way, same
      // fail-open choice `figureReferences` makes above.
    }
    const base = {
      id: t.id,
      title: t.title,
      ...
      status: t.status,
      people: t.people.map((p) => ({ name: p.name, email: p.email })),
    };
    ...
    return { ...base, answer: field.mode, figures: [] };
  });
}
