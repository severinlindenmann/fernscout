...
export async function isOwner(username: string, request?: Request): Promise<boolean> {
  ...
  /**
   * Found in B2204's own security review (merge, 2026-09-24). A narrower agent scope —
   * `write:trip:…`, `write:gps`, the twenty-minute `exchange:token` handover
   * credential — used to widen to "owner" here on the address match alone,
   * with no scope check at all: a phone holding nothing but a positions-only
   * token could list and revoke every credential on the journal
   * (`/api/auth/{user}/keys`), reach the contacts admin surface, and read
   * drafts and person-level photographs through `draftsVisibleTo`/
   * `readerLevelFor`, none of which that token is meant to touch.
   * `isJournalWideScope` is the same test `mayActAsOwner` (v1) and
   * `requireJournalOwner` (v2) already apply — the unqualified
   * `SESSION_SCOPE.agent` only. A trip-scoped token of the owner's own
   * address, which this branch previously admitted, is now refused here too;
   * it still opens its one trip through `mayWriteTrip`/`writableTrips`, which
   * is where a trip-scoped credential's authority has always come from.
   */
  if (!isJournalWideScope(agent.scope)) return false;
