 ...
 *
 *  Exported for `app/api/helper/[user]/studio/runs/route.ts` too — B1751
 *  Task 4.3's own review finding. A run with nothing unused has nothing left
 *  to resume: zero photographs (an abandoned `POST .../studio/start` with
 ...
 *  photograph (`effectiveDate(p)` unset) still counts as unused on purpose —
 *  it still needs a date and a telling, so a run holding only those stays
 *  listed.
 *
 *  Matches on `effectiveDate(p)` (`lib/extract/dayCount.ts`), not `p.date`
 *  raw — B1828. `commitDay` moves photographs by that same rule, camera date
 *  included, and never writes `photo.date` back onto the manifest row. A
 *  camera-dated photograph the person never hand-edited was therefore
 *  invisible to `p.date` even after its day committed, so a finished run
 *  kept looking unused: still on the resume list, still warned about in the
 *  expiry mail. */
export function unusedPhotoCount(run: RunManifest): number {
  ...
  return run.photos.filter((p) => {
    if (p.dropped) return false;
    const date = effectiveDate(p);
    return !(date && committedDates.has(date));
  }).length;
}
