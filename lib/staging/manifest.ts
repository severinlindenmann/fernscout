 ...
 *  that accepts it says so itself (`POST .../studio/day`), and nothing that
...
export type DayRow = {
   ...
   *  by `POST .../studio/commit` right after a successful hand-off, never
...
export type RunManifest = {
   ...
   * for the whole run, by `PATCH .../studio/party`.
   *
   ...
   * separately let in (`studio.photos.whoCame.reassure`) — so it lives here, on
