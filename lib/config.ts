...
export const FEATURE_NAMES = [
  ...
  // tickets (B2196–B2201) reuse this same switch rather than inventing a
  // second one for the same feature.
  "routeRecording",
  // B2217. See lib/capabilities.ts's REQUIREMENTS entry for the whole
...
type Owner = {
   ...
   * (`paid/whatsapp/lib/digest/dayWhatsapp.ts`) needs digits and should not be re-parsing a
...
const DEFAULT_FEATURES: Record<FeatureName, FeatureConfig> = {
  ...
  // `extract`: this reads the owner's own position history to help write
  // their own journal, not the operator's money.
  routeRecording: { enabled: false },
  // B2217. Off by default like every optional capability, and off means the
  // photobook route map never fetches a tile — the map draws exactly as it
  // did before this ticket. A journal opts in for itself, the same shape as
