 ...
 *
 * **Ten, not twenty — B1981.** Twenty was chosen without measuring what a
 * book weighs. A 102-page book writes 326 MB (`estimatedBookBytes` in
 * `paid/photobook/lib/photobook/build.ts` has the measurements), so twenty of them is 6.5 GB
 * against a `perUserBytes` ceiling of 5 GB: the retention policy and the
 * storage ceiling contradicted each other, and retention lost, because
 * nothing prunes until an order finishes. Ten keeps the worst case inside
 * the allowance.
 */
const DEFAULT_PHOTOBOOK_ORDERS_PER_USER = 10;
