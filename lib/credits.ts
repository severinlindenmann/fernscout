...
import { creditsFromUnits, toUnits } from "./creditsFormat";
 ...
 * `paid/whatsapp/lib/digest/dayWhatsapp.ts`, and `POSTCARD_CREDITS` per printed card posted
 * by `paid/postcard/lib/postcard/send.ts` (B434). Those three are the only callers of
 ...
 * `paid/postcard/lib/postcard/send.ts` for why the claim comes before the spend.
 *
 ...
 * callers, and `paid/test/credits.test.ts` asserts exactly that allowlist.
 * `paid/credits/scripts/grant-credits.ts` needs a shell on the server, after money has
 ...
 * that leaves the building.** `paid/test/credits.test.ts` asserts it, because this
 ...
 * property 1 above is stated as loudly as it is. `paid/credits/scripts/grant-credits.ts`
 ...
 * (`paid/credits/lib/payments.ts`) and `listFiguresPage` (`lib/figures.ts`) already use
