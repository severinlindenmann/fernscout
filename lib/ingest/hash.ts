 ...
 */
/**
 * `threshold` is how many bits either axis may differ by, and it defaults to
 * {@link DUPLICATE_THRESHOLD} — the upload door's own tight three.
 *
 * It is a parameter because the two callers have opposite cost asymmetries
 * (B1978). At upload a false positive silently destroys a photograph (see
 * `DUPLICATE_THRESHOLD`, and B872's 38 lost pictures), so three stays. In a
 * photobook the loser is not deleted — it stays in the journal and in the
 * gallery, it is merely not printed — so the book can afford 12, which is
 * where the owner's own journal measures a clean 16-bit gap: within-day
 * pairs have a 1st percentile of 28 and the cross-day noise floor is 46, so
 * everything between 12 and 28 is empty.
 *
 * `MIN_SIGNAL_BITS` is not parameterised and must not be: it is what stops
 * two unrelated skies matching, and a looser threshold needs it more, not
 * less.
 */
export function isDuplicate(a: string, b: string, threshold = DUPLICATE_THRESHOLD): boolean {
  ...
  for (const cut of [0, 16]) {
    ...
    if (hammingDistance(x, y) > threshold) return false;
