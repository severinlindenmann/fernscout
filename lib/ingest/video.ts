...
/**
 * `com.apple.quicktime.location.ISO6709`, e.g. `+47.4156+008.2563+389.847/`.
 *
 * ISO 6709's sign-prefixed form always carries the sign, so a regex is exact
 * rather than a heuristic — B1755. Altitude and the trailing solidus are
 * optional; a malformed or absent tag yields `undefined` rather than a wrong
 * number or a throw. Out-of-range values (a corrupt tag) and `0,0` (no fix,
 * same reasoning as the EXIF reader) are dropped too.
 */
export function readLocation(
  tags: Record<string, string> | undefined,
): { lat: number; lng: number } | undefined {
  if (!tags) return undefined;
  const raw = tags["com.apple.quicktime.location.ISO6709"];
  if (!raw) return undefined;
...
export async function probeVideo(file: string): Promise<VideoProbe | null> {
  ...
  try {
    ...
    const location = readLocation(json.format?.tags);
    return {
...
/**
 * The same shape `photoMetaFromExif` (lib/ingest/exif.ts) hands a photo's
 * sidecar, read off a clip's own container tags instead — B1852. Must run on
 * the *input* file, before `transcodeVideo` strips this with
 * `-map_metadata -1`: a clip sent as a WhatsApp document is often the only
 * place a date or a position exists at all, and the transcode is what throws
 * it away.
 */
/** @public open core: paid/ uses this (tagged by open-core/split). */
export function videoMetaFromProbe(
  probe: VideoProbe | null,
): { lat?: number; lon?: number; takenAt?: string } | null {
  if (!probe) return null;
  const takenAt = probe.takenAt ? `${isoDate(probe.takenAt)}T${isoTime(probe.takenAt)}:00` : undefined;
  if (probe.lat === undefined && probe.lng === undefined && takenAt === undefined) return null;
  return {
    ...(probe.lat !== undefined ? { lat: probe.lat } : {}),
    ...(probe.lng !== undefined ? { lon: probe.lng } : {}),
