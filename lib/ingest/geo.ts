...
/**
 * int32 lat, int32 lng, uint16 country, uint8 popScale, uint8 isSection,
 * uint8 nameLen, uint32 nameOffset.
 *
 * `isSection` — coordinator review 2026-09-24 — is GeoNames' `PPLX` feature
 * code (a section of a populated place: a city district or borough,
 * "Budapest III. kerület", "Kelenföld") in one byte rather than a second
 * index: `reverseGeocode` still finds a section exactly as before (a photo
 * taken in one is still named after it, which is the more precise answer),
 * `placesInBox` is the one reader that skips it — a route map draws towns,
 * not the districts of the one it is already naming as a stop.
 */
...
function readPlace(index: Index, i: number, lat: number, lng: number): Place {
  ...
  // `isSection` (at + 11) is read only by `placesInBox` — reverse geocoding
  // names a photograph after a city district exactly as it always has.
  const nameLen = index.records.readUInt8(at + 12);
  const nameOffset = index.records.readUInt32BE(at + 13);
  return {
 ...
 */
/**
 * A query box around a frame's own extent, padded proportionally to its
 * size rather than by a fixed number of degrees — B2211. A fixed degree pad
 * is generous on a close frame (Budapest/Csepel-sziget, a few hundredths of
 * a degree wide) and stingy on a continental one, so a close frame's box
 * reached past the visible page into whichever big city was nearest, and
 * `placesInBox`/`peaksNear`'s own population/height ranking then spent their
 * whole candidate limit on cities the page never draws. Floored so a
 * near-point frame still queries a usable area.
 */
export function paddedFrameBox(points: readonly { lat: number; lng: number }[]): {
  south: number;
  west: number;
  north: number;
  east: number;
  ...
  const pad = Math.max(north - south, east - west, 0.02) * 0.6 + 0.05;
...
export function placesInBox(
  ...
  for (let i = from; i < to; i++) {
    ...
    // A section of a populated place (GeoNames' `PPLX`: a city district or
    // borough, "Budapest III. kerület", "Kelenföld") is not a town a route
    // map should draw as one — coordinator review 2026-09-24. Skipped here,
    // not in `readPlace`: reverse geocoding a photograph still names it
    // after the district it was actually taken in.
