...
import { getFigureDoc, writeFigureDoc } from "./figures";
 ...
 * as given — B2009. Absent reads as `nights`, the older and stricter shape. */
export const PLAN_MODES = ["nights", "dates"] as const;
/** Whether a `plan` reads as the map alone, or the fuller stop-by-stop
 * detail — B2009. Absent reads as `map`, today's only rendering. */
export const PLAN_READERS = ["map", "details"] as const;
/** How a stop's coordinates were arrived at — B2009, so a reader (or a
 * future audit) can tell a hand-typed guess from a pin dropped on a map. */
export const PLAN_STOP_SOURCES = ["typed", "maps-link", "pin", "coordinates"] as const;
...
export type NewTrip = {
  ...
  /**
   * A budget for the whole trip — B2021. Raw, validated by
   * `costsBudgetBlock`: `{"total": 6200, "currency": "CHF"}`, currency
   * optional (defaults to the journal's own base currency at the read path,
   * so nothing is guessed here). Merged onto `costs.budget`, alongside
   * `costsVisibility`'s own `costs.visibility`, so a caller may answer
   * either question, both, or neither.
   */
  costsBudget?: unknown;
  /**
   * How the party is drawn, v2-style — B2021. Raw, validated against the
   * same `tripFigures` union the contract itself checks
   * (`{mode:"off"}` | `{mode:"journal"}` | `{mode:"custom", figures:[ids]}`),
   * so a trip this writes can never disagree with what a later PATCH would
   * accept. Takes precedence over `travellers` (the older, inline v1 shape)
   * when both are somehow given, since this is the shape the studio's new
   * step actually sends.
   */
  figuresMode?: unknown;
  /**
...
/** Every key a `costsBudget` answer may carry — checked so an unknown one is
 * refused by name rather than silently written. */
const COSTS_BUDGET_FIELDS: ReadonlySet<string> = new Set(["total", "currency"]);

/**
 * The trip's `costs.budget` — B2021, the new-trip step's "Money" question.
 *
 * A total and, optionally, a currency (defaults to the journal's base
 * currency at the read path — `costsBudgetBlock` never guesses one).
 * Refused, never dropped, the same discipline every other block here keeps:
 * a 201 that silently rounded or dropped a number is worse than a 400
 * naming the field.
 */
function costsBudgetBlock(raw: unknown): BlockResult {
  if (raw === undefined || raw === null) return { ok: true, lines: [], value: undefined };
  if (typeof raw !== "object" || Array.isArray(raw)) {
    return {
      ok: false,
      error: "invalid_costs_budget",
      message: 'costsBudget must be an object, e.g. {"total": 6200, "currency": "CHF"}.',
    };
  }
  const entry = raw as Record<string, unknown>;
  const unknown = Object.keys(entry).filter((k) => !COSTS_BUDGET_FIELDS.has(k));
  if (unknown.length > 0) {
    return {
      ok: false,
      error: "invalid_costs_budget",
      message:
        `costsBudget has ${unknown.map((k) => JSON.stringify(k)).join(", ")}, which is not a ` +
        `budget field. Expected: total, currency.`,
    };
  }
  const total = typeof entry.total === "string" ? Number(entry.total) : entry.total;
  if (typeof total !== "number" || !Number.isFinite(total) || total <= 0) {
    return {
      ok: false,
      error: "invalid_costs_budget",
      message: `costsBudget.total must be a positive number, got ${JSON.stringify(entry.total)}.`,
    };
  }
  let currency: string | undefined;
  if (entry.currency !== undefined) {
    const code = typeof entry.currency === "string" ? normalizeCurrency(entry.currency) : undefined;
    if (!code) {
      return {
        ok: false,
        error: "invalid_costs_budget",
        message: `costsBudget.currency is ${JSON.stringify(entry.currency)}; expected a three-letter currency code, like "CHF".`,
      };
    }
    currency = code;
  }
  return { ok: true, lines: [], value: { total, ...(currency ? { currency } : {}) } };
}

/**
 * The trip's `figures` mode — B2021's "Walking figures" question.
 *
 * Hand-validated against the same three shapes `tripFigures`
 * (`lib/api/v2/schemas/figures.ts`) checks, rather than importing that
 * schema — the same reason `createTrip`'s own `declined` handling does not
 * reimport `DECLINABLE_KEYS` from `lib/api/v2/schemas/trip.ts` (see that
 * block's own doc comment): `figures.ts` imports `ID_RE` from *this* file,
 * so importing its schema back here is a circular module — confirmed the
 * hard way (a `ReferenceError: Cannot access '…' before initialization` at
 * build time, the same class of failure `trip.ts`'s own comment warns
 * about). A `custom` id has to already exist in this journal's figure
 * library — chosen from it, never invented — so each is read back with
 * `getFigureDoc` before it is trusted.
 */
function figuresModeBlock(username: string, raw: unknown): BlockResult {
  if (raw === undefined || raw === null) return { ok: true, lines: [], value: undefined };
  const USAGE = 'figuresMode must be {"mode":"off"}, {"mode":"journal"}, or {"mode":"custom","figures":["id", …]}.';
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
    return { ok: false, error: "invalid_figures", message: USAGE };
  }
  const entry = raw as Record<string, unknown>;
  if (entry.mode === "off" || entry.mode === "journal") {
    if (Object.keys(entry).length > 1) {
      return { ok: false, error: "invalid_figures", message: USAGE };
    }
    return { ok: true, lines: [], value: { mode: entry.mode } };
  }
  if (entry.mode === "custom") {
    const ids = entry.figures;
    if (
      !Array.isArray(ids) ||
      ids.length === 0 ||
      ids.length > MAX_FIGURES ||
      !ids.every((id) => typeof id === "string" && id.length > 0)
    ) {
      return {
        ok: false,
        error: "invalid_figures",
        message: `figuresMode.figures must be a list of 1 to ${MAX_FIGURES} figure ids.`,
      };
    }
    const missing = ids.filter((id) => !getFigureDoc(username, id));
    if (missing.length > 0) {
      return {
        ok: false,
        error: "unknown_figure",
        message:
          `figuresMode.figures names ${missing.map((id) => JSON.stringify(id)).join(", ")}, which ` +
          `${missing.length === 1 ? "is" : "are"} not in this journal's figure library — ` +
          `create it at GET /api/v2/${username}/figures first.`,
      };
    }
    return { ok: true, lines: [], value: { mode: "custom", figures: ids } };
  }
  return { ok: false, error: "invalid_figures", message: USAGE };
}
...
export function createTrip(username: string, input: NewTrip): CreateTripResult {
   ...
   */
  if (input.teaser !== undefined && visibility === "public") {
    return {
      ok: false,
      error: "invalid_teaser",
      message:
        `teaser asks for a closed trip to be named on the trips page without being ` +
  ...
  const costsBudgetResult = costsBudgetBlock(input.costsBudget);
  if (!costsBudgetResult.ok) {
    return { ok: false, error: costsBudgetResult.error, message: costsBudgetResult.message };
  }
  const figuresModeResult = figuresModeBlock(username, input.figuresMode);
  if (!figuresModeResult.ok) {
    return { ok: false, error: figuresModeResult.error, message: figuresModeResult.message };
  }
  ...
  /**
   * `plan` is never asked at create — B2021. Nothing this function takes
   * lets a caller state a route, so a brand-new trip is always without one;
   * the planner's first stop is what retracts this decline (the same shape
   * `days` above already has).
   */
  if (declined.plan === undefined) {
    declined.plan = "not entered during setup";
  }
  ...
  const trip: TripFile = {
    ...
    // door has never asked for one on its own, so `costsVisibility` alone
    // still writes exactly that key and nothing invented beside it. B2021
    // adds the other half: a budget total, merged onto the same section
    // rather than fighting it for the key.
    ...(() => {
      const budget = costsBudgetResult.value as { total: number; currency?: string } | undefined;
      const section: Record<string, unknown> = {};
      if (costsVisibility === "guests") section.visibility = "guests";
      if (budget) section.budget = budget;
      return Object.keys(section).length ? { costs: section as TripFile["costs"] } : {};
    })(),
    ...
    // B2021's `figuresMode` (the v2 shape) takes precedence over the older,
    // inline `travellers:` block when both are somehow given — the studio
    // step only ever sends the former.
    ...(() => {
      if (figuresModeResult.value !== undefined) {
        return { figures: figuresModeResult.value as TripFile["figures"] };
      }
