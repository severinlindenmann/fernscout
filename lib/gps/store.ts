 ...
 * asserts the import graph, the same way `paid/test/postcard-orders.test.ts`
...
/**
 * Delete whole month files — the standalone purge (B1843 addendum): the
 * owner picking specific months, or all of them, from the studio location
 * page or the v2 API. Real removal, not a flag: `fs.rmSync` on the file
 * itself.
 */
export function deleteMonths(username: string, months: string[]): void {
  for (const month of months) {
    fs.rmSync(monthFile(username, month), { force: true });
  }
}

/** Every month currently on disk, sorted — what a purge screen offers, and
 * what "purge everything" means. Names months, never a fix. */
export function listMonths(username: string): string[] {
  let entries: string[];
  try {
    entries = fs.readdirSync(gpsDir(username));
  } catch {
    return [];
  }
  return entries
...
/**
 * Remove only the fixes in `[from, to]` — the discard step's own purge
 * (B1843 addendum), precise where `deleteMonths` is not: an owner who
 * imports a mid-trip export and discards it must not also lose whatever a
 * phone recorded for the rest of that month (B2196 records for weeks). A
 * month left with nothing is removed the same as `deleteMonths` would leave
 * it; a month left with something is rewritten whole, the same write path
 * `writeMonth` already uses.
 */
export function deleteRange(username: string, from: number, to: number): { removed: number; months: string[] } {
  // Stored fixes are `normalise`d to whole seconds (rounded to the *nearest*
  // second, not floored), but `from`/`to` arrive as raw millisecond
  // timestamps straight off the import — the first and last fix's own `t`,
  // before rounding. A fix a `.4` into its second rounds down on write and
  // then sits just *before* an unrounded `from`; one a `.6` in rounds up and
  // sits just *after* an unrounded `to`. Either way it reads as outside the
  // range and survives its own import's discard. Floor `from` and ceil `to`
  // to the second first — rounding can move a stored fix by at most 500 ms,
  // so widening by a whole second on each side covers every direction it
  // could have rounded, and the range stays inclusive.
  const rangeFrom = Math.floor(from / 1000) * 1000;
  const rangeTo = Math.ceil(to / 1000) * 1000;
  let removed = 0;
  const months: string[] = [];
  for (const month of listMonths(username)) {
    const existing = readMonth(username, month);
    const kept = existing.filter((f) => f.t < rangeFrom || f.t > rangeTo);
    if (kept.length === existing.length) continue;
    removed += existing.length - kept.length;
    months.push(month);
    if (kept.length === 0) fs.rmSync(monthFile(username, month), { force: true });
    else writeMonth(username, month, kept);
  }
  return { removed, months };
}
