...
export type GalleryItem = {
  ...
  caption?: string;
  /**
   * What the photograph shows, for a reader who cannot see it — B1867.
   *
   * Not a caption: a caption is what the picture is *about* and is printed
   * under it; this is what is in the frame, and it is announced instead of
   * the picture. Written once by the journal's helper into the photograph's
   * sidecar (`described.altText`, B1866) and read from there — never typed on
   * a day, never accepted over the API, and absent for every photograph
   * nothing has described. `alt ?? caption ?? ""` is what every `<img>` says.
   */
  alt?: string;
  width?: number;
  height?: number;
...
export type MediaTile = {
  ...
  type: "image" | "video";
  caption?: string;
  /** See `GalleryItem.alt` — carried through so a tile in the grid says what
   * the photograph shows rather than nothing. */
  alt?: string;
  width?: number;
  height?: number;
  poster?: string;
 ...
 * (`paid/postcard/lib/postcard/entry.ts`), because that module is `server-only` and the
...
export type PlanSee = { name: string; lat: number; lng: number };

/** One stop on the intended route, from a trip's plan.md or from a
 * future-dated draft (W33). */
export type PlannedStop = {
  /** The stop's own id, as `plan.private.stops` keys against it (B2009) —
   * absent for a draft-derived stop, which carries no id of its own. */
  id?: string;
  location: string;
  country: string;
  countryCode?: string;
  ...
  lng: number;
  note?: string;
  /** How many nights this stop lasts — the `details` reader level's own
   * field (B2012). */
  nights?: number;
  arrive?: string;
  leave?: string;
  /** Places worth seeing here — `details` reader level only. */
  see?: PlanSee[];
...
/** Whether a reader of the plan gets the map alone, or the fuller stop-by-
 * stop detail — B2012. Mirrors `PLAN_READERS` (lib/tripWrite.ts). */
export type PlanReaders = "map" | "details";

/** The stay and links behind a plan's stops — `lib/plan.ts`'s
 * `getPlanPrivate`, never handed to a client for anybody but a `person`
 * reader. A plain data shape here (rather than in that server-only module)
 * so a client component can name the type without importing anything that
 * throws when bundled for the browser. */
export type PlanPrivateView = {
  links: { label: string; url: string }[];
  stops: Record<string, { stay?: { name: string; lat: number; lng: number }; links?: { label: string; url: string }[] }>;
};
