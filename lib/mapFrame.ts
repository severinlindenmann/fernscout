...
export function place(frame: Frame, point: Point): [number, number] {
  ...
  return [round(nearestX(frame, x * frame.lngScale)), round(y)];
}

/**
 * The canvas is a cylinder, and a frame may run off its right-hand edge.
 *
 * `frameRoute` unwraps a route that crosses the antimeridian (see `unwrap`),
 * so its frame can sit past x = 1000 — and a point on the far side of the
 * line projects to a small x that would land thousands of kilometres to the
 * west of it. Three representations of the same longitude exist, one world
 * apart each; the one nearest the frame's centre is the one this frame means.
 */
function nearestX(frame: Frame, x: number): number {
  const world = MAP_VIEWBOX.width * frame.lngScale;
  const centre = frame.x + frame.w / 2;
  let best = x;
  for (const candidate of [x - world, x + world]) {
    if (Math.abs(candidate - centre) < Math.abs(best - centre)) best = candidate;
  }
  return best;
}

/**
 * A route that crosses the antimeridian, written as one continuous run.
 *
 * Auckland to Nadi is 2,150 km and spans 5° of longitude — but written as
 * 174.8 and -177.4 it spans 352°, so the bounding box below framed nearly the
 * whole world and put two stops on opposite edges of it. When the raw span is
 * more than half the globe the short way round is the other way, and adding
 * 360° to the western longitudes is what makes the numbers say so. `place()`
 * puts a point back inside the frame that results.
 */
function unwrap(points: readonly Point[]): Point[] {
  const lngs = points.map((p) => p.lng);
  if (Math.max(...lngs) - Math.min(...lngs) <= 180) return [...points];
  return points.map((p) => (p.lng < 0 ? { ...p, lng: p.lng + 360 } : p));
}
...
export function frameRoute(points: readonly Point[]): Frame {
  ...
  const plottable = unwrap(points.filter(isPlottable));
