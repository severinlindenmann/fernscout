 ...
 *
 * A row's own `date` (set by the person, `PATCH .../studio/run`) wins over
...
export async function commitDay(
  ...
  // Staged before the upload door checked pixels (B2205), or by anything that
  // skipped it: never handed on into a day folder. Read before the lock
  // because the header read is async and `move` is not; a staged file's id is
  // its content hash, so the answer cannot change under the lock.
  const tooManyPixels = new Set<string>();
  for (const photo of initial.kept) {
    const bytes = readStagedFile(username, runId, photo.id);
    if (bytes && (await declaresTooManyPixels(bytes))) tooManyPixels.add(photo.id);
  }
  ...
  const move = (): number => {
    ...
    for (const photo of kept) {
      ...
      if (!bytes) continue;
      if (tooManyPixels.has(photo.id)) continue;
...
/**
 * The title an imported day is written with — B2081. The place the person
 * answered for it, when that reads as a name (short, and something a slug can
 * be made of); otherwise the date in words ("2 July 2019"), a fact rather than
 * a placeholder, so the day's address and its photographs' folder are never
 * the bare `day`/`day-2` an untitled day falls back to.
 */
export function titleForDay(location: string | undefined, date: string): string {
  const place = location?.trim();
  if (place && place.length <= 60 && slugify(place) !== "") return place;
  return titleFromSpan(date, date);
}
 ...
 * discipline (`tripIdBase` from `lib/tripId.ts`, a collision loop through the
 * one function that already knows what a trip id is), asked for rather than
 * a second way to name a trip. Not imported directly: that one is a small
 * private helper in a route file, and generating an id is not itself "a way
 * to create a trip" — `createTrip` below is, and it is reused unchanged.
 */
