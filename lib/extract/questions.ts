...
export function questionsForDay(
  ...
  if (group.undated) {
    out.push({
      ...
      fills: "date",
      text:
        `These ${count} don't carry a date. Roughly when were they taken?`,
    });
  }
  ...
  if (group.lat === undefined) {
    out.push({
      ...
      kind: "gap",
      fills: "location",
      // A fact about the files, not a guess at where they came from (B2057):
      // nothing here knows whether they were sent, downloaded or shot with
      // location off.
      text: `These ${count} carry no place. Where were you?`,
    });
  }
