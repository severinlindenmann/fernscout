...
/**
 * The scope string a token that may only import positions carries — B2204.
 *
 * A third scope shape, beside the unqualified `write:content` and the
 * `write:trip:<id>` family. It is deliberately not journal-wide
 * (`isJournalWideScope` says no) and does not match `tripWriteScope`'s
 * prefix either, so every gate that already asks one of those two questions
 * — `mayActAsOwner`, `mayWriteTrip`/`tripWriteVerdict` — refuses it by
 * construction, with nothing new to teach them. The one door that means to
 * let it in (`POST /api/v2/{user}/import`) checks for this string itself,
 * narrowly, alongside `kind: "gps"` and `dryRun: false`.
 */
export const GPS_IMPORT_SCOPE = "write:gps";

/** Thirty days — long enough that a phone left recording over a trip does
 * not need the owner to re-open the studio mid-journey (B2204's D3: "keep
 * recording" lasts until the owner stops it), refreshed by the app rather
 * than left to expire on its own. Deliberately not `SESSION_TTL_MS.agent`
 * (seven days): this credential lives in a phone's Keychain, not a chat
 * log, so the exposure `handover`'s twenty minutes defends against does not
 * apply the same way. */
const GPS_TOKEN_TTL_MS = 30 * 24 * 60 * 60 * 1000;
export const GPS_TOKEN_TTL_DAYS = String(GPS_TOKEN_TTL_MS / (24 * 60 * 60 * 1000));
...
export function describeScope(
  ...
  // `write:gps` (B2204) is deliberately not translated here: this function's
  // two-value result feeds `GET /api/v2/{user}/status`'s frozen wire enum
  // (`lib/api/v2/schemas/status.ts`), and that route refuses a `write:gps`
  // bearer before it is ever called — see its own comment. The owner's keys
  // page needs a third, honest label for the same token and reads the raw
  // scope itself (`app/api/auth/[user]/keys/route.ts`'s `scopeOf`) rather
  // than asking this function to grow a value one of its two callers must
  // never see.
...
export async function issueCode(
  ...
  // App Review's one fixed code (B2125), or a random one for everybody else.
  const code = reviewLoginCodeFor(address, owner === NO_JOURNAL ? null : owner) ?? generateCode();
  /**
...
/**
 * Mint the owner's `write:gps` token — B2204, the credential a phone holds
 * to upload positions and nothing else.
 *
 * **One active token per owner is enough**, so minting a new one revokes
 * every live one this journal has already issued — a re-connect (a new
 * phone, a reinstall) is meant to replace, not accumulate. Revoking first
 * is the same "safe direction to fail" `exchangeHandover` uses: a failure
 * between the two leaves the owner with no working token rather than two
 * live ones, and pressing the button again is cheap.
 *
 * Kind stays `"agent"` on purpose — `authenticate()` asks `resolveSession`
 * for `"agent"` and nothing else, so this token has to be one to be usable
 * at all. What makes it narrow is the scope, not the kind, exactly the way
 * a trip-scoped token already works.
 */
export async function issueGpsToken(
  owner: string,
  email: string,
): Promise<{ token: string; expiresAt: string }> {
  await revokeGpsTokens(owner, email);
  const result = await openSession(owner, email, "agent", GPS_IMPORT_SCOPE, null, GPS_TOKEN_TTL_MS);
  if (!result.ok) throw new Error("could not open a gps session");
  return { token: result.token, expiresAt: result.expiresAt };
}

/** Every live `write:gps` token this journal issued to this address —
 * scoped narrower than `revokeSessionsForAddress`, which would also take
 * down an ordinary agent token the same address happens to hold. */
async function revokeGpsTokens(owner: string, email: string): Promise<void> {
  const { db } = await getDatabase();
  const address = normaliseEmail(email);
  await db
