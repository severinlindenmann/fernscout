...
/**
 * `next dev` writes `<distDir>/dev/types/validator.ts`, which imports every
 * route's `page.js` by path. Delete a route and that file keeps the stale
 * import — `next build`'s own type-check then fails with a TS2307 naming a
 * source file that never existed on this branch, which reads like a real
 * dangling reference rather than a leftover generated cache (B1973). It is
 * regenerated by the next `next dev` run, so clearing it before build costs
 * nothing.
 */
const distDir = process.env.NEXT_DIST_DIR || ".next";
const devTypes = path.join(process.cwd(), distDir, "dev", "types");
if (fs.existsSync(devTypes)) {
  fs.rmSync(devTypes, { recursive: true, force: true });
  console.log(`Cleared stale ${distDir}/dev/types (regenerated by \`next dev\`; avoids B1973).`);
}

let skipBuild = false;
if (quick) {
  const routeTypes = inspectRouteTypesStamp(process.cwd(), distDir);
