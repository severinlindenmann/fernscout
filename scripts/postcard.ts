 ...
 *   npm run postcard -- --photo <file> --message "..." --from-contacts --user <username>
 ...
 *
 * Recipients are either a JSON array of postal addresses (`--to`), or —
 * since B273 — `--from-contacts`, which reads the contacts table for this
 * journal instead: every `active` contact who ticked "send me a real
 * postcard" and has enough of an address to reach. `--to` is the fallback for
 * people who would rather keep a file, or who are posting to somebody who
 * never went through the guest form at all.
 *
 * Run through `npm run postcard`, not `tsx scripts/postcard.ts` directly:
 * `lib/postcard/contacts.ts` imports `server-only` lib/ modules, which throw
 * unless the `react-server` export condition is active — supplied here by the
 * npm script's `--conditions=react-server` flag.
 */
...
if (fromContacts && !hasContactsKey()) {
  fail(
    "--from-contacts needs CONTACTS_ENCRYPTION_KEY set, the same key the site decrypts postal " +
      "addresses with — without it every stored address reads as nothing.",
  );
}
if (backend !== "dry-run") {
...
async function main() {
  ...
  const recipients = fromContacts
    ...
    : readRecipients(toPath as string);
  if (fromContacts && recipients.length === 0) {
    fail(
      `No contact of ${owner}'s is both approved and asked for a postcard, with enough of an ` +
        "address to reach. Nobody has anywhere to write to yet — check the guest list at " +
        "/{owner}/contacts, or pass --to with a file instead.",
    );
  }
  ...
  for (const [index, to] of recipients.entries()) {
    ...
    write(
      `${base}-front.pdf`,
      renderPostcard({ photo, message: message as string, from, to, sides: "front" }).pdf,
    );
    write(
      `${base}-back.pdf`,
      renderPostcard({ photo, message: message as string, from, to, sides: "back" }).pdf,
    );
...
// The open SQLite handle is the only thing holding the event loop — same
// reason `scripts/digest.mts` closes it in a `finally`. Harmless when
// `--from-contacts` was never used: `closeDatabase` is a no-op with no handle
// open.
