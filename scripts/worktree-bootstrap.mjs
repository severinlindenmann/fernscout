...
function linkHarnessSkills() {
  const harnessSkills = harnessSkillsLinkTarget(main.worktree, root);
  if (!harnessSkills) return;
  const worktreeSkills = path.join(root, ".claude", "skills");
  fs.mkdirSync(path.dirname(worktreeSkills), { recursive: true });
  fs.symlinkSync(harnessSkills, worktreeSkills, "dir");
