...
# the owner's own open WhatsApp window. Never fatal for the same reason.
log "checking for WhatsApp gap nudges due tonight"
if (cd "$APP_DIR" && npm run --silent gap-nudges:send); then
  log "gap-nudge sweep done"
else
...
case "$repo_state" in
  ...
  absent)
    ...
    if [[ "${BACKUP_INIT_IF_MISSING:-0}" == "1" ]]; then
      ...
      log "ERROR:     sudo ./scripts/restic.sh init   # reads RESTIC_PASSWORD from $ENV_FILE, never argv (B1707)"
