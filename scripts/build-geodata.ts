...
type City = {
  ...
  country: string;
  population: number;
  /** GeoNames feature code `PPLX` — a section of a populated place (a city
   * district or borough), not a town in its own right. Reverse geocoding
   * still matches these; `lib/ingest/geo.ts`'s `placesInBox`, the route
   * map's own query, is the one reader that skips them (coordinator review
   * 2026-09-24 — "Budapest III. kerület" and "Kelenföld" drawn as towns on
   * a map of the city they are districts of). */
  isSection: boolean;
};
...
function pack(cities: City[], countryNames: Map<string, string>): Buffer {
  ...
  cities.forEach((city, i) => {
    ...
    records.writeUInt8(city.isSection ? 1 : 0, at + 11);
    records.writeUInt8(name.length, at + 12);
    records.writeUInt32BE(nameOffset, at + 13);
