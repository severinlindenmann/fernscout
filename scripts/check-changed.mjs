...
const KEEPERS = [
  {
    name: "agent instruction routing",
    matches: (file) => file === "AGENTS.md" || file.startsWith("docs/agents/"),
    tests: ["test/agent-instructions.test.ts"],
  },
  {
    name: "task ids and task commands",
    matches: (file) => file.startsWith("docs/tasks/") || file === "scripts/tasks.mjs",
    tests: [
      "test/task-ids.test.ts",
      "test/tasks-discovery.test.ts",
      "test/tasks-holds.test.ts",
      "test/tasks-script.test.ts",
    ],
  },
  {
  ...
  },
  {
    ...
    tests: ["test/capabilities.test.ts", "test/server-only-capabilities.test.ts"],
  },
];
