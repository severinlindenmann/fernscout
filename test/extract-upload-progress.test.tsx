...
afterEach(() => {
  vi.unstubAllGlobals();
  // Belt and suspenders alongside the ETA test's own `vi.useRealTimers()` —
  // a real timer here even if that test throws before reaching its own
  // call, so a fake `Date` never leaks into a test that runs after it.
  vi.useRealTimers();
  act(() => root?.unmount());
  container?.remove();
  root = undefined;
  ...
  act(() => {
    root!.render(
      <LocaleProvider locale="en" dictionary={dictionaryFor("en")}>
        <UploadStep username="agent" runId="run-1" onDone={() => {}} speech />
      </LocaleProvider>,
    );
  });
...
describe("a successful attempt", () => {
  test("shows the real done count and never invents an iCloud chip or a per-tile percentage", async () => {
    ...
    // slow (`studio.photos.upload.icloud`, shown before any upload starts) is a
...
describe("the ETA denominator after a batch fails outright", () => {
  test("B1803 fix round 1 — a failed batch's bytes are dropped from the total, not left inflating what's left", async () => {
    ...
    // B1887 — this used to bet on wall-clock timing twice: a real 5ms delay
    // so `etaMinutes`'s `elapsedMs` was "unambiguously nonzero", and a real
    // 50ms wait so batches 1 and 2 had "settled" by the time the chip was
    // read. Both bets lose on a loaded machine — seen twice in one day,
    // against the identical assertion, on a machine also running the full
    // suite. `Date` is faked instead: `etaMinutes`'s elapsed time comes from
    // two explicit `vi.setSystemTime` calls rather than from how much real
    // time happened to pass, and the two batches are awaited by settling
    // their own controlled promises rather than by outwaiting a duration —
    // `flush()` below drains the microtask queue, which needs no timer at
    // all. The shared `afterEach` above restores real timers, since nothing
    // else in this file fakes a clock.
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () => {
        ...
        if (call === 1) {
          await firstBatch;
          // The moment batch 1 is done failing is also the moment the test
          // controls to make `elapsedMs` nonzero — one explicit jump, not a
          // race against however long the real await above happened to take.
          vi.setSystemTime(new Date(startedAt.getTime() + 5));
          throw new Error("network down");
        }
    ...
    // Drains the microtask queue a fixed, generous number of times — state,
    // not time: every hop between here and batch 3's `await thirdBatch`
    // (reject, catch, the next `fetch`, its `.json()`, two `setState`s) is a
    // promise `.then`, and each `Promise.resolve()` await flushes exactly
    // one of those. Awaiting a duration would still be a duration; this
    // waits for as many microtasks as this attempt could possibly need and
    // no more, deterministically, the same act on every run regardless of
