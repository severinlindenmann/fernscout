...
describe("media stays inside its owner", () => {
  ...
  /**
   * B1878. The segment checks above refuse `..`; a symlink needs none — the
   * path stays inside the trip's media directory and the filesystem walks out
   * of it. Nothing here creates one, but the content folder is edited by hand
   * and synced between machines, and sync tools do.
   */
  test("refuses a symlink that points out of the media directory", () => {
    const outside = path.join(dir, "bea", "config.json");
    const link = path.join(dir, "ana", "trips", "alps-2026", "media", "escape.jpg");
    fs.symlinkSync(outside, link);
    expect(resolveMediaFile("ana", ["alps-2026", "escape.jpg"])).toBeNull();

    // A link to a directory is no better, and neither is a file under it.
    const dirLink = path.join(dir, "ana", "trips", "alps-2026", "media", "out");
    fs.symlinkSync(path.join(dir, "bea"), dirLink);
    expect(resolveMediaFile("ana", ["alps-2026", "out", "config.json"])).toBeNull();

    // And a real file beside them still resolves.
    expect(resolveMediaFile("ana", ["alps-2026", "photo.jpg"])).not.toBeNull();
  });

  /**
   * The case-folded walk (`foldedWalk`, lib/media.ts) exists for volumes whose
   * names differ in case from what a day document spells, and the realpath
   * check above sits behind it rather than in front of it.
   */
  test("a differently-cased name still resolves", () => {
    // Asserted case-insensitively: on a case-insensitive volume the exact
    // stat succeeds and the requested spelling comes back, on a sensitive one
    // the walk finds `photo.jpg`. Either is a resolved file; null is not.
    expect(resolveMediaFile("ana", ["alps-2026", "PHOTO.JPG"])?.toLowerCase()).toContain(
      path.join("alps-2026", "media", "photo.jpg"),
    );
  });
