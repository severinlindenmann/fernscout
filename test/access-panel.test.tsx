...
/**
 * B74's own rule — a control is shown only when it can work, and absent
 * rather than broken when it cannot — used to be tested here against the
 * owner block's own guest-list link. B2017 moved the whole owner block
 * (journal settings, the agent card, the people card, export and delete) off
 * `/me` and into the studio, so what holds here now is only that none of it
 * comes back: `test/studio-hub.test.tsx` covers the Journal group's own
 * contacts-off/on behaviour on its new page.
 */
describe("the access panel, for the owner", () => {
  ...
  // B2157 — the B2017 handover note and its own studio link left `/me` too;
  // the header's Studio tab is where the owner goes now.
  test("and no longer carries the handover note either", () => {
    const html = render({ viewer: owner });
    expect(html).not.toContain("now live in your studio");
  });
});
 ...
 *
 * This page used to draw a "who reads along" card pointing an owner at
 * `/{user}/contacts` (B79, then B282, after `components/InviteLinks.tsx` was
 * retired in favour of that page's own list). B2017 moved the card itself
 * into the studio hub's Journal group, alongside the rest of the owner
 * block — see `journalGroupItems` in `components/studio/StudioHub.tsx` and
 * `test/studio-hub.test.tsx`. What holds here now is only that nobody, owner
 * included, sees it on `/me` any more.
 */
describe("the way to the people, no longer offered on /me — B2017", () => {
  test("nobody sees it here, owner included", () => {
    const others: Viewer[] = [
      stranger,
      owner,
      ownerWithTrips,
...
describe("what somebody on a trip is told they can write", () => {
   ...
   * them the mail-a-code flow is two ways to do one job for a reader with no
   * basis for choosing — which is what B301 removed from the owner block,
   * and it must not come back through this door. The owner's own button
   * moved to `/studio/agent` by B2017 (`test/studio-hub.test.tsx` covers it);
   * what is asserted here is only that the buddy flow stays off.
   */
  test("an owner who was on the trip gets no second flow", () => {
    const html = render({
 ...
 * about conversations they never had. Signed out and guest both get nothing;
 * a buddy (through "traveller") keeps the block here. B2017 moved the
 * owner's own copy to `/studio/agent` (`test/studio-hub.test.tsx`), so the
 * owner is now the fourth case that gets nothing on `/me` either — the same
 * "no owner administration here any more" rule the rest of this file tests.
 */
describe("the sessions-consent block — B1385", () => {
  ...
  test("absent for the owner — it moved to /studio/agent", () => {
...
describe("the foot-of-page link for somebody with no journal — B1386", () => {
  test("points at signup when signup is open", () => {
