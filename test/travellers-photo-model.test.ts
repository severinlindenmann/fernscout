...
describe("the system prompt", () => {
  test("treats a child exactly like every other figure, and never more precisely", async () => {
    ...
    // that builds it is not exported — the same discipline
    // `test/describe-image.test.ts` uses for `describeImage`. Asserting on
    // `create`'s own call captures the same string either way.
    const { classifyTravellers } = await import("@/lib/helper/model");
