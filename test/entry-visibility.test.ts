...
describe("the surfaces that walk every entry", () => {
   ...
   * WhatsApp announcement (`send-whatsapp`, `paid/whatsapp/lib/digest/dayWhatsapp.ts`,
...
describe("the photographs of a held-back update", () => {
  ...
  /**
   * The file no gallery mentions — and since B1884 it is nobody's, whichever
   * folder it is sitting in. It used to follow the update the folder was
   * named for, which is right for a held-back update and wrong for a
   * published one: a lookup that missed (every v2 upload's folder, spelled
   * with the day's whole id) answered "anybody", so the fallback served
   * exactly the files it existed to hold back. An original in a published
   * day's folder is now as unserved as one in a held-back day's; the people
   * the trip is for still have both.
   */
  test("a file the gallery never mentions is served to nobody but the travellers", async () => {
    as("anonymous");
    expect((await fetchPhoto("open-2026", "arrival-guest-note", "loose.jpg")).status).toBe(404);
    expect((await fetchPhoto("open-2026", "arrival", "loose.jpg")).status).toBe(404);
