...
/**
 * B2179 round 2 — `pixelCountProblem` is the refusal for a header sharp
 * would not even read (`IMAGE_MAX_PIXELS`, a separate, hard ceiling from
 * `IMAGE_MAX_EDGE`). It used to be simulated by inventing a longest edge of
 * `limits.imageEdge + 1`, which reported a false width and, for an image
 * whose edge is actually within `imageEdge`, the wrong problem on the wrong
 * axis entirely (a 9000×9000 image is 81 MP but its edge is under 12000).
 */
describe("pixelCountProblem", () => {
  test("known dimensions: the real width, height and megapixel count", () => {
    const problem = pixelCountProblem("wall.png", { width: 9000, height: 9000 });
    expect(problem).toEqual({
      field: "wall.png.pixels",
      got: "9000×9000 (81 MP)",
      expected: `at most ${IMAGE_MAX_PIXELS / 1_000_000} MP`,
    });
  });

  test("unknown dimensions — a header sharp's own default ceiling also refused — says 'more than', not an invented number", () => {
    const problem = pixelCountProblem("bomb.png", undefined);
    expect(problem).toEqual({
      field: "bomb.png.pixels",
      got: `more than ${IMAGE_MAX_PIXELS / 1_000_000} MP`,
      expected: `at most ${IMAGE_MAX_PIXELS / 1_000_000} MP`,
    });
  });
});
