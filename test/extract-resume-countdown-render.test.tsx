...
// B1941 reworded the sentence these read: the clock never said what it was
// counting down to. The guard is unchanged — which parts of the duration
// appear, and that a zero part never dangles. Only the words after it moved.
describe("the resume screen's rendered countdown", () => {
  test("3 to 24 hours out reads as hours only, never '0 days'", () => {
    ...
    expect(text).toMatch(/3 hours until the photographs waiting here are deleted/);
  });
  ...
  test("a day and change out reads as both days and hours", () => {
    ...
    const text = render({ ...RUN_BASE, expiresAt });
    expect(text).toMatch(/1 day and 2 hours until the photographs waiting here are deleted/);
  });
  ...
  test("exactly one day out reads as a day, with no dangling '0 hours'", () => {
    ...
    const text = render({ ...RUN_BASE, expiresAt });
    expect(text).toMatch(/1 day until the photographs waiting here are deleted/);
