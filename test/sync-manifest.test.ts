...
describe("the manifest", () => {
  ...
  /**
   * A trip's sidecars moved out of `media/` and into their own `meta/`
   * (B1863), and a photograph now keeps that one file for life (B1864). The
   * `trips/` rule already carries them; this is here so nobody has to
  ...
  test("a trip's meta/ sidecars sync — they are the photographs' own facts", async () => {
    ...
    expect(inSync(`trips/${TRIP}/meta/over-the-susten/01.jpg.meta.json`)).toBe(true);

    const { clearSyncHashCache } = await import("@/lib/sync/manifest");
    clearSyncHashCache();
    const { body } = await manifest(await tokenFor(OWNER_EMAIL));
    expect(body.files!.map((f) => f.path)).toContain(
      `trips/${TRIP}/meta/over-the-susten/01.jpg.meta.json`,
    );
  });
