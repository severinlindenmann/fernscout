...
describe("dictionaries", () => {
  ...
  /**
   * B1894 — renamed from "every maintained locale covers every key English
   * has", which promised more than it checked in two separate ways.
   *
   * **It could never fail, for any locale, at all.** `dictionaryFor` merges
   * the shipped file *onto* English (`{ ...english, ...readDictionary(code) }`
   * — a deliberate fallback, so a locale that ships no chrome for a key still
   * shows something rather than the key name). Checking that merged object
   * for missing keys is checking whether English covers English: it always
   * does. `hu.json` was 336 keys behind the moment this went looking for a
   * gap and found none, because there was never a way for it to find one.
   *
   * **Its name claimed every maintained locale, and `installedLocales()` —
   ...
   * claim as the two rendering the same values. `contact.mailApprovedBuddyBody`
   * and `me.askOwnerNamed` both had this: the key was present everywhere,
   * `i18n:keys`' coverage report saw nothing wrong, and a `{title}`/`{name}`
   * that English uses twice was there only once in Hungarian (and, it turned
   * out once this test went looking, in German too) — grammatical, and quietly
   * missing a value for every reader of that language.
   *
   * This is a *count* comparison, not a set one, on purpose: a set check sees
   * `{title}` in both strings and calls it equal, which is exactly the bug it
   * exists to catch. It has to know each token's multiplicity.
   *
   * The one legitimate divergence is a plural-variant key (`.zero`, `.one`,
   * `.two`, `.few`, `.many`) picking up a placeholder English doesn't need —
   * English collapses `me.paymentBalance.one` to the literal "1 credit",
   * Hungarian keeps `{balance}` because a Hungarian numeral doesn't retire the
   * noun's normal form the way English's "one" does. So a plural-variant key
   * may ADD placeholders; nothing, anywhere, may DROP one English has.
   *
   * Runs over every shipped locale file (not just the parity set above) —
   * placeholder truthfulness does not wait for a locale to reach full key
   * coverage, and this only ever looks at keys the locale already has.
   */
  test("every locale keeps English's placeholder count for every key it has", () => {
    const localesDir = path.join(process.cwd(), "site", "locales");
    const english: Record<string, string> = JSON.parse(
      fs.readFileSync(path.join(localesDir, "en.json"), "utf8"),
    );
    ...
    const isPluralVariant = (key: string) => /\.(zero|one|two|few|many)$/.test(key);
    ...
    for (const file of shipped) {
      ...
      for (const key of Object.keys(english)) {
        ...
        const englishTokens = tokenCounts(english[key]);
        const localeTokens = tokenCounts(locale[key]);
        const names = new Set([...englishTokens.keys(), ...localeTokens.keys()]);
        for (const name of names) {
          const englishCount = englishTokens.get(name) ?? 0;
          const localeCount = localeTokens.get(name) ?? 0;
          if (localeCount < englishCount) {
            problems.push(
              `${code}.json "${key}": {${name}} appears ${englishCount}x in English, ${localeCount}x here`,
            );
          } else if (localeCount > englishCount && !isPluralVariant(key)) {
            problems.push(
              `${code}.json "${key}": {${name}} appears ${localeCount}x here, only ${englishCount}x in English`,
            );
          }
        }
      }
...
describe("Hungarian address is consistently informal (B481)", () => {
  ...
  /**
   * The harm the old "Hungarian is not invented" rule was really guarding
   * against — B1953, and conformance item L2.
   *
   * AGENTS.md now asks for the translation rather than the gap, so a missing
   * key is no longer the failure. A key whose Hungarian value IS its English
   * value still is: it falls back silently either way, but a pasted one
   * claims to have been translated, and nothing downstream will ever question
   * it. A genuinely missing key is visible in `npm run i18n:keys`; this is
   * not.
   *
   * `wa.onb.card` deliberately carries every language at once — it is the
   * card that asks somebody to pick one — so it reads identically in all
   * three files by design.
   */
  test("no Hungarian entry is its English original pasted across", () => {
    const en = dictionaryFor("en");
    const hu = JSON.parse(
      fs.readFileSync(path.join(process.cwd(), "site", "locales", "hu.json"), "utf8"),
    ) as Record<string, string>;
    const MULTILINGUAL = new Set(["wa.onb.card"]);
    const pasted = Object.keys(hu).filter(
      (key) =>
        !MULTILINGUAL.has(key) &&
    ...
    expect(
      pasted,
      `English sitting in hu.json, claiming to be a translation:\n${pasted.join("\n")}`,
    ).toEqual([]);
  });
});
