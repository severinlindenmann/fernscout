...
describe("with FERNSCOUT_ADMIN_EMAIL set, the address owns every journal", () => {
  ...
  // B1166: `request` used to answer 202 here and mail a code that `redeem`
  // always refused with `invalid_code` — a code for the admin, on a journal
  // it does not own by `config.json`, that nobody could ever spend. `request`
  // and `redeem` now agree: both refuse it, the same way they refuse a
  // stranger. The admin's real path is its own journal's token, used
  // cross-journal — the next test.
  test("request and redeem agree: a write code for a journal it does not own is refused by both", async () => {
    const asked = await requestCode(OWNER, ADMIN);
    expect(asked.status).toBe(403);
    expect(asked.body.error).toBe("not_authorised");

    // And if a code existed anyway — minted directly, bypassing `request` —
    // `redeem` refuses it too, on the same address and journal `request` just
    // refused. The two routes are asked the same question and must not
    // disagree.
    const { issueCode } = await import("@/lib/auth");
    await issueCode(OWNER, ADMIN, "agent");
    const { POST: redeem } = await import("@/app/api/auth/codes/redeem/route");
    const response = await redeem(
      new Request("https://example.test/api/auth/codes/redeem", {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({ user: OWNER, email: ADMIN, code: CODE, for: "write" }),
      }),
    );
    expect(response.status).toBe(401);
  });

  test("asking for that unredeemable code does not revoke the owner's own live code", async () => {
    const { issueCode, verifyCode } = await import("@/lib/auth");
    const owner = await issueCode(OWNER, OWNER_EMAIL, "agent");

    // The admin asks anyway — refused, and mints nothing.
    const asked = await requestCode(OWNER, ADMIN);
    expect(asked.status).toBe(403);

    // The owner's own code, issued before the admin's refused request, is
    // still live.
    const session = await verifyCode(OWNER, OWNER_EMAIL, owner.code, "agent");
