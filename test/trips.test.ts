...
// B1947 — `beta-2026` (2026-08-14 to 2027-01-31) is only "current" while
// today falls inside that span, and `gamma-2027` only reads as "upcoming"
// while today is before it starts. Both are shared fixtures on disk, not
// written per test, so pin the clock the way B1946 did rather than let the
// suite's idea of "current" drift with the calendar.
const DURING_BETA = new Date("2026-09-06T12:00:00Z");

beforeEach(() => {
  process.env.CONTENT_DIR = FIXTURES;
  vi.useFakeTimers({ toFake: ["Date"] });
  vi.setSystemTime(DURING_BETA);
});
