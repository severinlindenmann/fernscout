 ...
 * this screen is a count over the manifest `GET .../studio/run` already
  ...
  await act(async () => {
    root!.render(
      <LocaleProvider locale="en" dictionary={dictionaryFor("en")}>
        <FoundStep username="alex" runId="run-1" onContinue={() => {}} speech />
      </LocaleProvider>,
    );
  });
  await act(async () => {
...
describe("the import's step strip counts only the screens shown — B2081", () => {
  test("without speech the talk-or-type screen is not counted, so upload is 2 of 4, not 3 of 5", async () => {
    const { importStep } = await import("@/components/extract/StepIndicator");
    expect(importStep("trip", false)).toEqual({ current: 1, total: 4 });
    expect(importStep("upload", false)).toEqual({ current: 2, total: 4 });
    expect(importStep("found", false)).toEqual({ current: 3, total: 4 });
    expect(importStep("upload", true)).toEqual({ current: 3, total: 5 });
  });
});
