...
function fakeCaches(entries: Cached[]) {
  ...
  const cache = {
    ...
    match: async (key: { url: string } | string, options?: { ignoreSearch?: boolean }) => {
      const url = typeof key === "string" ? new URL(key, "https://journal.test").href : key.url;
  ...
  return {
    ...
    match: async (request: { url: string } | string, options?: { ignoreSearch?: boolean }) => {
      ...
      // The real `caches.match` searches every cache on the origin, kept
      // ones included — which is why the worker no longer calls it (B2158).
      const hit = find(url, options?.ignoreSearch)?.response.clone();
      if (hit) return hit;
...
function loadWorkerWithCaches(
  network: (request: { url: string }, init?: never) => Promise<Response>,
  ...
  const scope = {
    self: null as unknown,
    caches: store,
    fetch: async (request: { url: string }, init?: unknown) =>
      (network as (r: { url: string }, i?: unknown) => Promise<Response>)(request, init),
    setTimeout,
    clearTimeout,
    Response,
    URL,
...
/**
 * A trip kept for a plane — B2158, `docs/plans/W43-a-trip-on-a-plane.md`.
 *
 * The page sends `fernscout-keep`; the worker fetches the manifest and then
 * everything it lists into one cache named after the reader and the trip,
 * and from then on `caches.match` finds a kept photograph, day page or story
 * window when the network is gone. Signing out takes a signed-in reader's
 * kept trips with it, and a version bump leaves them alone.
 */
describe("a trip kept for reading with no signal", () => {
  ...
  const network = (manifestBytes = manifest.bytes) => async (request: { url: string } | string) => {
    const url = typeof request === "string" ? request : request.url;
    if (url.endsWith("/keep.json")) {
      return new Response(JSON.stringify({ ...manifest, bytes: manifestBytes }), {
        headers: { "content-type": "application/json" },
      });
    }
    if (url === HOME) return homePayload("aaaa1111");
    if (url.includes("/day/one")) {
      return new Response('<html><script src="/_next/static/chunks/day.js"></script></html>', {
        headers: { "content-type": "text/html" },
      });
    }
    return new Response(`body of ${url}`, { headers: { "content-type": "image/jpeg" } });
  };

  async function keep(handlers: Handlers, user = "alex", trip = "alps") {
    const said: Record<string, unknown>[] = [];
    const pending: Promise<unknown>[] = [];
    handlers.message({
      data: { type: "fernscout-keep", user, trip },
      source: { postMessage: (m: Record<string, unknown>) => said.push(m) },
      waitUntil: (p: Promise<unknown>) => pending.push(p),
    });
    await Promise.all(pending);
    return said;
  ...
  test("keeps everything the manifest lists, plus the scripts a kept page needs", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network());
    const said = await keep(handlers);

    expect(said.at(-1)).toMatchObject({ state: "kept", total: 4, failed: 0, build: "abc" });
    const kept = caches.named.get("kept-public-alex-alps");
    ...
    // The scratch cache the copy was built in is gone.
    expect([...caches.named.keys()]).not.toContain("kept-public-alex-alps-building");
  });

  test("a kept photograph is served when the network is gone", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network());
    await keep(handlers);
    // The worker keys by the relative address it fetched; a page asks by
    // absolute URL. Store it as the browser would.
    const kept = caches.named.get("kept-public-alex-alps")!;
    for (const [key, res] of [...kept.entries()]) kept.set(new URL(key, "https://journal.test").href, res);
   ...
   */
  test("fetches a photograph the way an <img> would, so the kept copy is the served one", async () => {
    const accepts: Record<string, string | undefined> = {};
    const { handlers } = loadWorkerWithCaches(async (request, init?: { headers?: Record<string, string> }) => {
      const url = typeof request === "string" ? request : request.url;
      accepts[url] = init?.headers?.Accept;
  ...
  test("online, the server answers before a kept copy does", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network());
    await keep(handlers);
    const kept = caches.named.get("kept-public-alex-alps")!;
    for (const [key, res] of [...kept.entries()]) kept.set(new URL(key, "https://journal.test").href, res);

    const online = loadWorkerWithCaches(
  ...
  test("refuses a trip above the ceiling before fetching anything", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network(600 * 1024 * 1024));
    const said = await keep(handlers);
    expect(said).toEqual([
      expect.objectContaining({ state: "refused", bytes: 600 * 1024 * 1024 }),
    ]);
    expect([...caches.named.keys()].filter((k) => k.startsWith("kept-"))).toEqual([]);
  });

  test("is named after the signed-in reader, and signing out takes it away", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network());
    await run(handlers, HOME);
    await keep(handlers);
    expect([...caches.named.keys()]).toContain("kept-aaaa1111-alex-alps");
  ...
  test("a version bump leaves a kept trip alone, and unkeep removes it", async () => {
    const { handlers, caches } = loadWorkerWithCaches(network());
    await keep(handlers);
    ...
    handlers.activate({ waitUntil: (p: Promise<unknown>) => pending.push(p) });
    await Promise.all(pending);
    expect([...caches.named.keys()]).toContain("kept-public-alex-alps");

    pending = [];
    const said: Record<string, unknown>[] = [];
    handlers.message({
      data: { type: "fernscout-unkeep", user: "alex", trip: "alps" },
      source: { postMessage: (m: Record<string, unknown>) => said.push(m) },
      waitUntil: (p: Promise<unknown>) => pending.push(p),
    });
    await Promise.all(pending);
