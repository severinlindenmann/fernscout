...
/**
 * `DELETE .../extract/run` — B1805. The resume screen's "let this run go"
...
function del(user: string, runId: string): Promise<Response> {
  return import("@/app/api/helper/[user]/extract/run/route").then(({ DELETE }) =>
    DELETE(new Request(`http://x/api/helper/${user}/extract/run?run=${runId}`, { method: "DELETE" }), {
