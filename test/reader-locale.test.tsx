...
/**
 * The places a journal's name is not the first thing in the path — B921.
 *
 * A 71-year-old whose daughter set up her iPhone has English menus and a
 * German journal, and met the whole conversation in English. The chat room
 * (`/agent/mila/chat`) is gone since B2173, but `POST /api/helper/mila/ask`
 * still begins with `api`, so **every
...
describe("a journal whose name is not the first segment", () => {
  ...
  test("the journal's set is what the device is narrowed against, not the instance's", () => {
    ...
    expect(readerLocaleForPath("/api/v1/mila/trips", undefined, "hu-HU,hu;q=0.9")).toBe("de");
  ...
  test("the retired room's /agent/<user> no longer names a journal — B2173", () => {
    withGermanOnlyJournal();
    expect(readerLocaleForPath("/agent/oma/chat", undefined, "en-GB")).toBe("en");
  });

  test("her own choice still wins over the journal's default", () => {
    instance();
    expect(readerLocaleForPath("/api/helper/mila/ask", "en", "de-DE")).toBe("en");
  });
});
