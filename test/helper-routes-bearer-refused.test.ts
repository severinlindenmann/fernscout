...
describe("the helper's own routes never accept a bearer token", () => {
  ...
  test("day/place: GET — B2200, the one door onto `gps/`", async () => {
    const { GET } = await import("@/app/api/helper/[user]/day/place/route");
    const res = await GET(bearerOnly("https://t.test/api/helper/alex/day/place?trip=x&date=2026-06-22"), params);
    expect(res.status).toBe(404);
  });
  ...
  test("gps/line: GET — B2226, the owner's raw route preview", async () => {
    const { GET } = await import("@/app/api/helper/[user]/gps/line/route");
    const res = await GET(bearerOnly("https://t.test/api/helper/alex/gps/line?trip=x"), params);
    expect(res.status).toBe(404);
  });

  test("gps/trip: DELETE — B2226, one trip's or one day's recording", async () => {
    const { DELETE } = await import("@/app/api/helper/[user]/gps/trip/route");
    const res = await DELETE(
      bearerOnly("https://t.test/api/helper/alex/gps/trip?trip=x", { method: "DELETE" }),
      params,
    );
    expect(res.status).toBe(404);
  });
