...
const LIB_API_ALLOWLIST = [
  ...
  "lib/api/fetchMedia",
  // B2243. `readJsonBody` / `readBoundedJson` — one stream-counted JSON body
  // reader with a size ceiling, shared by the v2 routes and the helper
  // routes so every JSON door holds the same limit. It reads bytes and
  // returns a value; it shapes no response of its own.
  "lib/api/jsonBody",
