 ...
 *
 * (both strings have since changed — B1949 — but the symptom this test
 * guards against, an untranslated literal served under a foreign `lang`,
 * does not care what the words are)
 *
...
import { generateMetadata as welcomeMetadata } from "@/app/welcome/page";
 ...
 * journal. `/welcome` joined them in B2170, when it stopped being a
 * `redirect("/")` and became the signup page.
 *
...
describe("every page outside a journal", () => {
  test("has a title in the reader's language, not an English literal", async () => {
    ...
    const welcome = (await welcomeMetadata()).title as { absolute: string };
    expect(welcome.absolute).toBe(de["signupPage.metaTitle"].replace("{name}", SITE_NAME));
  });
});
