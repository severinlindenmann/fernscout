...
import { MAX_CREDITS, MIN_CREDITS, POSTCARD_CREDITS, priceRappen } from "@paid/credits/lib/credits/pricing";
...
describe("the payment section", () => {
  ...
  test("the AI sentence appears only when an AI row is there to explain — B2090", () => {
    const html = render({ payment: { ...payment, spent: [{ reason: "storage", credits: 50 }] } });
    expect(html).toContain(dictionaryFor("en")["me.spentTitle"]);
    expect(html).not.toContain(dictionaryFor("en")["me.spentAiNote"]);
  });

  test("a test-mode card key is said once under the buy button — B2090", () => {
    expect(render({ payment })).not.toContain(dictionaryFor("en")["me.buyTestMode"]);
    expect(render({ payment: { ...payment, cardTestMode: true } })).toContain(
      dictionaryFor("en")["me.buyTestMode"].replace("'", "&#x27;"),
    );
  });
