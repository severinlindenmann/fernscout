...
/**
 * `PATCH .../extract/party` — S8a's "Save who came", B1803 Task 3.6.
 *
...
function patch(body: unknown) {
  return import("@/app/api/helper/[user]/extract/party/route").then(({ PATCH }) =>
    PATCH(
      new Request("http://x/api/helper/alex/extract/party", {
        method: "PATCH",
...
describe("PATCH .../extract/party", () => {
  ...
  test("never writes anything to trip.json's people — this fact stays on the run alone", async () => {
    writeManifest("alex", baseManifest());
    await patch({ run: "run-1", size: 2, names: ["Severin", "Nora"] });
