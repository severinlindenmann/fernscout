...
/**
 * `describeImage` mocked for the sample and enrich routes — Ruling R5. What
 ...
 *
 * Since B1866 this is **one call per photograph**, not one per batch, and the
 * answer is a whole `DescribedForm` kept beside the staged file — so a
 * "called once" about a batch of N here is a call count of N, and a
 * photograph already described is not sent or charged for a second time.
 */
const described = (locales: readonly string[]) => ({
  caption: Object.fromEntries(locales.map((code) => [code, "A quiet street."])),
  altText: Object.fromEntries(locales.map((code) => [code, "A street."])),
  longDescription: Object.fromEntries(locales.map((code) => [code, null])),
  tags: ["cityscape"],
...
vi.mock("@/lib/helper/model", async (importOriginal) => {
  ...
  return { ...actual, describeImage: helperModel.describeImage };
});
...
describe("the extract routes with the capability off", () => {
  test("start answers 404, not 500 and not 403", async () => {
    const { POST } = await import("@/app/api/helper/[user]/studio/start/route");
    const res = await POST(new Request("http://x/api/helper/alex/studio/start", { method: "POST" }), {
  ...
  test("upload answers 404 too", async () => {
    const { POST } = await import("@/app/api/helper/[user]/studio/upload/route");
    const res = await POST(new Request("http://x/api/helper/alex/studio/upload", { method: "POST" }), {
      params: Promise.resolve({ user: "alex" }),
    });
    expect(res.status).toBe(404);
  });
...
function startRunFor(user: string): Promise<Response> {
  return import("@/app/api/helper/[user]/studio/start/route").then(({ POST }) =>
    POST(new Request(`http://x/api/helper/${user}/studio/start`, { method: "POST" }), {
...
function startRunWith(body: Record<string, unknown>): Promise<Response> {
  return import("@/app/api/helper/[user]/studio/start/route").then(({ POST }) =>
    POST(
      new Request("http://x/api/helper/alex/studio/start", {
        method: "POST",
...
function upload(runId: string, files: File[]): Promise<Response> {
  ...
  return import("@/app/api/helper/[user]/studio/upload/route").then(({ POST }) =>
    POST(
      new Request("http://x/api/helper/alex/studio/upload", { method: "POST", body: form }),
      { params: Promise.resolve({ user: "alex" }) },
    ),
  );
}
...
describe("the extract routes with the capability on", () => {
  ...
  test("B2205 — a picture whose header declares 15000×15000 is refused before it is staged", async () => {
    const { runId } = (await (await startRun()).json()) as { runId: string };
    const { craftedPng } = await import("./support/pictures");
    const bomb = new File([new Uint8Array(craftedPng(15000, 15000))], "bomb.png", { type: "image/png" });
    const res = await upload(runId, [bomb]);
    const body = (await res.json()) as { accepted: unknown[]; rejected: { filename: string; reason: string }[] };
    expect(body.accepted).toEqual([]);
    expect(body.rejected).toEqual([{ filename: "bomb.png", reason: "too_many_pixels" }]);
    const { readManifest } = await import("@/lib/staging/manifest");
    expect(readManifest("alex", runId)?.photos).toEqual([]);
    const { journalStagingBytes } = await import("@/lib/staging/store");
    expect(journalStagingBytes("alex")).toBe(0);
  });
...
describe("the run and day routes with the capability on", () => {
  ...
  test("patching a photograph refuses a visibility word that is not one of the two", async () => {
    const { PATCH } = await import("@/app/api/helper/[user]/studio/run/route");
  ...
  test("GET groups the run's photographs into days and asks about each", async () => {
    ...
    const { GET } = await import("@/app/api/helper/[user]/studio/run/route");
  ...
  test("POST .../day appends the answer to the manifest, not to inbox/days", async () => {
    const { runId } = (await (await startRun()).json()) as { runId: string };
    const { POST } = await import("@/app/api/helper/[user]/studio/day/route");
...
describe("the sample and enrich routes — B1751 Task 4.1", () => {
  ...
  beforeEach(async () => {
    ...
    helperModel.describeImage.mockReset();
    helperModel.describeImage.mockImplementation(
      async (_image: unknown, _owner: string, locales: string[]) => described(locales),
    );
  });
  ...
  async function stagedManifest(runId: string, filenames: string[]) {
    ...
    const manifest = readManifest("alex", runId);
    if (!manifest) throw new Error("run vanished");
    // A distinct copy per name. `putStagedFile` is content-addressed, so the
    // identical bytes under two names were one staged file with one id all
    // along — harmless while nothing was kept about a file, and misleading
    // now that a description is (B1866): two rows pointing at one file are
    // one photograph and are described once, which is correct but is not
    // what these tests mean by "two photographs". The trailing bytes are
    // after the JPEG's end marker, so every one of them still decodes.
    const staged = filenames.map((name) =>
      putStagedFile("alex", runId, name, Buffer.concat([CAMERA_JPEG, Buffer.from(name)])),
    );
  ...
  test("spending credits captions every live photograph, at the ref the expiry warning reads", async () => {
    ...
    const { POST } = await import("@/app/api/helper/[user]/studio/enrich/route");
    const res = await POST(
  ...
  test("R30 — resuming a run and adding photographs charges again for the new ones, not free and not a double refusal", async () => {
    ...
    // ever changing it.
    helperModel.describeImage.mockClear();
  ...
  test("a photograph the free sample already described is not sent or paid for again — B1866", async () => {
    ...
    const { POST: sample } = await import("@/app/api/helper/[user]/studio/sample/route");
    const tasted = await sample(
      new Request("http://x", { method: "POST", body: JSON.stringify({ run: runId, photoId: sampled.id }) }),
      { params: Promise.resolve({ user: "alex" }) },
    );
    expect(tasted.status).toBe(200);
    expect(helperModel.describeImage).toHaveBeenCalledTimes(1);
  ...
  test("every photograph failing to resize is given back too — B1795", async () => {
    ...
    // the same as a corrupt original, so the model is never asked anything.
    const staged = putStagedFile("alex", runId, "broken.jpg", Buffer.from("not really a jpeg"));
    const manifest = readManifest("alex", runId);
    if (!manifest) throw new Error("run vanished");
    manifest.photos.push({ id: staged.id, filename: staged.filename, bytes: staged.bytes, kind: "image" });
    writeManifest("alex", manifest);
...
describe("the runs route", () => {
  test("capability off answers 404, not 500 and not 403", async () => {
    const { GET } = await import("@/app/api/helper/[user]/studio/runs/route");
    const res = await GET(new Request("http://x/api/helper/alex/studio/runs"), {
      params: Promise.resolve({ user: "alex" }),
    });
    expect(res.status).toBe(404);
  });
  ...
  describe("with the capability on", () => {
    ...
    // an abandoned `POST .../studio/start` with no upload behind it stayed
    ...
    test("listing a warned, not-yet-extended run twice leaves its expiresAt unchanged both times", async () => {
      ...
      const { GET } = await import("@/app/api/helper/[user]/studio/runs/route");
      const call = () =>
        GET(new Request("http://x/api/helper/alex/studio/runs"), { params: Promise.resolve({ user: "alex" }) });
