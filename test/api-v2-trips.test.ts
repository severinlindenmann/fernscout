...
describe("PATCH /api/v2/{user}/trips/{trip} — cost bounds judge only what is sent (B2243 review F1)", () => {
  test("a stored trip over the bounds still takes a patch; a patch over them is refused", async () => {
    ...
    await putTrip(OWNER, "over-bounds", fullTrip("over-bounds"), token);
    const { readTripFile, writeTripFile } = await import("@/lib/api/v2/store");
    const stored = readTripFile(OWNER, "over-bounds")!;
    const items = Array.from({ length: COST_LINES_MAX + 1 }, () => ({ label: "y".repeat(COST_LABEL_MAX_CHARS + 1), amount: 1 }));
    const { costs: _declined, ...declined } = (stored.declined ?? {}) as Record<string, string>;
    writeTripFile(OWNER, "over-bounds", { ...stored, costs: { ...stored.costs, items }, declined } as typeof stored);

    const renamed = await patchTrip(OWNER, "over-bounds", { title: "Renamed" }, token);
    expect(renamed.status, JSON.stringify(renamed.body)).toBe(200);

    const sent = await patchTrip(OWNER, "over-bounds", { costs: { items } }, token);
    expect(sent.status).toBe(400);
    expect(JSON.stringify(sent.body.details)).toContain("costs.items");
  });
});
