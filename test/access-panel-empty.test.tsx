...
describe("an owner whose journal has no trips", () => {
  ...
  /** The line points at the studio, which the header carries on every page
   * — B2157 removed the B2017 handover note and its link from the bottom of
   * `/me`, so the page must not still promise a studio link further down. */
  test("is pointed at the studio, not at a link further down the page", () => {
    const html = render(owner);
    expect(html).toContain(line("en", "me.ownerNoTrips"));
    expect(line("en", "me.ownerNoTrips")).not.toMatch(/further down/);
