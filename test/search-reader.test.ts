...
/**
 * B1964 — the studio was findable nowhere: `STUDIO_DESTINATION` lost its
 * last reader on 2026-09-20 and `lib/search.ts` never carried it, so typing
 * "studio" found nothing for the one page that lists everything a person
 * can do. Same discipline as the account page above: owner-only, so a reader
 * must not learn the studio exists by searching their own journal for it.
 */
...
describe("the studio's flows in the index — B2019", () => {
  ...
  test("the owner's search finds the trip-editing flow by 'rename' or 'delete', neither of which is in its own title", async () => {
    const json = await jsonFor("owner");
    expect(found(json, "rename")).toBe(true);
    expect(found(json, "delete")).toBe(true);
  });

  test("nobody but the owner finds a studio flow — not a fellow traveller, not an approved guest, not a stranger, not the anonymous index", async () => {
    for (const viewer of ["buddy", "guest", "stranger"]) {
      const json = await jsonFor(viewer);
      expect(json).not.toContain("/studio/");
    }
    const { buildSearchIndexJson } = await import("@/lib/search");
    expect(buildSearchIndexJson(OWNER)!).not.toContain("/studio/");
  });
});
 ...
 *
 * Same discipline as the account page above: `/studio/readers` (B2092, was
 * `/contacts`) is the owner's own
