...
/**
 * B1746 — `create_trip` refuses a title this journal already has, naming the
 * trip that exists, rather than proposing a second trip with the same name.
 *
 * `npm run helper:bench` caught the model doing this in roughly one run in
 * six against a journal that already had "Ungarn 2026": it had just read the
 * trip and proposed `create_trip` for a second "Ungarn 2026" anyway. A
 * prompt rule made it worse (B1744's own numbers), so this is the guard.
 */
function configureAlex(dir: string) {
...
describe("create_trip refuses a title this journal already has", () => {
  test("an exact title match refuses, naming the trip that exists", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fernscout-tools-"));
    const before = process.env.CONTENT_DIR;
    process.env.CONTENT_DIR = dir;
    try {
      configureAlex(dir);
      writeTripFixture("alex", { id: "ungarn-2026", title: "Ungarn 2026", start: "2026-05-01", end: "2026-05-10" });
      const ran = await runTool(
        "alex",
        "create_trip",
        { title: "Ungarn 2026", start: "2026-05-01", end: "2026-05-10" },
        say,
        "2026-09-07",
        [],
        "",
      ...
      expect(ran.blocks).toEqual([{ shape: "say", text: "agent.tool.createTripExists Ungarn 2026" }]);
      expect(fs.readdirSync(dir)).toEqual(["alex"]);
    } finally {
      if (before === undefined) delete process.env.CONTENT_DIR;
      else process.env.CONTENT_DIR = before;
      fs.rmSync(dir, { recursive: true, force: true });
  ...
  test("a title that only differs by case or punctuation still refuses", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fernscout-tools-"));
    const before = process.env.CONTENT_DIR;
    process.env.CONTENT_DIR = dir;
    try {
  ...
  test("a genuinely new title proposes exactly as it does today", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fernscout-tools-"));
    const before = process.env.CONTENT_DIR;
    process.env.CONTENT_DIR = dir;
    try {
      ...
      const ran = await runTool(
        "alex",
        "create_trip",
        { title: "Ungarn 2027", start: "2027-05-01", end: "2027-05-10" },
        say,
        "2026-09-07",
        [],
        "",
      ...
      expect(ran.proposal?.tool).toBe("create_trip");
      expect(ran.proposal?.arguments.title).toBe("Ungarn 2027");
    } finally {
      if (before === undefined) delete process.env.CONTENT_DIR;
      else process.env.CONTENT_DIR = before;
      fs.rmSync(dir, { recursive: true, force: true });
...
describe("asking about photographs points at the room's own controls", () => {
  ...
  test("add_photos says where the pane is, and touches nothing", async () => {
    const ran = await runTool("alex", "add_photos", {}, say, "2026-09-07", [], "", WEB_CALLER);
    expect(ran.ok).toBe(true);
...
describe("finding a day by what was in it, not by its date", () => {
  ...
  test("a sentence naming a thing resolves to a day, not a proposal", async () => {
    ...
    const ran = await runTool("alex", "find_day", { said: "the day with the photo of Anna" }, say, "2026-09-07", [], "", WEB_CALLER);
    expect(ran.ok).toBe(true);
    expect(ran.proposal).toBeUndefined();
  ...
  test("an id the catalogue never sent it is dropped, not resolved", async () => {
    ...
    const ran = await runTool("alex", "find_day", { said: "anna" }, say, "2026-09-07", [], "", WEB_CALLER);
  ...
  test("nothing found says so honestly, and does not fall through to a proposal", async () => {
    ...
    said.hits = [];
    const ran = await runTool("alex", "find_day", { said: "the day we got lost" }, say, "2026-09-07", [], "", WEB_CALLER);
    expect(ran.ok).toBe(true);
 ...
 * is one now (`paid/printOrder/lib/helper/tools/areas/printed.ts`): it writes a real order
...
describe("a proposal can only be pressed into the helper's own routes", () => {
  const ALLOWED = [
    ...
    "/api/helper/alex/contacts/add-me",
    "/api/helper/alex/contacts/import",
    "/api/helper/alex/invite-contact",
    "/api/helper/alex/assemble-day",
  ];
  ...
  test("publishing renders the day first, and then the press", async () => {
    ...
    try {
      ...
      const ran = await runTool("alex", "publish_day", { trip: "reise" }, say, "2026-09-07", [], "", WEB_CALLER);
...
describe("who may read a trip is read before it is chosen", () => {
  ...
  async function proposed(args: Record<string, string>) {
    const ran = await runTool("someone", "create_trip", args, say, "2026-09-07", [], "", WEB_CALLER);
  ...
  test("the card points at the label rather than paraphrasing it", async () => {
    const ran = await runTool("someone", "create_trip", { title: "Am See" }, say, "2026-09-07", [], "", WEB_CALLER);
...
describe("every string a tool says exists in every maintained locale", () => {
  ...
  const source = [root, ...paidCounterparts("lib/helper/tools")]
    .flatMap((dir) =>
      fs
