...
describe("storing an upload", () => {
  test("writes a resized derivative and keeps the original", async () => {
    const sent = await paintJpeg(4200, 2800);
    const result = await storeUploads(REF, "lanterns-of-hoi-an", [
      { filename: "DSC_4471.jpg", bytes: sent },
    ]);
    ...
    // C10 — "originals are untouched" as bytes, not only as metadata that
    // happens to match a re-encode. The served derivative is genuinely
    // different (it was resized); the print master on disk is the exact
    // buffer the upload sent, byte for byte.
    expect(Buffer.compare(fs.readFileSync(kept), sent)).toBe(0);
    expect(Buffer.compare(fs.readFileSync(served), sent)).not.toBe(0);
  });
