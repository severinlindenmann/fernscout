...
describe("a trip this journal does not have", () => {
  test("proposes nothing at all, rather than a day in the one it does have", async () => {
    const ran = await runTool("alex", "start_day", { trip: "Antarctica Expedition", date: "2026-03-04" }, say, "2026-09-08", [], "", WEB_CALLER);
  ...
  test("a read says which of the two it is, and never that there are no trips", async () => {
    const ran = await runTool("alex", "days", { trip: "Antarctica Expedition" }, say, "2026-09-08", [], "", WEB_CALLER);
...
describe("the trip somebody does mean", () => {
  ...
  test.each([
    ...
    const ran = await runTool("alex", "start_day", said === undefined ? {} : { trip: said }, say, "2026-09-08", [], "", WEB_CALLER);
...
describe("a name that fits more than one trip", () => {
  ...
  test("resolves to nothing rather than to the newer one", async () => {
    const ran = await runTool("alex", "start_day", { trip: "balkan" }, say, "2026-09-08", [], "", WEB_CALLER);
    expect(ran.proposal).toBeUndefined();
  });

  test("and the reason names both, so the person can say which", async () => {
    const ran = await runTool("alex", "days", { trip: "balkan" }, say, "2026-09-08", [], "", WEB_CALLER);
    const why = (ran.result as { why?: string }).why ?? "";
  ...
  test.each([
    ...
    const ran = await runTool("alex", "start_day", { trip: said }, say, "2026-09-08", [], "", WEB_CALLER);
    expect(ran.proposal?.arguments.trip).toBe(expected);
  });
});
