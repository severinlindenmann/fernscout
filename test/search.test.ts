...
// B1947 — `buildDocs` skips a trip whose *derived* status is "upcoming"
// (lib/search.ts:371), and `upcoming-2027` (2027-01-01 to 2027-01-10) is
// only upcoming while today is before it. Pin the clock so the fixture's
// tense stops depending on when the suite happens to run.
const BEFORE_UPCOMING_2027 = new Date("2026-09-06T12:00:00Z");

beforeEach(() => {
  process.env.CONTENT_DIR = FIXTURES;
  vi.useFakeTimers({ toFake: ["Date"] });
  vi.setSystemTime(BEFORE_UPCOMING_2027);
});
afterEach(() => {
