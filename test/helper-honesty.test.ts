...
/**
 * B1563 — live evidence, the operator's journal, 2026-09-12: asked to "beschreibe
...
/**
 * A date the model states, checked against a date a tool this turn actually
 * returned — B1307.
 *
 * `read_day` answered with the fixture's own day, 2026-05-01; the model's
 * own trailing sentence named a different one. The card or preview the
 * person is looking at always carries the true date (it comes straight off
 * the tool's own result) — this is the words beside it disagreeing with it.
 */
describe("a date said that no tool, today or the person actually gave", () => {
  test("a date the read tool did not return is caught, not left to stand", async () => {
    create
      .mockResolvedValueOnce(calls("read_day", { trip: "reise", date: "2026-05-01" }))
      .mockResolvedValueOnce(says("Der Tag vom 2026-05-02 enthält: Worte."))
      .mockResolvedValueOnce(says("Der Tag vom 2026-05-01 enthält: Worte."));
    const answered = await read(await ask("was steht im tag vom 1. mai"));

    expect(create).toHaveBeenCalledTimes(3);
    expect(String(answered.body.answer)).not.toContain("2026-05-02");
    expect(String(answered.body.answer)).toContain("2026-05-01");
  });

  test("and left alone when the date it names is the one the tool gave", async () => {
    create
      .mockResolvedValueOnce(calls("read_day", { trip: "reise", date: "2026-05-01" }))
      .mockResolvedValueOnce(says("Der Tag vom 2026-05-01 enthält: Worte."));
    const answered = await read(await ask("was steht im tag vom 1. mai"));
