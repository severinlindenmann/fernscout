...
/**
 * B1874 — the upload route answers with each accepted file's staged id, and
 * that id is the only picture a HEIC has in a browser that cannot decode one
 * locally. The grid used to throw the reply away.
 */
describe("a tile keeps the staged id its upload came back with", () => {
  test("an uploaded photograph gets the thumb URL for its own id", async () => {
    const fetchMock = vi.fn(async () =>
      new Response(
        JSON.stringify({
          runId: "run-1",
          accepted: [{ filename: "IMG_0001.heic", id: "abc123.heic" }],
          rejected: [],
          stagedBytes: 10,
        }),
    ...
    Object.defineProperty(input, "files", {
      value: [new File(["x"], "IMG_0001.heic", { type: "image/heic" })],
      configurable: true,
    });
    act(() => {
      input.dispatchEvent(new Event("change", { bubbles: true }));
    ...
    const send = [...container.querySelectorAll("button")].find((b) =>
      /upload|hochladen/i.test(b.textContent ?? ""),
    )!;
    await act(async () => {
      send.click();
    ...
    // The local object URL is still what is drawn — until the browser says it
    // cannot. What matters here is that the staged one is now there to fall
    // through to at all.
    const img = container.querySelector("img")!;
    act(() => img.dispatchEvent(new Event("error")));
    expect(container.querySelector("img")!.getAttribute("src")).toBe(
      "/api/helper/agent/studio/thumb/run-1/abc123.heic",
    );
  });
});
