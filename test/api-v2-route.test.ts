...
describe("problemsFrom", () => {
  ...
  // B1703 — a near-miss unknown key gets a did-you-mean, when the schema's
  // own shape is passed in; without it (the call above) the message is
  // unchanged, so every existing caller keeps working with no suggestion.
  it("suggests a near-miss unknown key when the shape is given", () => {
    const bad = { ...bareDay, transport_mode: "bus", declined: { transportMode: "n/a" } };
    const result = dayWrite.safeParse(bad);
    if (result.success) throw new Error("expected an unrecognised key to fail");
    const problems = problemsFrom(result.error, dayDoc.shape as unknown as Record<string, ZodType>);
    const row = problems.find((p) => p.problem.includes("transport_mode"));
    expect(row).toBeDefined();
    expect(row!.problem).toContain('did you mean "transportMode"');
  });

  it("offers no suggestion for a key that resembles nothing", () => {
    const bad = { ...bareDay, zzzzzzzz: "bus" };
    const result = dayWrite.safeParse(bad);
    if (result.success) throw new Error("expected an unrecognised key to fail");
    const problems = problemsFrom(result.error, dayDoc.shape as unknown as Record<string, ZodType>);
    const row = problems.find((p) => p.problem.includes("zzzzzzzz"));
    expect(row).toBeDefined();
    expect(row!.problem).not.toContain("did you mean");
  });
});
...
describe("readJson", () => {
  ...
  it("B2243 — a body over the JSON ceiling answers body_too_large at 413", async () => {
    const { JSON_BODY_MAX_BYTES } = await import("../lib/api/jsonBody");
    const request = new Request("https://example.com/v2/x", {
      method: "POST",
      body: JSON.stringify({ a: "x".repeat(JSON_BODY_MAX_BYTES) }),
    });
    const result = await readJson(request);
    if (result.ok) throw new Error("expected failure");
    ...
    expect(body.details).toEqual({ maxBytes: JSON_BODY_MAX_BYTES });
  });

  it("B2243 — a chunked body with no Content-Length is held to the same ceiling", async () => {
    const { JSON_BODY_MAX_BYTES } = await import("../lib/api/jsonBody");
    const chunk = new Uint8Array(1024 * 1024).fill(0x20);
    let sent = 0;
    const stream = new ReadableStream<Uint8Array>({
      pull(controller) {
        // Never ends on its own: only the ceiling stops the read.
        sent += chunk.byteLength;
        controller.enqueue(chunk);
      },
    });
    const request = new Request("https://example.com/v2/x", { method: "POST", body: stream, duplex: "half" } as RequestInit);
    const result = await readJson(request);
    if (result.ok) throw new Error("expected failure");
    expect(result.response.status).toBe(413);
    expect(sent).toBeLessThanOrEqual(JSON_BODY_MAX_BYTES + 3 * chunk.byteLength);
  });
