...
describe("the trip fields that had no writer", () => {
  ...
  test("every field the reader knows is now either written or decided", () => {
    ...
    const written = [
      ...
      "figures",
      "test",
      // B2021 — `createTrip` always declines `plan` for free (see the
      // "declined" comment just below): a brand new trip never has a route
      // yet, and `NewTrip` has no field one could arrive through. The
      // top-level `plan` key is still never written — only `declined.plan`
      // is — the same shape `days` (not itself a `KNOWN_TRIP_FIELDS` member)
      // would have had, had it been one.
      "plan",
      ...
      // and (B2021) `plan`, both of which it always writes for free — a
      // brand new trip never has either yet, and `NewTrip` has no field a
      // planned route could arrive through in the first place — see
      // `lib/tripWrite.ts`'s own comment on `createTrip`.
      "declined",
    ];
