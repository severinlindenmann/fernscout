...
describe("assemble_day", () => {
  test("a date with nothing missing proposes creating the day", async () => {
    ...
    const ran = await runTool(USERNAME, "assemble_day", { trip: TRIP, date: "2026-05-04" }, say, "2026-05-06", [], "", WEB_CALLER);
  ...
  test("undated content implying more than one day asks which, before anything else", async () => {
    ...
    void a;
    void b;
    const ran = await runTool(USERNAME, "assemble_day", { trip: TRIP }, say, "2026-05-06", [], "", WEB_CALLER);
