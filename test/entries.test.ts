...
describe("grouping", () => {
  ...
  /**
   * B1871. The merge is by name, and a name is as coarse as whoever wrote it.
   * Two days about twelve kilometres apart both named for the region around
   * them became one stop — and a route of one stop is not a route, so the trip
   * printed no map page at all (`routeFor` in lib/photobook/source.ts needs
   * two).
   */
  test("days sharing a coarse name but twelve kilometres apart stay two places", () => {
  ...
  test("four days in one town are still one place", () => {
    const places = getPlaces("u/stay-2026");

    expect(places).toHaveLength(1);
    expect(places[0].firstDate).toBe("2026-05-01");
    expect(places[0].lastDate).toBe("2026-05-04");
  });
