...
describe("the tools", () => {
   ...
   */
  test.skipIf(!hasPaid())("are the reads the plan names, and B900's writes and one link", () => {
  ...
  test("a tool nobody has is a fact the model can read, not a crash", async () => {
    const { ok, result } = await runTool("alex", "delete_day", {}, say, "2026-09-07", [], "", WEB_CALLER);
   ...
   * fixed confirmation `paid/whatsapp/lib/whatsapp/dispatch.ts` sends after a press reads
...
describe("what a turn costs", () => {
  ...
  //
  // **B1449 — rebased on the wire prefix, not a proxy for it.** This test
  // used to measure `tool.properties + tool.describe`, which is not what
  // `rounds()` (lib/helper/model.ts) sends: `toolSchemas()` also puts the
  // tool `name`, the `input_schema` wrapper and the `required` array on the
  // wire, and that part is per-tool boilerplate the old metric never counted.
  // The gap was already 17% at B1449's filing and grows with every tool
  // added, so a budget tracking the proxy drifts further from the thing it
  // guards on every future raise. Measured the same day: the old metric read
  // 8,523 tokens — **27 short** of its own 8,550 ceiling, at 51 tools against
  // the 47 it was set for, so the next tool would have tripped it. That near
  // miss is the ticket's point rather than an aside: the gauge was about to
  // read full while the real prefix had already reached
  // **10,179** — `JSON.stringify(toolSchemas(TOOLS)) +
  // threadSystemPrompt(...)`, the actual string `rounds()` sends. The ceiling
  // is reset here from that baseline, not raised from the old number, because
  // the two were never the same quantity.
  //
  // **Why one ceiling, not one per channel.** B1891 narrows which areas a
  // WhatsApp turn is offered (`WHATSAPP_AREAS`, three areas), so a WhatsApp
  // round's real wire prefix is smaller than this. It is not measured here:
  // `paid/test/helper-tool-areas.test.ts` already measures that channel's own
  // numbers, for its own reason (staying above Haiku's cache floor, not
  // staying under a model-attention budget). The web door still receives
  // `toolSchemas(TOOLS)` — the whole registry, `channelAreas("web")` in
  // lib/helper/model.ts — so it is still the largest prefix any turn can
  // reach, and the one this ceiling has to hold the line against; a
  // per-channel ceiling would let the web door's prompt grow unwatched every
  // time WhatsApp's narrower one still fit under it.
  //
  // Headroom: 10,179 measured, 11,000 ceiling — **821 tokens**, worth about
  // six more tools at the ~131 tokens/tool this registry currently averages
  // (26,630 chars of schemas ÷ 51 tools ÷ 4), or a few sentences of prompt.
  // Enough for the next tool or two to land without tripping the build over a
  // number nobody budgeted for; not so much that the ceiling stops meaning
  // anything. If this binds again, read the "what to do when this fails"
  // comment below before raising it further.
  //
  // **This ceiling exists for the model's attention, not the bill.** B1450
  // caches this same prefix, which makes the token count cheap to re-send but
  // does nothing about a model choosing worse among a longer list — see
  // B1049. Caching the prefix is not a reason to delete or loosen this test.
  const CEILING = 11000;
  ...
  test("the prompt and the tool list stay under the ceiling", () => {
    // The actual wire prefix `rounds()` sends for a web turn (the largest any
    // channel reaches — see the comment above `CEILING`): the system prompt
    // plus `toolSchemas(TOOLS)` exactly as `JSON.stringify` would serialise it
    // for the API call, names/`input_schema`/`required` included — B1449.
    const wire = JSON.stringify(toolSchemas(TOOLS));
    const characters = threadSystemPrompt("2026-09-07").length + wire.length;
    const tokens = Math.round(characters / 4);
    // Reported on every run, pass or fail, so the budget is visible without
    // having to break the build to see it — B1449.
    console.log(`helper thread wire prefix: ~${tokens} tokens against a ceiling of ${CEILING}.`);
    expect(
      tokens,
      `The wire prefix is ~${tokens} tokens against a ceiling of ${CEILING}. ` +
