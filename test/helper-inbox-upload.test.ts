...
describe("the owner uploading from the room's files pane", () => {
  ...
  test("a photograph's dimensions are measured once, at upload, and land on its sidecar (B1995)", async () => {
    await request("alex", [{ name: "beach.jpg", bytes: await paintJpeg(640, 480, 4), type: "image/jpeg" }]);
    const staged = listInbox("alex");
    expect(staged.media[0].dimensions).toEqual({ width: 640, height: 480 });
  });
