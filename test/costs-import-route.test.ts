 ...
 * covered by `paid/test/api-v2-print-inbox.test.ts`; this file keeps the deeper
...
describe("the whole file, kept in written order", { shuffle: false }, () => {
  ...
  describe("putting the agreed rows on the days", () => {
    ...
    // B1844 — a date with no day is filed to the trip itself, not attached
    // to the nearest one and not left unwritten.
    test("a date with no day is filed to the trip, not attached to the nearest one", async () => {
      const token = await ownerToken();
      const { body } = await applyCall(token, {
      ...
      expect(body.filedToTrip).toEqual([{ date: "2026-06-24", rows: 1 }]);
      expect(body.next).toMatch(/filed to the trip/);

      const { readTripJson } = await import("@/lib/api/tripFile");
      const { tripRef } = await import("@/lib/trips");
      const read = readTripJson(tripRef(OWNER, TRIP));
      expect(read?.trip.costs?.items).toEqual([
        { label: "Petrol", amount: 40, currency: "CHF", category: "transport" },
      ]);
    });
