...
describe("the published trip example", () => {
  ...
  test("carries every field of NewTrip except the three deliberately held back", () => {
    ...
    for (const key of newTripKeys()) {
      ...
      // `figuresMode` (B2021) is left out because the example already shows
      // `travellers` — the older, inline way to say how the party is drawn
      // — and `createTrip` prefers `figuresMode` over it when both are
      // given, which would silently make the worked `travellers` entry
      // below inert. One example cannot carry both, the same reasoning
      // `teaser` gets.
      if (key === "figuresMode") continue;
