...
describe("the receipt form", () => {
  ...
  test("the day starts as the day on the screen", () => {
    // B2167 — the date field shows the day in words (the year only when it
    // is not this one), and the grid under it has that day pressed.
    const html = render();
    expect(html).toContain('value="Samstag, 2. Mai');
    expect(html).toMatch(/aria-pressed="true" aria-label="Samstag, 2\. Mai"/);
  });
