 ...
 *
 * Live on `severin`, 2026-09-14: `handleContactCard` stored a shared card and
...
const EMAIL = "andreas.lindenmann@example.test";
...
describe("the line that says what is waiting", () => {
  ...
  test("names a staged contact card and its id, and nothing that is inside it", () => {
    stageCard(OWNER, "Andreas Lindenmann", EMAIL);

    const line = describeWaiting(OWNER);
    expect(line).toContain("andreas-lindenmann");
...
describe("the files pane", () => {
  test("draws a staged contact card, which it used to drop on the floor", () => {
    ...
    expect(files.find((f) => f.kind === "contact")?.name).toBe("andreas-lindenmann.vcf");
  });
});
...
describe("adding a waiting contact to a trip's byline", () => {
  test("trip_people reads the address off the card instead of asking for it again", async () => {
    ...
    const id = stageCard(OWNER, "Andreas Lindenmann", EMAIL);

    const ran = await runTool(OWNER, "trip_people", { trip: "Elsass", contact: id }, say, "2026-09-14");
    ...
    expect(ran.proposal?.arguments.person).toBe("Andreas Lindenmann");
  });
