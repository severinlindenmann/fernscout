...
function writeTrip(spec: (typeof TRIPS)[number]) {
  ...
  for (const [date, slug, title, draft] of [
    ...
    writeDayFixture(dir, OWNER, spec.id, {
      ...
      content: title,
      // And the day names it — B1884. A file no gallery mentions is served to
      // nobody but the people who were there now, so a fixture that means to
      // ask "who may see this day's photographs" has to attach them to the
      // day, the way every writer that puts bytes in that folder does.
      media: [{ src: `/media/${spec.id}/${slug}/01.jpg` }],
    });
  }
}
...
describe("a draft day's photographs", () => {
  ...
  /**
   * B1893 — and the spelling of the day folder decides nothing.
   *
   * `/ana/media/open-2026/UNFINISHED/01.jpg` served the bytes, publicly
   * cacheable, while `.../unfinished/01.jpg` 404'd: the draft gate looked the
   ...
   * under either spelling.** A published day's photograph is public bytes, so
   * there is nothing to protect by refusing a varied spelling — and refusing
   * one would be a second, quieter rule about case in a route that already
   * has one. What must not vary is whether the gate *runs*, which is the
   * draft row below.
   */
  describe("under a differently-cased day folder", () => {
    test("a draft day's are refused, exactly as the correctly-spelled one is", async () => {
      for (const viewer of ["anonymous", "stranger", "guest"]) {
        as(viewer);
        expect((await fetchPhoto("open-2026", "UNFINISHED")).status).toBe(404);
        expect((await fetchPhoto("open-2026", "unfinished")).status).toBe(404);
      }
    ...
    test("and still reach the owner, who may see the draft either way", async () => {
      as("owner");
      expect((await fetchPhoto("open-2026", "UNFINISHED")).status).toBe(200);
      expect((await fetchPhoto("open-2026", "unfinished")).status).toBe(200);
    });

    test("a published day's serve under both spellings", async () => {
      as("anonymous");
      expect((await fetchPhoto("open-2026", "ARRIVAL")).status).toBe(200);
      expect((await fetchPhoto("open-2026", "arrival")).status).toBe(200);
    });

    test("and a varied draft folder is no more cacheable than the plain one", async () => {
      as("owner");
      const varied = await fetchPhoto("open-2026", "UNFINISHED");
      expect(varied.headers.get("Cache-Control")).toBe("private, no-store");
    });
  });
});
...
describe("nothing under app/[user] decides drafts for itself", () => {
  ...
  const OWNER_ONLY = new Set([
    // B1981. Here "draft" is a *photobook* draft — one trip's arrangement of
    // its own photographs into a book, which is the owner's own working copy
    // and nobody else's business at any visibility. It is not an unpublished
    // day, so `draftsVisibleTo` has no opinion to have instead: the question
    // this route asks is "is this the owner", which is the whole of the rule.
