...
describe("the day board shows the day's own photographs", () => {
  test("a strip of tiles renders from the staged-thumb route, on the collapsed card", async () => {
    ...
    expect(img?.getAttribute("src")).toBe("/api/helper/alex/studio/thumb/run-1/p1");
  });
  ...
  test("expanding the day shows each photograph beside its chips, not a bare filename", async () => {
    ...
    // The old bare-filename row is gone — from the board's own day list;
    // the studio bar's back link (B2081 mounts the bar) truncates its own label.
    expect(container!.querySelector("ul span.truncate")).toBeNull();
  });
});
