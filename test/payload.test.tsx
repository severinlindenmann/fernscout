...
// B1947 — both fixtures start 2026-08-14. The window-size assertion below
// needs "today" to sit well inside the long (200-day) trip — near either
// end, `windowFor` clips the window against the trip's own boundary — while
// also being past the short (13-day) trip's own end, so that one's window is
// the clipped case the same assertion contrasts it with. This held by
// accident, for whichever day the suite happened to run on; pin it instead.
const DURING_THE_LONG_TRIP = new Date("2026-09-06T12:00:00Z");
...
beforeAll(() => {
  ...
  vi.setSystemTime(DURING_THE_LONG_TRIP);
});
...
describe("story payload", () => {
  it("sends a window of full days, however long the trip is", () => {
    ...
    // mechanism, stated as an assertion. "Today" (pinned above) sits well
    // inside the long trip, so its window opens full and uncapped.
    expect(long.days.length).toBe(2 * STORY_WINDOW + 1);
    // The short trip already ended by "today", so it opens on its last day
    // and the window is clipped by the end of the trip. What matters is
    // that neither is anywhere near the length of its trip.
