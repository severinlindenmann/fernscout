...
import { checkGpsImporter, isSaneFix, type GpsImporter } from "@/importers/gps/schema";
...
describe("google-timeline — Android's own shape (B1819)", () => {
  // Android writes `Timeline.json`: the same `semanticSegments` wrapper as
  // iOS, but coordinates as a bare degree-signed pair rather than a `geo:`
  // URI, numbers where iOS sends strings, and a visit's place nested one
  // level deeper, under `latLng`. B1819: this file used to detect as a
  // Timeline export and then silently import zero fixes.
  const androidFile = JSON.stringify({
    semanticSegments: [
      {
        startTime: "2026-06-22T06:00:00.000Z",
        endTime: "2026-06-22T07:00:00.000Z",
        timelinePath: [
          { point: "47.10000°, 8.10000°", durationMinutesOffsetFromStartTime: 0 },
          { point: "47.20000°, 8.20000°", durationMinutesOffsetFromStartTime: 30 },
        ],
      },
      {
        startTime: "2026-06-22T08:00:00.000+02:00",
      ...
      },
      {
        ...
        visit: {
          topCandidate: {
            placeId: "ChIJ-synthetic-0000",
            placeLocation: { latLng: "47.50000°, 8.50000°" },
  ...
  test("recognises Timeline.json", () => {
    expect(timeline.detect(androidFile, "Timeline.json")).toBe(true);
  });

  test("reads a non-zero, sane fix count from the object wrapper and degree-sign coordinates", () => {
    const out = timeline.parse(androidFile);
    expect(out.map((f) => f.lat)).toEqual([47.1, 47.2, 47.3, 47.4, 47.5]);
    expect(out.every(isSaneFix)).toBe(true);
  });

  test("reads a visit's place nested under latLng", () => {
    const out = timeline.parse(androidFile);
    expect(out[4]).toMatchObject({ lat: 47.5, lon: 8.5 });
  });
});
