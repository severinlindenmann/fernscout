...
describe("funnel", () => {
  test("a day.json written before this change has no funnel block and reads as collecting", () => {
    const dir = journal();
    const folder = dayInboxDir("u", "2026-05-04");
    fs.mkdirSync(folder, { recursive: true });
    fs.writeFileSync(path.join(folder, "day.json"), JSON.stringify({ without: ["costs"], weatherAsked: true }));
    const r = readDayReadiness("u", "2026-05-04");
    expect(r.funnel).toEqual({ stage: "collecting" });
    expect(r.without).toEqual(["costs"]);
    expect(r.weatherAsked).toBe(true);
    fs.rmSync(dir, { recursive: true, force: true });
  });

  test("writing funnel.asksSent leaves without/unrecorded/weatherAsked/location untouched", () => {
    journal();
    writeDayReadiness("u", "2026-05-04", {
      without: ["costs"],
      weatherAsked: true,
      location: { lat: 46.02, lon: 7.75, source: "browser" },
    });
    writeDayReadiness("u", "2026-05-04", { funnel: { asksSent: 1 } });
    const r = readDayReadiness("u", "2026-05-04");
    expect(r.without).toEqual(["costs"]);
    expect(r.weatherAsked).toBe(true);
    expect(r.location).toEqual({ lat: 46.02, lon: 7.75, source: "browser" });
    expect(r.funnel.asksSent).toBe(1);
  });

  test("writing one funnel field does not wipe the other funnel fields", () => {
    journal();
    writeDayReadiness("u", "2026-05-04", {
      funnel: { stage: "drafted", trip: "japan-2026", declined: ["words"] },
    });
    writeDayReadiness("u", "2026-05-04", { funnel: { asksSent: 1 } });
    const r = readDayReadiness("u", "2026-05-04");
  ...
  test("draftWords survives a round trip, simulating a thread that has expired", () => {
    journal();
    writeDayReadiness("u", "2026-05-04", {
      funnel: { draftWords: { title: "A day in Kyoto", prose: "We walked through the bamboo grove." } },
    });
    // The 24-hour thread that composed this is gone; only the folder remains.
    const r = readDayReadiness("u", "2026-05-04");
    expect(r.funnel.draftWords).toEqual({
      title: "A day in Kyoto",
      prose: "We walked through the bamboo grove.",
    });
  });
});
