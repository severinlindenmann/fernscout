...
describe("the whole file, kept in written order", { shuffle: false }, () => {
  describe("staging a file before any day exists", () => {
    test("the round trip: stage, write the day, file it, bucket empty", async () => {
      ...
      // The derivative in `media/`, and its sidecar in the trip's own
      // `meta/` — B1863 moved the sidecar out of the directory the media
      // route resolves into, so `media/` holds the photograph and nothing
      // else.
      const inTrip = fs.readdirSync(path.join(tripPath(), "media", DAY));
      expect(inTrip).toHaveLength(1);
      const sidecars = fs.readdirSync(path.join(tripPath(), "meta", DAY));
      expect(sidecars).toEqual([`${inTrip[0]}.meta.json`]);
    ...
    // B1627 — `storeMediaV2` now runs the same format/size check for
    // `kind: "photo"` before it branches on `intent.trip`, so a photo
    // declined straight to the inbox is checked exactly like one landing on
    // a trip: one guard in the shared function, not a second copy in the
    // inbox path.
    test("a photo declined to the inbox still gets a format check (B1627)", async () => {
      const token = await ownerToken();
      const staged = await stage(token, [{ name: "payload.exe", bytes: Buffer.from("MZ") }]);
      expect(staged.status).toBe(400);
      expect(staged.body.error).toBe("invalid_media");
    });

    /**
     * B1842 — what EXIF measured off a photograph is stored (`lib/inbox.ts`'s
     * `InboxMeta`) and used to be unreadable. `storeInboxFile` is the same
     * write the helper's own EXIF-reading upload door (`lib/inboxUpload.ts`)
     * calls, so seeding a sidecar with `measuredFrom: "exif"` here stands in
     * for that earlier read — this test is about the OUTPUT side, GET
     * exposing what is on disk, not re-testing EXIF parsing itself.
     */
    test("what EXIF measured is readable back, in the inbox and once filed", async () => {
      const token = await ownerToken();
      const { storeInboxFile } = await import("@/lib/inbox");
      const bytes = await jpeg(900, 600, 40);
      const { entry } = storeInboxFile(OWNER, "media", "DSC_measured.jpg", bytes, {
        lat: 35.6812,
        lon: 139.7671,
        takenAt: "2026-01-01T09:15:00",
        measuredFrom: "exif",
      });

      const listed = await readInbox(token);
      const row = listed.body.items.media.find((i: { id: string }) => i.id === entry.id);
      expect(row).toBeDefined();
      expect(row.lat).toBe(35.6812);
      expect(row.lon).toBe(139.7671);
      expect(row.takenAt).toBe("2026-01-01T09:15:00");
      expect(row.measuredFrom).toBe("exif");

      writeDay("measured-day", "2026-01-01");
      const filed = await fileIntoDay(token, entry.id, "2026-01-01-measured-day");
      expect(filed.status, JSON.stringify(filed.body)).toBe(201);
      expect(filed.body.lat).toBe(35.6812);
      expect(filed.body.lon).toBe(139.7671);
      expect(filed.body.takenAt).toBe("2026-01-01T09:15:00");
      expect(filed.body.measuredFrom).toBe("exif");
    });

    test("a photograph with no EXIF reads back with no capture fields at all", async () => {
      const token = await ownerToken();
      const staged = await stage(token, [{ name: "DSC_bare.jpg", bytes: await jpeg(900, 600, 90) }]);
      expect(staged.status).toBe(201);
      const listed = await readInbox(token);
      const row = listed.body.items.media.find((i: { id: string }) => i.id === staged.body.items[0].id);
      expect(row).toBeDefined();
      expect(row.lat).toBeUndefined();
      expect(row.lon).toBeUndefined();
      expect(row.takenAt).toBeUndefined();
      expect(row.measuredFrom).toBeUndefined();
      // Clean up so it does not leak into other tests' counts.
      const { DELETE } = await import("@/app/api/v2/[user]/inbox/[id]/route");
      await DELETE(
        new Request(`https://example.test/api/v2/${OWNER}/inbox/${row.id}`, {
          method: "DELETE",
          headers: headers({ authorization: `Bearer ${token}` }),
        }),
        { params: Promise.resolve({ user: OWNER, id: row.id as string }) },
      );
    });
  });
