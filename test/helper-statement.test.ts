...
describe("costs reach the days", () => {
  test("the whole file is read for free, and only agreed rows are written", async () => {
    ...
    expect((written.body.written as { filedToTrip: unknown[] }).filedToTrip).toHaveLength(1);
    expect(written.body.filedToTrip).toBe(1);
  });

  // B2056 — a bank that writes money going out as a positive number. Read
  // without the flip, every line used to vanish into \`outside\` and the
  // screen offered "File 0 costs"; now the sign is named as the reason.
  const SEVEN = [
    HEADER,
    ...STATEMENT_ROWS.map(line),
    line("08.03.2026,09.03.2026,Museum,9,00,EUR,Everyday"),
    line("09.03.2026,10.03.2026,Bus,2,80,EUR,Everyday"),
  ].join("\n");

  test("positive outflows read without the flip are counted as the wrong sign", async () => {
    const id = stage("export.csv", SEVEN);
    const unflipped = await read(
      await applyRoute(
        json("/api/helper/owner/statement/apply", {
          inbox: id,
          trip: "the-islands",
          mapping: { ...MAPPING, outgoingPositive: false },
        }),
        params,
      ),
    );
    expect(unflipped.body).toMatchObject({ read: 7, outside: 7, wrongSign: 7, spending: [] });

    const flipped = await read(
      await applyRoute(
        json("/api/helper/owner/statement/apply", { inbox: id, trip: "the-islands", mapping: MAPPING }),
        params,
      ),
    );
    expect(flipped.body).toMatchObject({ read: 7, outside: 0, wrongSign: 0 });
    expect(flipped.body.spending).toHaveLength(7);
  });

  test("a file whose outflows are already negative behaves as before", async () => {
    const negative = SEVEN.replace(/,"(\d)/g, ',"-$1');
    const id = stage("export.csv", negative);
    ...
    expect(answer.body).toMatchObject({ read: 7, outside: 0, wrongSign: 0 });
    expect(answer.body.spending).toHaveLength(7);
  });
