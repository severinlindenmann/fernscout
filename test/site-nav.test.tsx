...
describe("SiteNav", () => {
  ...
  /**
   * The credits-and-storage destination — B821, moved out of the nav by
   * B2016. It used to be its own owner-only tab; now the studio hub's own
   * Journal group (`components/studio/StudioHub.tsx`) is the only way in,
   * same shape as "the studio destination" below.
   */
  describe("the account destination", () => {
    test("is not a nav row for anybody, owner or reader", () => {
  ...
  /**
   * **The studio has no row here — B1958.**
   *
   * It had one from B1797 until the owner asked for the studio to keep the
   * shape the agent's control had: its own dark button in the header, and its
   * own section above this list in the phone panel. A row here *as well* was
   * two links to one page in one header.
   *
   * These tests used to assert the row was offered to an owner and withheld
   * from a reader. The second half is the one that mattered and it still
   * holds — it just holds by there being no row at all. The first half moved
   * to `PageHeader`, which is where the control moved; `test/page-header-title.test.tsx`
   * is where a regression that loses the owner's way in would now show.
   *
   * `STUDIO_DESTINATION` is deliberately still in `lib/navDestinations.ts`:
   * `lib/search.ts` reads the same table, and being findable by typing its
   * name is a separate question from drawing a tab.
   */
  describe("the studio destination", () => {
    test("is not a nav row for anybody, owner or reader, capability on or off", () => {
      for (const isOwner of [true, false]) {
        for (const extractEnabled of [true, false]) {
          site.isOwner = isOwner;
          site.extractEnabled = extractEnabled;
          try {
            expect(
              render(false),
