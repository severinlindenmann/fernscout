...
describe("committing a day", () => {
  ...
  test("B2205 — a staged picture whose header declares 15000×15000 is not moved into the day", async () => {
    const { craftedPng } = await import("./support/pictures");
    const bomb = craftedPng(15000, 15000);
    const fine = Buffer.from("an-ordinary-photo");
    const bombId = await stageFile("bomb.png", bomb);
    const fineId = await stageFile("fine.jpg", fine);
    await writeRunManifest(
      [
        { id: bombId, filename: "bomb.png", bytes: bomb.byteLength, kind: "image", date: DATE },
        { id: fineId, filename: "fine.jpg", bytes: fine.byteLength, kind: "image", date: DATE },
      ],
      [{ date: DATE, words: "A day.", answered: [] }],
    );

    const { commitDay } = await import("@/lib/extract/commit");
    expect((await commitDay(USER, RUN, DATE)).moved).toBe(1);
    const { listDayInbox } = await import("@/lib/inbox");
    expect(listDayInbox(USER, DATE).media.map((m) => m.filename)).toEqual(["fine.jpg"]);
  });
...
describe("committing a day, through the real route, all the way into the journal", () => {
  ...
  test("the created entry's own file says status: draft, and nothing else — the constraint this whole feature exists to keep", async () => {
    ...
    const { POST } = await import("@/app/api/helper/[user]/studio/commit/route");
    const res = await POST(
      new Request("http://x/api/helper/alex/studio/commit", {
        method: "POST",
  ...
  test("committing the same date twice leaves exactly one entry for that date, never a second one", async () => {
    ...
    const post = () =>
      import("@/app/api/helper/[user]/studio/commit/route").then(({ POST }) =>
        POST(
          new Request("http://x/api/helper/alex/studio/commit", {
            method: "POST",
            body: JSON.stringify({ run: RUN, date: DATE }),
...
describe("an imported day's order and title — B2081", () => {
  beforeEach(() => setup(10_000_000));

  async function commitThroughRoute(): Promise<{ entry: string | null }> {
    const { POST } = await import("@/app/api/helper/[user]/studio/commit/route");
    const res = await POST(
      new Request("http://x/api/helper/alex/studio/commit", {
        method: "POST",
    ...
    expect(res.status).toBe(200);
    return (await res.json()) as { entry: string | null };
  }

  async function entryOf(slug: string): Promise<{ title?: string; media?: { src: string; caption?: string }[] }> {
    const { readManifest } = await import("@/lib/staging/manifest");
    const { userDir } = await import("@/lib/users");
    const tripId = readManifest(USER, RUN)!.tripId!;
    const file = path.join(userDir(USER), "trips", tripId, "entries", `${DATE}-${slug}.json`);
    return JSON.parse(fs.readFileSync(file, "utf8"));
  }

  test("photographs keep the order they were taken in, whatever order they were uploaded in", async () => {
    const camera = fs.readFileSync("test/fixtures/ingest/camera.jpg");
    const whatsapp = fs.readFileSync("test/fixtures/ingest/camera-button-synthetic.jpg");
    // Committed in manifest order, so the morning one reaches the day folder
    // first — and `listDayInbox` lists newest first, which used to number
    // the evening one 01.
    const eveningId = await stageFile("a2.jpg", camera);
    const morningId = await stageFile("a1.jpg", whatsapp);
    await writeRunManifest(
      [
        { id: morningId, filename: "a1.jpg", bytes: whatsapp.byteLength, kind: "image", takenAt: `${DATE}T09:12:00`, caption: "morning" },
        { id: eveningId, filename: "a2.jpg", bytes: camera.byteLength, kind: "image", takenAt: `${DATE}T17:40:00`, caption: "evening" },
      ],
      [{ date: DATE, words: "A whole day.", answered: [] }],
    );
    const { entry } = await commitThroughRoute();
    const day = await entryOf(entry!);
    expect(day.media?.map((m) => [m.caption, m.src.split("/").pop()])).toEqual([
      ["morning", "01.jpg"],
      ["evening", "02.jpg"],
    ]);
  });

  test("a day lands titled with the place the person answered, and its slug says so", async () => {
    const camera = fs.readFileSync("test/fixtures/ingest/camera.jpg");
    const id = await stageFile("p.jpg", camera);
    await writeRunManifest(
      [{ id, filename: "p.jpg", bytes: camera.byteLength, kind: "image", date: DATE }],
      [{ date: DATE, words: "Market, then the harbour.", location: "Hội An", answered: [] }],
    );
    const { entry } = await commitThroughRoute();
    expect(entry).toBe("hoi-an");
    expect((await entryOf(entry!)).title).toBe("Hội An");
  });

  test("with no place answered, the title is the date in words and the slug is never day or day-2", async () => {
    const camera = fs.readFileSync("test/fixtures/ingest/camera.jpg");
    const id = await stageFile("d.jpg", camera);
    await writeRunManifest(
      [{ id, filename: "d.jpg", bytes: camera.byteLength, kind: "image", date: DATE }],
      [{ date: DATE, words: "Nothing about where.", answered: [] }],
    );
    const { entry } = await commitThroughRoute();
    expect(entry).toBe("2-july-2019");
    expect((await entryOf(entry!)).title).toBe("2 July 2019");
  });

  test("titleForDay falls back to the date when the answer is a sentence, not a name", async () => {
    const { titleForDay } = await import("@/lib/extract/commit");
    expect(titleForDay("Somewhere along the coast road, I honestly could not tell you which town it was", DATE)).toBe(
      "2 July 2019",
    );
    expect(titleForDay("  ", DATE)).toBe("2 July 2019");
    expect(titleForDay("Lisbon", DATE)).toBe("Lisbon");
  });
});
