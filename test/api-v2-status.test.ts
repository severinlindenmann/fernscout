...
import { MAX_IMPORT_ROWS } from "@/lib/contacts/importRows";
...
describe("GET /api/v2/status", () => {
  test("answers with no journal and no auth", async () => {
    ...
    // B2179 round 2 — the separate, hard pixel ceiling, discoverable before a
    // caller hits it: an image can be under `imageMaxEdge` on both sides and
    // still be refused on total pixel count.
    expect((body.limits as Record<string, unknown>).imageMaxPixels).toBeGreaterThan(0);
    // B1848 — discoverable before a caller hits it importing contacts.
    expect((body.limits as Record<string, unknown>).contactsImportMaxRows).toBe(MAX_IMPORT_ROWS);
    // B2243 — the JSON body ceiling and the cost-line bounds, readable first.
