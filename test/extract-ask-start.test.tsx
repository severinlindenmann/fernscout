 ...
 * manifest, not on the screen." This asserts on what `POST .../studio/start`
 * actually received — the same thing a manifest read back would show, since
 * the route writes the body verbatim (`app/api/helper/[user]/studio/start/
 * route.ts`).
 *
...
async function mount(consentedSpeech: boolean) {
  ...
  await act(async () => {
    root!.render(
      <LocaleProvider locale="en" dictionary={dictionaryFor("en")}>
        {/* The photos page is under the studio layout's bar; its intro's
            primary lives there since B2137. */}
  ...
  // Let checkResume's GET .../studio/runs resolve.
  await act(async () => {
