...
describe("the deletion and invite links carry no-referrer (B1690)", () => {
  ...
  test.each(sources)("%s overrides the baseline referrer policy", async (source) => {
    ...
    expect(rule, `next.config.ts must declare a rule for ${source}`).toBeDefined();
    const referrer = rule!.headers.find((h) => h.key.toLowerCase() === "referrer-policy")?.value;
    expect(referrer).toBe("no-referrer");

    const baselineIndex = all.findIndex((r) => anyPath(r.source));
    const ruleIndex = all.findIndex((r) => r.source === source);
    expect(ruleIndex).toBeGreaterThan(baselineIndex);
  });
});
...
describe("an SVG served out of somebody's content folder", () => {
  ...
  beforeEach(() => {
    ...
    // The day that names them — B1884: the media route serves a file because
    // something names it, so a fixture about response *headers* still has to
    // be a fixture about a real day's photographs.
    writeDayFixture(dir, "alex", "asia-2023", {
      slug: "day",
      date: "2026-01-01",
      title: "A day",
      media: [
        { src: "/media/asia-2023/day/placeholder.svg" },
        { src: "/media/asia-2023/day/photo.jpg" },
      ],
    });
    clearConfigCache();
    clearUserCache();
