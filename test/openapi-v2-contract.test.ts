...
describe("every error code a route answers with is published", () => {
  ...
  test("and nothing in ERROR_CODES is answered by no route", () => {
    // Open core: without paid/, codes that only paid routes answer. Written by
    // open-core/split/split.mjs from the paid/ it split off, never by hand.
    const paidOnly: string[] = existsSync(join(process.cwd(), "paid")) ? [] : ["unknown_order","unknown_payment","invalid_amount","postcards_disabled","photobook_disabled","no_database","bad_token","bad_method","unknown_recipient","unknown_photo"];
    const dead = Object.keys(ERROR_CODES).filter((code) => !spoken.has(code) && !paidOnly.includes(code)).sort();
