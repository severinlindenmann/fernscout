...
describe("a day's own part of the route (B2199)", () => {
  const VISIBLE = new Set(["2026-06-22", "2026-06-23"]);

  beforeEach(() => {
    writeTrack(USER, TRIP, {
      generated: "2026-06-24T00:00:00Z",
      segments: [
        { from: "2026-06-22T09:00:00Z", day: "2026-06-22", points: [[47, 8], [47.01, 8]] },
        { from: "2026-06-23T09:00:00Z", day: "2026-06-23", points: [[46, 7], [46.01, 7]] },
        // A legacy segment, derived before `day` existed.
        { from: "2026-06-21T09:00:00Z", points: [[45, 6], [45.01, 6]] },
      ],
    });
  });

  test("hands back only the segments tagged with that date", () => {
    expect(dayTrack(USER, TRIP, VISIBLE, "2026-06-22")).toEqual([[[47, 8], [47.01, 8]]]);
  });

  test("never another date's segment", () => {
    const track = dayTrack(USER, TRIP, VISIBLE, "2026-06-22")!;
    expect(track).not.toContainEqual([[46, 7], [46.01, 7]]);
  });

  test("never a legacy segment with no day at all", () => {
    for (const date of ["2026-06-21", "2026-06-22", "2026-06-23"]) {
      const track = dayTrack(USER, TRIP, VISIBLE, date) ?? [];
      expect(track).not.toContainEqual([[45, 6], [45.01, 6]]);
    }
  });

  test("undefined for a date with nothing to draw, not an empty array pretending there was", () => {
    expect(dayTrack(USER, TRIP, VISIBLE, "2026-06-23")).toEqual([[[46, 7], [46.01, 7]]]);
    expect(dayTrack(USER, TRIP, new Set(["2026-06-30"]), "2026-06-30")).toBeUndefined();
  });
