 ...
 *
 * Same jsdom + `createRoot` harness `test/extract-credits-screen.test.tsx`
 ...
 * screen, and `extract.resume.daysStay` — "days you've already finished are
 * safe" — was left behind on the list. The brief requires that reassurance
...
type Fixture = {
  /** What `GET .../extract/runs` (the pure list) hands back for this run —
   *  the pre-touch state `ResumeScreen`/`continueRun` see. */
  listed: Record<string, unknown>;
  /** What `GET .../extract/run` (which really does call `extendOnTouch`)
...
function stubResumeFetch(fixture: Fixture) {
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/extract/run?run=")) {
        return {
          ok: true,
...
async function renderResumed(fixture: Fixture) {
  ...
  // Let `checkResume`'s GET .../extract/runs resolve and ResumeScreen mount.
  await act(async () => {
  ...
  // Let DayBoard's own GET .../extract/run resolve and fire onLoaded.
  await act(async () => {
    await Promise.resolve();
    await Promise.resolve();
  });
...
describe("the resumed run's notice carries the reassurance in every state", () => {
  for (const [state, fixture] of Object.entries(FIXTURES) as [keyof typeof FIXTURES, Fixture][]) {
    test(`${state}: the reassurance and the state's own sentence both appear, reassurance first`, async () => {
      ...
      expect(reassurance!.textContent).toContain("part of your journal and stay");
