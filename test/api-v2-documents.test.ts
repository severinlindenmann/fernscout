...
// B1907 — the half of B1540 that was left open. Every writer calls this, so
// a day that names a country carries the code the book prints its name from.
describe("deriveCountryCode", () => {
  const day = (over: Partial<DayFile>): DayFile => ({
    slug: "2026-07-13-a-day",
    title: "A day",
    date: "2026-07-13",
    content: "",
    status: "draft",
  ...
  it("derives the code from an unambiguous name", () => {
    const d = day({ country: "Hungary" });
    deriveCountryCode(d);
    expect(d.countryCode).toBe("HU");
  });

  it("leaves the field absent rather than guessing at a name it does not know", () => {
    const d = day({ country: "Somewhere over the Andaman Sea" });
    deriveCountryCode(d);
    expect(d.countryCode).toBeUndefined();
  });

  it("never overwrites a code the caller sent", () => {
    const d = day({ country: "Hungary", countryCode: "AT" });
    deriveCountryCode(d);
    expect(d.countryCode).toBe("AT");
  });

  it("keeps a decline rather than answering the question twice", () => {
    const d = day({
      country: "Hungary",
      declined: { countryCode: "no country code was recorded for this day" },
    });
    deriveCountryCode(d);
    expect(d.countryCode).toBeUndefined();
  });
