...
describe("a trip that is still running", () => {
  /**
   * These three need the trip to actually be running, and that is not
   * something a fixture can state any more.
   *
   * `status: current` was the author's own word until v2 retired the field
   * (`docs/v2-migration/00-decisions.md`); a trip's status is now derived from
   * its dates **at read time** against the real clock (`deriveStatus`,
   * lib/trips.ts:210). `isOver` short-circuits on that status, so the `now`
   * these tests hand to `getCostSummary` reaches the arithmetic but never the
   * question "is this trip over" — which is the one the three assertions
   * below are about.
   *
   * So the clock is what has to move. B1946: on 2026-09-20 the fixture's own
   * end date arrived and all three went red having changed nothing, on a
   * branch about a map. Only `Date` is faked — the React render in this file's
   * other blocks still wants real timers.
   */
  const DURING_THE_TRIP = new Date("2026-09-06T12:00:00Z");

  beforeEach(() => {
    vi.useFakeTimers({ toFake: ["Date"] });
    vi.setSystemTime(DURING_THE_TRIP);
  });
