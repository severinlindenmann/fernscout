...
/**
 * `unusedPhotoCount` — B1828. A photo whose `date` was never hand-edited has
 * only `takenAt` (the camera's reading), the ordinary case for every import
 * nobody touched by hand. `commitDay` still moves it by that camera date and
 * never writes `photo.date` back, so this has to match the same way
 * `commitDay` does or a finished, fully-committed run keeps looking unused.
 */
describe("unusedPhotoCount — B1828, camera-dated photographs after commit", () => {
  test("a camera-dated photo whose day committed is not unused", () => {
    const committed = run({
      photos: [{ id: "p1", filename: "p1.jpg", bytes: 1, kind: "image", takenAt: "2026-09-15T10:00:00" }],
      days: [{ date: "2026-09-15", committed: true, answered: [] }],
    });
    expect(unusedPhotoCount(committed)).toBe(0);
  });

  test("a hand-dated photo whose day committed is not unused", () => {
    const committed = run({
      photos: [{ id: "p1", filename: "p1.jpg", bytes: 1, kind: "image", date: "2026-09-15" }],
      days: [{ date: "2026-09-15", committed: true, answered: [] }],
    });
    expect(unusedPhotoCount(committed)).toBe(0);
  });

  test("a photo whose day has not committed is still unused", () => {
    const uncommitted = run({
      photos: [{ id: "p1", filename: "p1.jpg", bytes: 1, kind: "image", takenAt: "2026-09-15T10:00:00" }],
      days: [{ date: "2026-09-15", committed: false, answered: [] }],
    });
    expect(unusedPhotoCount(uncommitted)).toBe(1);
  });

  test("an undated photo still counts as unused, on purpose", () => {
    const undated = run({ photos: [{ id: "p1", filename: "p1.jpg", bytes: 1, kind: "image" }], days: [] });
    expect(unusedPhotoCount(undated)).toBe(1);
  });

  test("a dropped photo never counts, committed or not", () => {
    const dropped = run({
      photos: [{ id: "p1", filename: "p1.jpg", bytes: 1, kind: "image", takenAt: "2026-09-15T10:00:00", dropped: true }],
      days: [],
    });
    expect(unusedPhotoCount(dropped)).toBe(0);
  });
});
...
describe("spentOnRun — what the expiry warning tells somebody they are about to lose", () => {
  ...
  test("sums every spend under this run's own ref, across more than one enrich (R30's widened ref)", async () => {
    ...
    // Two `studio/enrich` calls on the same run, two different photo-set
