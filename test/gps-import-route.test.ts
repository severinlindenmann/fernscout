...
async function monthsHeld(token: string) {
  const { GET } = await import("@/app/api/v2/[user]/gps/route");
  const response = await GET(
    new Request(`https://example.test/api/v2/${OWNER}/gps`, {
      headers: headers({ authorization: `Bearer ${token}` }),
    }),
    { params: Promise.resolve({ user: OWNER }) },
  );
...
beforeAll(async () => {
  ...
  // A day on the trip so `getDays`/`readerTrack`-based reads elsewhere in
  // this suite have something to resolve — derivation itself no longer needs
  // a published day at all (B2202 rework: it covers every trip date).
  writeDayFixture(dir, OWNER, TRIP, { slug: "last-day", date: "2026-06-24" });
...
describe("the whole file, kept in written order", { shuffle: false }, () => {
  ...
  describe("the standalone purge — B1843 addendum, and its own security review", () => {
    test("names the months held, over the agent's own read door", async () => {
      const token = await tokenFor(OWNER_EMAIL);
      await importCall(token, { kind: "gps", text: fixesJsonl(5, "2026-09-01") });

      const before = await monthsHeld(token);
      expect(before.status).toBe(200);
      expect(before.body.monthsHeld).toContain("2026-09");
    });

    // Security review, 2026-09-24: purging for real from a bearer token, in
    // one call with no second step, is exactly the "identity cookie proves
    // an address and grants nothing by itself" line AGENTS.md draws for a
    // delete API call in general — and this is the one category of data the
    // same file calls the most sensitive in the repository. There is now no
    // `DELETE` export at all: Next answers an unexported method with its own
    // 405, and the store is never touched from this door, whatever the
    // token holds — a bearer-facing door that can never delete anything is
    // truthful in a way a route that always answered 403 and still had to
    // document a success response could not be (`test/openapi-v2-contract
    // .test.ts` requires one of each per documented operation).
    test("DELETE is not a route here any more — nothing left to call, nothing touched", async () => {
      const token = await tokenFor(OWNER_EMAIL);
      await importCall(token, { kind: "gps", text: fixesJsonl(5, "2026-09-01") });

      // The module exports no `DELETE` at all — which is what makes Next
      // answer an unexported method with its own 405 for a real request,
      // rather than this route ever reaching the store.
      const routeModule = await import("@/app/api/v2/[user]/gps/route");
      expect("DELETE" in routeModule).toBe(false);

      const { readRange } = await import("@/lib/gps/store");
      expect(readRange(OWNER, 0, Date.now() + 1e10).length).toBeGreaterThan(0);
      expect(fs.existsSync(path.join(dir, OWNER, "gps", "2026-09.jsonl"))).toBe(true);
    });

    test("the openapi document has no delete for this path — nothing to lie about", async () => {
      const { openApiDocumentV2 } = await import("@/lib/api/v2/openapi");
      const document = openApiDocumentV2();
      expect(Object.keys(document.paths["/api/v2/{user}/gps"] ?? {})).not.toContain("delete");
    });
  });
});
